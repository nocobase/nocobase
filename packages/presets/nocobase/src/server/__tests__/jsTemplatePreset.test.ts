...
describe('JS Template preset boundary', () => {
  it('ships one canonical plugin package and has no standalone VSC metadata', () => {
    ...
    const clientV1Map = readJson('packages/core/app/client/src/.plugins/packageMap.json');
    ...
    expect(preset.deprecated).not.toContain(JS_TEMPLATE_PACKAGE);
    ...
    expect(clientV1Map).not.toHaveProperty(VSC_FILE_PACKAGE);
