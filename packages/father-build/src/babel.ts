...
export default async function(opts: IBabelOpts) {
  ...
  function getTsconfigCompilerOptions(path: string) {
    ...
    if (config && config.extends) {
      const exts = parseTsconfig(join(cwd, config.extends));
      const compilerOptions = exts ? exts.compilerOptions : {};
      return merge(compilerOptions, config.compilerOptions || {});
    }
  ...
  function createStream(src) {
    ...
    return vfs
      ...
      .pipe(
        gulpIf(tsConfig.sourceMap, sourcemaps.init())
      )
      ...
      .pipe(
        gulpIf(tsConfig.sourceMap, sourcemaps.write('.'))
      )
