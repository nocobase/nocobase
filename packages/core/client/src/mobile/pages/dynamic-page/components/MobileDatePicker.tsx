    ...
    const handleConfirm = useCallback(
      (nextValue) => {
        ...
        if (dateOnly) {
          ...
          if (showTime) {
            onChange(dayjs(nextValue).format('YYYY-MM-DD HH:mm:ss'));
          } else {
            onChange(dayjs(nextValue).startOf(picker).format('YYYY-MM-DD'));
          }
        } else {
          const selectedDateTime = new Date(nextValue);
          onChange(selectedDateTime);
        }
      },
      [showTime, onChange, picker, dateOnly, utc],
    );
  ...
  return (
    <Space.Compact>
      <Select
        ...
        onChange={(nextValue) => {
          const nextFormat = getPickerFormat(nextValue);
          field.setComponentProps({
            picker: nextValue,
            format: nextFormat,
          });
          newProps.picker = nextValue;
          newProps.format = nextFormat;
