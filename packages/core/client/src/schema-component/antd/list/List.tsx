              ...
              >
                {field.value?.length
                  ? field.value.map((item, index) => {
                    return (
                      <ListItem key={index} field={field} index={index} schema={getSchema(index)} />
                    );
                  })
                  : null}
              </AntdList>
...
function ListItem(props: { field: any; index: number; schema: any }) {
  const { isMobileLayout } = useMobileLayout();
