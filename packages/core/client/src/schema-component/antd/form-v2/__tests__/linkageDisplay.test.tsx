...
describe('form field displaylinkage', () => {
  it(' name field hidden when single field value is apple', () => {
    const ComponentA = () => {
  ...
  it(' name field required when single field value is apple', async () => {
    const ComponentA = () => {
      const useFormBlockProps = () => {
        const form = useMemo(() => createForm(), []);
        return {
    ...
    const { getAllByPlaceholderText, getByText } = render(<ComponentA />);
    const f1 = getAllByPlaceholderText('single')[0];
    fireEvent.input(f1, { target: { value: 'apple' } });
