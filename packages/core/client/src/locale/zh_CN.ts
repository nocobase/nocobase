export default {
  'Display <1><0>10</0><1>20</1><2>50</2><3>100</3></1> items per page':
    '每页显示 <1><0>10</0><1>20</1><2>50</2><3>100</3></1> 条',
  'Page number': '页码',
  'Page size': '每页条数',
  ...
  'Math.js comes with a large set of built-in functions and constants, and offers an integrated solution to work with different data types':
    'Math.js 包含大量内置函数和常量，并提供了集成的解决方案来处理不同的数据类型。',
  ...
  'Link to': '关联',
  'Link to description':
    '用于快速创建表关系，可兼容大多数普通场景。适合非开发人员使用。作为字段存在时，它是一个下拉选择用于选择目标数据表的数据。创建后，将同时在目标数据表中生成当前数据表的关联字段。',
  'Sub-table': '子表格',
  'System info': '系统信息',
  'Created at': '创建日期',
  ...
  'One to many description':
    '用于创建一对多关系，比如一个国家会有多个城市。作为字段存在时，它是一个子表格用于显示目标数据表的数据。创建后，会在目标数据表里自动生成一个多对一字段。',
  'Many to one description':
    '用于创建多对一关系，比如一个城市只能属于一个国家，一个国家可以有多个城市。作为字段存在时，它是一个下拉选择用于选择目标数据表的数据。创建后，会在目标数据表里自动生成一个多对一字段。',
  'Many to many description':
    '用于创建多对多关系，比如一个学生会有多个老师，一个老师也会有多个学生。作为字段存在时，它是一个下拉选择用于选择目标数据表的数据。',
  ...
  'Identifier for program usage. Support letters, numbers and underscores, must start with an letter.':
    '用于程序使用的标识符，支持字母、数字和下划线，必须以字母开头。',
  Drawer: '抽屉',
  Dialog: '对话框',
  'Delete action': '删除操作',
  ...
  'This is likely a NocoBase internals bug. Please open an issue at <1>here</1>':
    '这可能是 NocoBase 内部的问题，你可以在<1>这里</1>将该错误反馈给我们，我们会尽快修复',
  'Render Failed': '渲染失败',
  Feedback: '反馈问题',
  'Try again': '重试一下',
