export default {
  'Display <1><0>10</0><1>20</1><2>50</2><3>100</3></1> items per page':
    '各ページ<1><0>10</0><1>20</1><2>50</2><3>100</3></1> 件表示',
  'Meet <1><0>All</0><1>Any</1></1> conditions in the group':
    'グループ内の<1><0>すべて</0><1>一部</1></1> の条件を満たす',
  'Open in<1><0>Modal</0><1>Drawer</1><2>Window</2></1>':
    '<1><0>モーダル</0><1>ドロワー</1><2>ウィンドウ</2></1>で開く',
  ...
  'This is a demo text, **supports Markdown syntax**.':
    'これはデモテキストです。 **マークダウン構文をサポートしています。**',
  Filter: 'フィルター',
  ...
  'Randomly generated and can be modified. Support letters, numbers and underscores, must start with an letter.':
    'ランダムに生成され、変更可能です。 アルファベット、数字、アンダースコアをサポートし、アルファベットから始まる必要があります。',
  Edit: '編集',
  ...
  'Link to': 'リンク',
  'Link to description':
    'コレクションの関連付けを素早く作成するためにしようされ、ほとんどの一般的なシナリオに対応しています。開発者以外の方のしようにも適しています。フィールドとして存在する場合、参照元コレクションのレコードを選択するために使用されるドロップダウンです。一度作成されると、参照先コレクションに現在のコレクションの関連フィールドが同時に生成されます。',
  'Sub-table': 'サブテーブル',
  ...
  'One to one description':
    '1対1の関係を作成するために使用します。例えば、1つのユーザーは1つのプロファイルを持つことになります。',
  'One to many description':
    '1対多の関係を作成するために使用します。例えば、国には多くの都市があり、都市は1つの国にしか存在できません。フィールドとして存在する場合、それは参照先コレクションのレコードを表示するために使用されるサブテーブルです。作成されると、多対一のフィールドが参照先コレクションに自動的に生成されます。',
  'Many to one description':
    'これは多対一の関係を作るために使われます。例えば、都市は1つの国にしか属さず、国は複数の都市を持つことができます。 フィールドとして存在する場合、参照先コレクションのレコードを選択するために使用されるドロップダウンです。 一度作成されると、対象の参照先コレクションに多対一のフィールドが自動的に生成されます。',
  'Many to many description':
    'これは多対多の関係を作成するために使用されます。例えば、生徒は複数の教師を持ち、教師は複数の生徒を持つことになります。 フィールドとして存在する場合、参照先コレクションのレコードを選択するために使用されるドロップダウン選択です。',
  'Foreign key 1': '外部キー1',
  'Foreign key 2': '外部キー2',
  'Add filter': 'フィルターを追加',
  ...
  'All collections use general action permissions by default; permission configured individually will override the default one.':
    'すべてのコレクションは、デフォルトで一般操作権限を使用します。 各コレクションに対して個別に権限を設定することができます。',
  'Allows configuration of the whole system, including UI, collections, permissions, etc.':
    'UI設定、コレクション設定、権限設定、システム設定等、システム全体の設定を許可する',
  'New menu items are allowed to be accessed by default.':
    '新しいメニュー項目を追加すると、デフォルトでアクセスが許可されます',
  ...
  'After clicking the custom button, the following field values will be assigned according to the following form.':
    '現在のカスタムボタンをクリックすると、次のフォームに従って次のフィールド値が割り当てられます。',
  'After clicking the custom button, the following fields of the current record will be saved according to the following form.':
    'カスタムボタンをクリックすると、現在のレコードの次のフィールドが次の形式に従って保存されます。',
  ...
  Print: 'Print',
  'Single select and radio fields can be used as the grouping field':
    '単一選択フィールドとラジオ フィールドをグループ化フィールドとして使用できます',
  'Sign up successfully, and automatically jump to the sign in page':
    'アカウントの登録に成功すると、ログインページにリダイレクトされます',
  /* add */
  Unique: '重複を許可しない',
  ...
  'Continue after any branch succeeded, or exit after any branch failed':
    'いずれかの分岐で成功し続行するか、分岐が失敗した後終了します',
  Delay: '遅延実行',
  Duration: '間隔',
  'End Status': '終了状態',
  ...
  'This is likely a NocoBase internals bug. Please open an issue at <1>here</1>':
    'これはNocoBaseの内部バグの可能性があります。 <1>こちら</1>で問題を報告してください',
  'Render Failed': 'レンダリングに失敗しました',
  Feedback: 'フィードバック',
  'Try again': '再試行',

  'Click or drag file to this area to upload': 'クリックまたはドラッグしてファイルをアップロード',
  'Support for a single or bulk upload, file size should not exceed':
    '単一または複数のファイルをアップロードできます。ファイルサイズは',
  'Default title for each record': '各レコードのデフォルトタイトル',

  'If collection inherits, choose inherited collections as templates':
    'コレクションが継承されている場合、継承されたコレクションをテンプレートとして選択してください',
  'Select an existing piece of data as the initialization data for the form':
    '既存のデータを選択して、フォームの初期化データとして使用します',
  'Only the selected fields will be used as the initialization data for the form':
    '選択したフィールドのみがフォームの初期化データとして使用されます',
  ...
  'If the compatibility check fails, you should change the dependent version to meet the version requirements.':
    '互換性チェックに失敗した場合は、依存関係のバージョンを変更して、バージョン要件を満たす必要があります。',
  ...
  Homepage: 'ホームページ',
  'Drag and drop the file here or click to upload, file size should not exceed 30M':
    'ファイルをここにドラッグ＆ドロップするか、クリックしてアップロードしてください。ファイルサイズは10Mを超えてはいけません',
