...
import { useCollectionManagerV2 } from 'packages/core/client/src/application';
...
const useSourceFieldsOptions = () => {
  ...
  sources.forEach((item: string) => {
    ...
    const inherits = cm.getInheritCollections(item);
    const result = inherits.map((v) => {
      const fields: FieldOptions[] = cm.getParentCollectionFields(v, item);
      return {
        type: 'group',
        key: v,
