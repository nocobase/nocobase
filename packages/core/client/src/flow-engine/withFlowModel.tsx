...
interface WithFlowModelOptionsBase {
  settings?: {
    component?: React.ComponentType<{ model?: FlowModel; uid?: string, children?: React.ReactNode; [key: string]: any }>; // 设置组件，作为wrapper
...
function WithFlowModelInternal<P extends object>(
  ...
  if (propModel) {
    ...
    if (options?.use && options?.uid) {
      throw new Error('Cannot provide both model prop and use/uid options. When use and uid are provided, model should not be provided.');
    }
