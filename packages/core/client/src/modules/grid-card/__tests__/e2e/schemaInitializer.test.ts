...
test('create actions', async ({ page, mockPage }) => {
...
test('create fields of item', async ({ page, mockPage, mockRecord }) => {
...
test.pgOnly('create inherit fields', async ({ page, mockPage }) => {});
...
test('create custom action buttons of item', async ({ page, mockPage, mockRecord }) => {
  const nocoPage = await mockPage(oneEmptyGridCardBlock).waitForInit();
  await mockRecord('general');
  await nocoPage.goto();
