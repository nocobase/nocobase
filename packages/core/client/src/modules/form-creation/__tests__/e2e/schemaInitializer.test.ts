...
test('create form block in page', async ({ page, mockPage }) => {
  await mockPage().goto();
...
test('create and delete fields', async ({ page, mockPage }) => {
  ...
  await expect(page.getByLabel('block-item-CollectionField-general-form-general.manyToOne.nickname')).not.toBeVisible();
...
test('create custom actions', async ({ page, mockPage }) => {
  await mockPage(oneEmptyForm).goto();
