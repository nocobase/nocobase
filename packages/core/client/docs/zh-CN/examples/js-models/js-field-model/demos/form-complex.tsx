...
'</select>';const el=ctx.element.querySelector('#cty');el?.addEventListener('change',()=>{ctx.setValue(el.value);const p=map[el.value];const old=String(ctx.form?.getFieldValue?.(['phone'])||'').replace(/^[+][0-9]+ */,'');ctx.form?.setFieldValue?.(['phone'],p+' '+old);});`,
                        },
                      },
                    },
                  },
                ...
                subModels: {
                  field: {
                    ...
                    stepParams: {
                      ...
                      jsSettings: {
                        runJs: {
                          code: `const v=String(ctx.getValue()||'');ctx.element.innerHTML='<input id="ph" style="width:100%;padding:4px 8px" value="'+v.replace(/"/g,'&quot;')+'" />';ctx.element.querySelector('#ph')?.addEventListener('input',(e)=>ctx.setValue(String(e.target.value||'')));`,
                        },
                      },
                    },
                  },
                ...
                subModels: {
                  field: {
                    ...
                    stepParams: {
                      ...
                      jsSettings: {
                        runJs: {
                          code: `const v=String(ctx.getValue()||'');ctx.element.innerHTML='<div><input id="prod" style="width:100%;padding:4px 8px" value="'+v.replace(/"/g,'&quot;')+'" /><ul id="sug" style="list-style:none;padding-left:0;margin:6px 0 0"></ul></div>';const ipt=ctx.element.querySelector('#prod');const ul=ctx.element.querySelector('#sug');ipt?.addEventListener('input',async()=>{ctx.setValue(String(ipt.value||''));try{const {data}=await ctx.api.axios.get('/suggest',{params:{keyword:ipt.value}});const items=(data?.items||[]).slice(0,6);ul.innerHTML=items.map(x=>'<li style="padding:2px 4px;cursor:pointer;">'+x+'</li>').join('');Array.from(ul.children).forEach((li)=>li.addEventListener('click',()=>{ipt.value=li.textContent;ctx.setValue(li.textContent);ul.innerHTML='';}));}catch(e){ul.innerHTML='';}});`,
                        },
                      },
                    },
                  },
                ...
                subModels: {
                  field: {
                    ...
                    stepParams: {
                      ...
                      jsSettings: {
                        runJs: {
                          code: `ctx.element.innerHTML='<div><input id="cd" style="width:100%;padding:4px 8px" /><div id="msg" style="font-size:12px;margin-top:6px;color:#999">仅允许大写字母与数字，长度 4-8</div></div>';const ip=ctx.element.querySelector('#cd');const m=ctx.element.querySelector('#msg');ip?.addEventListener('input',()=>ctx.setValue(String(ip.value||'')));ip?.addEventListener('blur',async()=>{try{const {data}=await ctx.api.axios.post('/validate-code',{value:ip.value});if(data.valid){m.style.color='#52c41a';m.textContent='可用';}else{m.style.color='#ff4d4f';m.textContent=data.message||'不可用';}}catch(e){m.style.color='#ff4d4f';m.textContent='校验失败';}});`,
                        },
                      },
                    },
                  },
                ...
                subModels: {
                  field: {
                    ...
                    stepParams: {
                      ...
                      jsSettings: {
                        runJs: {
                          code: `const v=String(ctx.getValue()||'');ctx.element.innerHTML='<input id="f" type="date" value="'+v+'" />';const f=ctx.element.querySelector('#f');f?.addEventListener('change',()=>{ctx.setValue(f.value);const t=String(ctx.form?.getFieldValue?.(['to'])||'');if(t&&f.value&&t<f.value){ctx.form?.setFieldValue?.(['to'],f.value);}});`,
                        },
                      },
                    },
                  },
                ...
                subModels: {
                  field: {
                    ...
                    stepParams: {
                      ...
                      jsSettings: {
                        runJs: {
                          code: `const v=String(ctx.getValue()||'');ctx.element.innerHTML='<input id="t" type="date" value="'+v+'" />';const t=ctx.element.querySelector('#t');t?.addEventListener('change',()=>{const f=String(ctx.form?.getFieldValue?.(['from'])||'');if(t.value&&f&&t.value<f){t.value=f;}ctx.setValue(t.value);});`,
                        },
                      },
                    },
                  },
                ...
                subModels: {
                  field: {
                    ...
                    stepParams: {
                      ...
                      jsSettings: {
                        runJs: {
                          code: `const arr=Array.isArray(ctx.getValue())?ctx.getValue():[];ctx.element.innerHTML='<div><div id="chips" style="margin-bottom:6px"></div><input id="tg" style="width:100%;padding:4px 8px" placeholder="输入后回车添加"/></div>';const chips=ctx.element.querySelector('#chips');const ipt=ctx.element.querySelector('#tg');function render(){chips.innerHTML=(ctx.getValue()||[]).map((t,i)=>'<span data-i="'+i+'" style="display:inline-block;margin:2px;padding:2px 6px;background:#f5f5f5;border-radius:10px;cursor:pointer">'+t+' ✕</span>').join('');Array.from(chips.children).forEach(el=>el.addEventListener('click',()=>{const i=Number(el.getAttribute('data-i'));const a=(ctx.getValue()||[]).slice();a.splice(i,1);ctx.setValue(a);render();}));}ctx.setValue(arr);render();ipt?.addEventListener('keydown',(e)=>{if(e.key==='Enter'){e.preventDefault();const v=ipt.value.trim();if(v){const a=(ctx.getValue()||[]).slice();a.push(v);ctx.setValue(a);ipt.value='';render();}}});`,
                        },
                      },
                    },
                  },
                ...
                subModels: {
                  field: {
                    ...
                    stepParams: {
                      ...
                      jsSettings: {
                        runJs: {
                          code: `const v=String(ctx.getValue()||'');ctx.element.innerHTML='<input id="up" type="number" step="0.01" min="0" style="width:100%;padding:4px 8px" value="'+v+'" />';const el=ctx.element.querySelector('#up');el?.addEventListener('input',()=>{ctx.setValue(el.value); ${calcTotalJS} });`,
                        },
                      },
                    },
                  },
                ...
                subModels: {
                  field: {
                    ...
                    stepParams: {
                      ...
                      jsSettings: {
                        runJs: {
                          code: `const v=String(ctx.getValue()||'');ctx.element.innerHTML='<input id="qt" type="number" step="1" min="0" style="width:100%;padding:4px 8px" value="'+v+'" />';const el=ctx.element.querySelector('#qt');el?.addEventListener('input',()=>{ctx.setValue(el.value); ${calcTotalJS} });`,
                        },
                      },
                    },
                  },
                ...
                subModels: {
                  field: {
                    ...
                    stepParams: {
                      ...
                      jsSettings: {
                        runJs: {
                          code: `const v=Number(ctx.getValue()||0);ctx.element.innerHTML='<div style="display:flex;gap:8px;align-items:center"><input id="ds" type="range" min="0" max="100" value="'+v+'"/><span id="dsv" style="width:40px;text-align:right">'+v+'%</span></div>';const sl=ctx.element.querySelector('#ds');const sv=ctx.element.querySelector('#dsv');function sync(){sv.textContent=sl.value+'%';ctx.setValue(sl.value); ${calcTotalJS} }sl?.addEventListener('input',sync);sync();`,
                        },
                      },
                    },
                  },
