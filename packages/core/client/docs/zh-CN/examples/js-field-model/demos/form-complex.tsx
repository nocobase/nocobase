                ...
                subModels: {
                  field: {
                    ...
                    stepParams: {
                      ...
                      jsSettings: {
                        runJs: {
                          code: `const v=String(ctx.getValue()||'');ctx.element.innerHTML='<div><input id="p" style="width:100%;padding:4px 8px" value="'+v.replace(/"/g,'&quot;')+'" /><ul id="sg" style="list-style:none;margin:6px 0 0;padding-left:0"></ul></div>';const p=document.getElementById('p');const sg=document.getElementById('sg');p?.addEventListener('input',async()=>{ctx.setValue(p.value);try{const {data}=await ctx.api.axios.get('/suggest',{params:{keyword:p.value}});sg.innerHTML=(data?.items||[]).slice(0,6).map(x=>'<li style="padding:2px 4px;cursor:pointer">'+x+'</li>').join('');Array.from(sg.children).forEach(li=>li.addEventListener('click',()=>{p.value=li.textContent;ctx.setValue(p.value);sg.innerHTML='';}));}catch(e){sg.innerHTML='';}});`,
                        },
                      },
                    },
                  },
                ...
                subModels: {
                  field: {
                    ...
                    stepParams: {
                      ...
                      jsSettings: {
                        runJs: {
                          code: `const v=String(ctx.getValue()||'');ctx.element.innerHTML='<div><input id="c" style="width:100%;padding:4px 8px" value="'+v.replace(/"/g,'&quot;')+'" /><div id="msg" style="font-size:12px;margin-top:6px;color:#999">仅允许大写字母与数字，长度 4-8</div></div>';const c=document.getElementById('c');const m=document.getElementById('msg');c?.addEventListener('input',()=>ctx.setValue(c.value));c?.addEventListener('blur',async()=>{try{const {data}=await ctx.api.axios.post('/validate-code',{value:c.value});if(data.valid){m.style.color='#52c41a';m.textContent='可用';}else{m.style.color='#ff4d4f';m.textContent=data.message||'不可用';}}catch(e){m.style.color='#ff4d4f';m.textContent='校验失败';}});`,
                        },
                      },
                    },
                  },
