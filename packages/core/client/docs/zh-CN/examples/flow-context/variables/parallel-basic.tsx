...
DemoFieldModel.registerFlow(
  defineFlow({
    ...
    steps: {
      text: {
        defaultParams: { text: 'User: {{ctx.record.username}}' },
...
const makeUsers = (count = 12) =>
  Array.from({ length: count }).map((_, i) => ({ id: i + 1, username: `user_${i + 1}` }));
...
class DemoPlugin extends Plugin {
  async load() {
    ...
    const baseBuf = { total: 0, variablesResolve: 0, variablesUnique: 0, usersList: 0 } as any;
    const optBuf = { total: 0, variablesResolve: 0, variablesUnique: 0, usersList: 0 } as any;
    const baseVarSigs = new Set<string>();
    const optVarSigs = new Set<string>();
          ...
          columns: [
            {
              ...
              subModels: { field: { use: 'DemoFieldModel' } },
              props: { title: '用户名' },
            },
          ],
        },
    ...
    tableBaseline.context.defineProperty('api', { value: baselineApi });
    tableOptimized.context.defineProperty('api', { value: optimizedApi });
    const fmStub = { bindToTarget: (_: string) => {}, unbindFromTarget: (_: string) => {} };
    tableBaseline.context.defineProperty('filterManager', { value: fmStub });
    tableOptimized.context.defineProperty('filterManager', { value: fmStub });

    // baseline：强制所有模板上行解析，并在前端补齐解析
    tableBaseline.context.defineMethod('resolveJsonTemplate', async function (template: any) {
      try {
          ...
          <div style={{ padding: 16 }}>
            <Space direction="vertical" style={{ width: '100%' }} size={16}>
              <Typography.Paragraph type="secondary">
                并排对比：左侧 Baseline（统一上行解析），右侧 Optimized（当前实现）。
              </Typography.Paragraph>
              <Space size={16} wrap>
                <Card title="Baseline 表格">
                  <BaselineStat />
                  <FlowModelRenderer model={tableBaseline} />
                </Card>
                <Card title="Optimized 表格">
                  <OptimizedStat />
                  <FlowModelRenderer model={tableOptimized} />
                </Card>
              </Space>
            </Space>
          </div>
