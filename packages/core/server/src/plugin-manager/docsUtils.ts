...
import { PLUGIN_STATICS_PATH, getExposeReadmeUrl, getPackageJsonPath } from './clientStaticUtils';
...
function getMenuData(plugins: PluginData[], currentLang: string, defaultLang = 'en-US', locale: object = {}) {
  const packages = plugins.map((plugin) => PluginManager.getPackageName(plugin.name));
 ...
 * get doc sidebar
 */
export const getDocSidebar = (plugins: PluginData[], currentLang: string, defaultLang = 'en-US'): string => {
...
function getPluginList(plugins: PluginData[], lang: string) {
  const packageJsonPaths = plugins
    .map((plugin) => {
      let packageName;
      let packageJson;
      try {
        packageName = PluginManager.getPackageName(plugin.name);
      ...
      return {
        packageName,
        readmeUrl: getExposeReadmeUrl(packageName, lang),
        description: packageJson[`description.${lang}`] || packageJson.description,
      };
    })
    .filter(Boolean);
  return packageJsonPaths
    .map((item) =>
      item.description
        ? `* [${item.packageName}](${item.readmeUrl}) - ${item.description}`
        : `* [${item.packageName}](${item.readmeUrl})`,
    )
    .join('\n');
}
 ...
 */
export const getDocReadme = (plugins: PluginData[], currentLang: string, defaultLang = 'en-US') => {
  const menuData = getMenuData(plugins, currentLang, defaultLang);
