...
export default class SourceDownload extends Command {
  ...
  static prompts: PromptsCatalog = {
    ...
    replace: {
      ...
      initialValue: false,
      hidden: (values) => SourceDownload.hideOutputDirAndReplaceSteps(values),
    },
    devDependencies: {
      type: 'boolean',
  ...
  async download(): Promise<DownloadCommandResult> {
    const { flags } = await this.parse(SourceDownload);
