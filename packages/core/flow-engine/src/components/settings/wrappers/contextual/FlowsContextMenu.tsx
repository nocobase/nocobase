...
const FlowsContextMenuWithModel: React.FC<ModelProvidedProps> = observer(
  ({ model, children, enabled = true, position = 'right', showDeleteButton = true }) => {
    ...
    const [configurableFlowsAndSteps, setConfigurableFlowsAndSteps] = useState<ConfigurableFlow[]>([]);
    ...
    const handleMenuClick = useCallback(
      async ({ key }: { key: string }) => {
        if (key === 'delete') {
          ...
          const [flowKey, stepKey] = key.split(':');
          try {
            await model.openFlowSettings({ flowKey, stepKey });
          } catch (error) {
            // 用户取消或出错，静默处理
      ...
      [isStepMenuItemDisabled, model, t],
    );
      ...
      try {
        ...
        const result = await Promise.all(
          flowsArray.map(async (flow) => {
            const flowSteps = await getFlowSettingSteps(model as FlowModel, flow, flow.key);
            ...
            const configurableSteps = await Promise.all(
              Object.entries(flowSteps).map(async ([stepKey, actionStep]) => {
                ...
                if (await shouldHideStepInSettings(model as FlowModel, flowForSettings, actionStep)) {
                  return null;
                }
                ...
                const mergedUiSchema = await resolveStepUiSchema(model as FlowModel, flowForSettings, actionStep);
            ...
            return configurableSteps.length > 0 ? { flow: flowForSettings, steps: configurableSteps } : null;
          }),
        );
