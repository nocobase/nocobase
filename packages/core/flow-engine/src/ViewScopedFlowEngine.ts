...
export function createViewScopedEngine(parent: FlowEngine): FlowEngine {
  ...
  // Prefer delegating everything to parent by default, except the few
  // APIs that must use local instance/cache.
  const localOnly = new Set<keyof FlowEngine | string>(['_modelInstances', '_applyFlowCache', 'executor', 'context']);
  ...
  const handler: ProxyHandler<FlowEngine> = {
    ...
    set(target, prop: string, value: unknown) {
      ...
      return Reflect.set(parent as any, prop, value);
    },
