...
describe('FlowEngine.saveModel concurrent save prevention', () => {
  ...
  it('should persist a fork through its master without serializing local props', async () => {
    const saveSpy = vi.spyOn(mockRepository, 'save').mockImplementation(async (savedModel) => savedModel.serialize());
    const fork = model.createFork({ disabled: true });

    const result = await fork.saveStepParams();

    expect(saveSpy).toHaveBeenCalledWith(model, { onlyStepParams: true });
    expect(result.props).toEqual({ title: 'Test Model' });
  });

  it('should run an overridden saveStepParams hook on the fork before persisting its master', async () => {
    const saveHook = vi.fn();
    class HookedFlowModel extends FlowModel {
      override async saveStepParams() {
        saveHook((this as FlowModel & { isFork?: boolean }).isFork === true);
        return super.saveStepParams();
      }
    }
    engine.registerModels({ HookedFlowModel });
    const master = engine.createModel<HookedFlowModel>({
      uid: 'hooked-model-uid',
      use: 'HookedFlowModel',
      props: { title: 'Hooked Model' },
    });
    const saveSpy = vi.spyOn(mockRepository, 'save').mockImplementation(async (savedModel) => savedModel.serialize());
    const fork = master.createFork({ disabled: true });

    const result = await fork.saveStepParams();

    expect(saveHook).toHaveBeenCalledOnce();
    expect(saveHook).toHaveBeenCalledWith(true);
    expect(saveSpy).toHaveBeenCalledWith(master, { onlyStepParams: true });
    expect(result.props).toEqual({ title: 'Hooked Model' });
  });

  it('should persist a nested fork through the root master without serializing fork-local props', async () => {
    let persistedModel: FlowModel | undefined;
    vi.spyOn(mockRepository, 'save').mockImplementation(async (savedModel) => {
      persistedModel = savedModel;
      return savedModel.serialize();
    });
    const fork = model.createFork({ disabled: true });
    const nestedFork = fork.createFork({ hidden: true });

    const result = await nestedFork.saveStepParams();
