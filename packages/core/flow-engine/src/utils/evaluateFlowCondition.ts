...
export async function evaluateFlowCondition({
  ...
  const targetLogger = (ctx as any)?.logger || model.context.logger;
  ...
  const jsonLogic = getJsonLogic(ctx);
  const evaluator: ConditionEvaluator = ((left: any, operator: string, right: any) => {
    return applyConditionOperator(model, operator, left, right, jsonLogic);
...
function getJsonLogic(context: FlowRuntimeContext): any | null {
  // 直接从运行态上下文读取 app.jsonLogic，与 linkageRules 的用法保持一致
  return (context as any)?.app?.jsonLogic || null;
}

function applyConditionOperator(model: FlowModel, operator: string, left: any, right: any, jsonLogic: any): boolean {
  if (jsonLogic && typeof jsonLogic.apply === 'function') {
    try {
      ...
      model?.context?.logger?.warn?.(
        { err: error },
        `FlowEngine: jsonLogic evaluation failed for operator '${operator}'`,
      );
    }
  }
  return false;
