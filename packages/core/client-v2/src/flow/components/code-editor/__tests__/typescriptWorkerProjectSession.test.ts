...
`;
  ...
  it('keeps serialized auto-import code actions while skipping details for ordinary entries', async () => {
    const session = createTypeScriptProjectSession({ workerFactory: inMemoryFactory() });
    const code = 'ab';
    const currentProject: CodeEditorTypeScriptProject = {
      currentFilePath: 'src/main.ts',
      files: [
        ...
        { content: 'export const abc = 42;', path: 'src/helper.ts' },
      ],
    };
    const completion = await session.getCompletionResult(currentProject, code.length, code, true);
    const autoImport = completion?.options.find((entry) => entry.label === 'abc');
    expect(autoImport?.detail).toMatch(/Auto import/);
    expect(autoImport?.apply).toBeTypeOf('function');
    const dispatch = vi.fn();
    if (typeof autoImport?.apply === 'function') {
      autoImport.apply({ dispatch } as never, autoImport, 0, code.length);
    }
    expect(dispatch).toHaveBeenCalledWith(
      expect.objectContaining({
        changes: expect.arrayContaining([
          expect.objectContaining({ insert: expect.stringMatching(/import.*abc/s) }),
          expect.objectContaining({ from: 0, insert: 'abc', to: code.length }),
        ]),
      }),
    );
  });

  it('filters unsupported package worker auto imports while keeping workspace auto imports', async () => {
    const session = createTypeScriptProjectSession({ workerFactory: inMemoryFactory() });
    ...
    const currentProject: CodeEditorTypeScriptProject = {
      ...
      files: [
        { content: externalCode, path: 'src/main.ts' },
        { content: 'export const workspaceThing = 42;', path: 'src/helper.ts' },
      ],
      rewriteBuiltInAutoImports: true,
    };

    const externalCompletion = await session.getCompletionResult(
      currentProject,
      externalCode.length,
      externalCode,
      true,
    );
    expect(externalCompletion?.options.some((entry) => entry.label === 'externalThing') ?? false).toBe(false);

    const workspaceCode = 'workspaceTh';
    const workspaceCompletion = await session.getCompletionResult(
      currentProject,
      workspaceCode.length,
      workspaceCode,
      true,
    );
    expect(workspaceCompletion?.options.some((entry) => entry.label === 'workspaceThing')).toBe(true);
  });
  ...
  it('keeps rewriting built-in worker auto imports after an incremental project update', async () => {
    const session = createTypeScriptProjectSession({ workerFactory: inMemoryFactory() });
    const initialCode = 'useEff';
    const currentProject: CodeEditorTypeScriptProject = {
      currentFilePath: 'src/main.tsx',
      files: [{ content: initialCode, path: 'src/main.tsx' }],
      rewriteBuiltInAutoImports: true,
      typeLibraryIds: ['react'],
    };

    const initialCompletion = await session.getCompletionResult(currentProject, initialCode.length, initialCode, true);
    expect(initialCompletion?.options.find((entry) => entry.label === 'useEffect')?.detail).toBe(
      'Auto import from ctx.libs.React',
    );

    const updatedCode = 'useSta';
    const updatedCompletion = await session.getCompletionResult(currentProject, updatedCode.length, updatedCode, true);
    const useStateCompletion = updatedCompletion?.options.find((entry) => entry.label === 'useState');
    const dispatch = vi.fn();

    expect(useStateCompletion?.detail).toBe('Auto import from ctx.libs.React');
    if (typeof useStateCompletion?.apply === 'function') {
      useStateCompletion.apply({ dispatch } as never, useStateCompletion, 0, updatedCode.length);
    }
    const changes = JSON.stringify(dispatch.mock.calls[0]?.[0]?.changes || []);
    expect(changes).toContain('const { useState } = ctx.libs.React;');
    expect(changes).not.toContain(`from 'react'`);
  });

  it('rewrites a worker default built-in auto import to a direct ctx.libs declaration', async () => {
    const session = createTypeScriptProjectSession({ workerFactory: inMemoryFactory() });
    const code = 'Reac';
    const currentProject: CodeEditorTypeScriptProject = {
      currentFilePath: 'src/main.ts',
      declarationFiles: [
        {
          content: `declare module 'react' { export default function React(): void; }`,
          path: 'src/library.d.ts',
        },
      ],
      files: [{ content: code, path: 'src/main.ts' }],
      rewriteBuiltInAutoImports: true,
    ...
    const completion = await session.getCompletionResult(currentProject, code.length, code, true);
    const reactCompletion = completion?.options.find((entry) => entry.label === 'React');
    const dispatch = vi.fn();

    if (typeof reactCompletion?.apply === 'function') {
      reactCompletion.apply({ dispatch } as never, reactCompletion, 0, code.length);
    }
    const changes = JSON.stringify(dispatch.mock.calls[0]?.[0]?.changes || []);
    expect(changes).toContain('const React = ctx.libs.React;');
    expect(changes).not.toContain('const { React }');
  });

  it('does not duplicate an existing worker ctx.libs React binding', async () => {
    const session = createTypeScriptProjectSession({ workerFactory: inMemoryFactory() });
    ...
    if (typeof useEffectCompletion?.apply === 'function') {
      useEffectCompletion.apply({ dispatch } as never, useEffectCompletion, code.length - 'useEff'.length, code.length);
    }
    const changes = JSON.stringify(dispatch.mock.calls[0]?.[0]?.changes || []);
