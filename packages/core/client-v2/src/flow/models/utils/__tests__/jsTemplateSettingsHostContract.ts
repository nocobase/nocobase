...
import { getJsTemplateSettingStepKey } from '@nocobase/runjs/settings';
...
function createDescriptor(templateId: string): RunJSSourceSettingsDescriptor {
  return {
    entryId: templateId,
    settingsSchemaHash: `${templateId}-schema-v1`,
    defaults: {
      label: 'Default label',
      pageSize: 20,
      displayOptions: {
...
function createUpdatedDescriptor(templateId: string): RunJSSourceSettingsDescriptor {
  return {
    entryId: templateId,
    settingsSchemaHash: `${templateId}-schema-v2`,
    defaults: {
...
function createNextDescriptor(templateId: string): RunJSSourceSettingsDescriptor {
  return {
    entryId: templateId,
    settingsSchemaHash: `${templateId}-schema-v1`,
    defaults: {
...
export async function assertJsTemplateSettingsHostContract(options: {
  model: FlowModel;
  flowKey: string;
  ...
  let currentDescriptor = createDescriptor(String(sourceBinding.templateId));
  const nextDescriptor = createNextDescriptor(String(nextSourceBinding.templateId));

  RunJSSourceResolverRegistry.registerResolver({
    sourceMode: 'js-template',
    resolve: () => ({ code: '' }),
    getSettingsDescriptor: async (input) =>
      input.sourceBinding.templateId === nextSourceBinding.templateId ? nextDescriptor : currentDescriptor,
  });
  ...
  currentDescriptor = createUpdatedDescriptor(String(sourceBinding.templateId));
  ...
  expect(conditionError).toMatchObject({
    ...
    templateId: nextDescriptor.entryId,
  ...
  expect(openFlowSettings).toHaveBeenNthCalledWith(1, {
    flowKey,
    stepKey: getJsTemplateSettingStepKey(String(sourceBinding.templateId), 'apiKey'),
  });
  expect(openFlowSettings).toHaveBeenNthCalledWith(2, {
    flowKey,
    stepKey: getJsTemplateSettingStepKey(String(sourceBinding.templateId), 'options'),
  });
}
