...
import {
  ...
  createJsTemplateSourceBindingStep,
  createJsTemplateSourceModeStep,
...
export { INLINE_SOURCE_MODE, JS_TEMPLATE_SOURCE_MODE };
export const JS_ITEM_OWNER_KIND = 'flowModel.itemSettings';

export type JSItemSourceMode = RunJSSourceMode;
...
const jsItemSource = createJsTemplateSourcePlumbing<JSItemRuntimeModel>({
  flowKey: 'jsSettings',
  stepKey: 'runJs',
...
export function normalizeJSItemSourceMode(value: unknown): JSItemSourceMode {
  return normalizeJsTemplateSourceMode(value);
}
...
export function hasRunnableJSItemSource(model: JSItemRuntimeModel, inlineCode?: string): boolean {
  const runJs = getJSItemRunJsStepParams(model);
  if (normalizeJSItemSourceMode(runJs.sourceMode) === JS_TEMPLATE_SOURCE_MODE) {
...
export function createJSItemSourceModeStep(): StepDefinition {
  return createJsTemplateSourceModeStep({
    kind: 'js-item',
    component: JS_ITEM_JS_TEMPLATE_FULL_SOURCE_FIELD,
...
export function createJSItemSourceBindingStep(): StepDefinition {
  return createJsTemplateSourceBindingStep({
    kind: 'js-item',
    component: JS_ITEM_JS_TEMPLATE_FULL_SOURCE_FIELD,
    hooks: {
      defaultParams: getJSItemSourceDefaultParams,
...
export function createJSItemRunJsUISchema(options: { scene: string; minHeight?: string } = { scene: 'block' }) {
  return createJsTemplateRunJsUISchema({
    kind: 'js-item',
    scene: options.scene,
    surfaceStyle: 'render',
...
export async function getJSItemRuntimeFlowSettingSteps(
  ...
  return createJsTemplateSettingSteps<JSItemRuntimeModel>({
    descriptor,
    settings: isRecord(params.settings) ? params.settings : {},
    component: JS_ITEM_JS_TEMPLATE_SETTINGS_STEP_FIELD,
    syncValue: syncJsTemplateSettingToRunJs,
    afterParamsSave: refreshJSItemAfterSettingsSave,
  });
}
...
async function getJsTemplateSettingsDescriptor(model: JSItemRuntimeModel, params: Record<string, unknown>) {
  return jsItemSource.getSettingsDescriptor(model, params);
}

function syncJsTemplateSettingToRunJs(ctx: FlowSettingsContext<JSItemRuntimeModel>, fieldName: string, value: unknown) {
...
function normalizeRuntimeError(error: unknown): JSItemRuntimeError {
  return normalizeJsTemplateRuntimeError(error, {
    ...
    outdatedHint: 'Refresh the item settings and choose the current template.',
