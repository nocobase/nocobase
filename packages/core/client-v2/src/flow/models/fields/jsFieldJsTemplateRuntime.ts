...
export type JSFieldSourceMode = RunJSSourceMode;
...
const jsFieldSource = createJsTemplateSourcePlumbing<JSFieldRuntimeModel>({
  flowKey: 'jsSettings',
  stepKey: 'runJs',
...
export function hasRunnableJSFieldSource(model: JSFieldRuntimeModel, inlineCode?: string): boolean {
  const runJs = getJSFieldRunJsStepParams(model);
  if (normalizeJSFieldSourceMode(runJs.sourceMode) === JS_TEMPLATE_SOURCE_MODE) {
...
export function createJSFieldSourceModeStep(): StepDefinition {
  return createJsTemplateSourceModeStep({
    kind: 'js-field',
    component: JS_FIELD_JS_TEMPLATE_FULL_SOURCE_FIELD,
...
export async function getJSFieldRuntimeFlowSettingSteps(
  ...
  return createJsTemplateSettingSteps<JSFieldRuntimeModel>({
    descriptor,
    settings: isRecord(params.settings) ? params.settings : {},
    component: JS_FIELD_JS_TEMPLATE_SETTINGS_STEP_FIELD,
...
async function getJsTemplateSettingsDescriptor(model: JSFieldRuntimeModel, params: Record<string, unknown>) {
  return jsFieldSource.getSettingsDescriptor(model, params);
}

function syncJsTemplateSettingToRunJs(
...
function normalizeRuntimeError(error: unknown): JSFieldRuntimeError {
  return normalizeJsTemplateRuntimeError(error, {
    ...
    outdatedHint: 'Refresh the field settings and choose the current template.',
