...
export interface JSItemSourceModeFieldProps {
  value?: string;
  onChange?: (value: string) => void;
  disabled?: boolean;
}

export const JSItemSourceModeField: React.FC<JSItemSourceModeFieldProps> = (props) => {
  const ctx = useFlowContext<{
    engine?: {
      flowSettings?: {
        components?: Record<string, React.ElementType<JSItemSourceModeFieldProps> | undefined>;
      };
    };
  }>();
  const FullSourceField = ctx?.engine?.flowSettings?.components?.[JS_ITEM_LIGHT_EXTENSION_FULL_SOURCE_FIELD];

  if (FullSourceField && FullSourceField !== JSItemSourceModeField) {
    return <FullSourceField {...props} />;
  }

  return <JSItemInlineSourceModeField {...props} />;
};

const JSItemInlineSourceModeField: React.FC<JSItemSourceModeFieldProps> = ({ value, onChange, disabled }) => {
