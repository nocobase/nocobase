...
function getEmptySettingsDescriptor(sourceBinding: Record<string, unknown>): RunJSSourceSettingsDescriptor {
  const templateId = String(sourceBinding.templateId);
  return {
    entryId: templateId,
    settingsSchemaHash: `${templateId}-empty-settings`,
    defaults: {},
    schema: {
      type: 'object',
      properties: {},
...
ctx.render(
  <span data-testid="level-label" style={{ color: ctx.settings.vipColor }}>
    {ctx.item.index}:{ctx.record.level}:{ctx.settings.vipColor}:{ctx.runJsSource.context.jsTemplate.templateId}
  </span>
);
      `,
      version: 'v2',
    ...
    await waitFor(() => {
      expect(screen.getByTestId('level-label')).toHaveTextContent('2:VIP:#f5222d:jtt_level_label');
    });
  ...
  it('refreshes the current item after selecting a different JS Item entry', async () => {
    RunJSSourceResolverRegistry.registerResolver({
      ...
      resolve: (input) => ({
        code:
          input.sourceBinding?.templateId === 'jtt_open_message'
