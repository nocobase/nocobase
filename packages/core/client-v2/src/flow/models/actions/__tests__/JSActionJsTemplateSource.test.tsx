...
import { runJSActionRuntime } from '../jsActionJsTemplateRuntime';
import {
  createActionModel,
...
describe('JSActionModel JS Template source', () => {
  ...
  it('runs a JS Template action through the runtime resolver and only toggles the current action loading', async () => {
    const resolve = vi.fn(() => ({
      code: `
...
ctx.message.success(ctx.settings.successMessage + ':' + ctx.runJsSource.context.jsTemplate.templateId);
      `,
      version: 'v2',
      settings: {
    ...
    expect(message.success).toHaveBeenCalledWith('Approved:jtt_mark_approved');
  ...
  it('blocks JS Action settings values that violate schema constraints', async () => {
    ...
    ).toThrow('JS Template settings validation failed.');
    expect(
      () =>
