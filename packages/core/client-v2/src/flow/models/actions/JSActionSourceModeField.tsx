...
export interface JSActionSourceModeFieldProps {
  value?: string;
  onChange?: (value: string) => void;
  disabled?: boolean;
}

export const JSActionSourceModeField: React.FC<JSActionSourceModeFieldProps> = (props) => {
  const ctx = useFlowContext<{
    engine?: {
      flowSettings?: {
        components?: Record<string, React.ElementType<JSActionSourceModeFieldProps> | undefined>;
      };
    };
  }>();
  const FullSourceField = ctx?.engine?.flowSettings?.components?.[JS_ACTION_LIGHT_EXTENSION_FULL_SOURCE_FIELD];

  if (FullSourceField && FullSourceField !== JSActionSourceModeField) {
    return <FullSourceField {...props} />;
  }

  return <JSActionInlineSourceModeField {...props} />;
};

const JSActionInlineSourceModeField: React.FC<JSActionSourceModeFieldProps> = ({ value, onChange, disabled }) => {
