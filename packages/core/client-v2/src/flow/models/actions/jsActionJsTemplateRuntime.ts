...
export type JSActionSourceMode = RunJSSourceMode;
...
const jsActionSource = createJsTemplateSourcePlumbing<JSActionRuntimeModel>({
...
export function createJSActionSourceModeStep(): StepDefinition {
  return createJsTemplateSourceModeStep({
    kind: 'js-action',
    component: JS_ACTION_JS_TEMPLATE_FULL_SOURCE_FIELD,
...
export async function getJSActionRuntimeFlowSettingSteps(
  ...
  return createJsTemplateSettingSteps<JSActionRuntimeModel>({
    descriptor,
    settings: isRecord(params.settings) ? params.settings : {},
    component: JS_ACTION_JS_TEMPLATE_SETTINGS_STEP_FIELD,
...
async function getJsTemplateSettingsDescriptor(model: JSActionRuntimeModel, params: Record<string, unknown>) {
...
function normalizeRuntimeError(error: unknown): JSActionRuntimeError {
  return normalizeJsTemplateRuntimeError(error, {
    ...
    outdatedHint: 'Refresh the action settings and choose the current template.',
