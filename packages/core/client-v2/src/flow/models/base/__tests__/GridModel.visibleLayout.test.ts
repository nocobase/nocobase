...
describe('GridModel.getVisibleLayout (hidden items filtering)', () => {
  ...
  it('keeps all rows/cols when config mode enabled, even if items are hidden', async () => {
  ...
  it('filters out columns whose all items are hidden in runtime mode', async () => {
  ...
  it('removes hidden items inside a mixed cell and preserves column sizes', async () => {
    await engine.flowSettings.disable();
