...
import { Button, Empty, Flex, Input, Radio, Space, theme } from 'antd';
...
function IconField(props: IconPickerProps) {
  ...
  const filterIcons = debounce((search) => {
    const searchValue = search?.trim() ?? '';
    setFilteredIcons(
      searchValue.length
        ? availableIcons.filter((iconName) => iconName.split(' ').some((value) => value.includes(searchValue)))
        : availableIcons,
    );
  }, 250);
