...
import { Button, Checkbox, DatePicker, Dropdown, Form, Input, InputNumber, List, Radio, Select, theme } from 'antd';
...
export const FieldValidation = observer((props: FieldValidationProps) => {
  ...
  const normalizeArgsForDomainLike = useCallback((ruleName: string, args: Record<string, any> | undefined) => {
    ...
    delete next.tlds_allow;
    delete next.tlds_deny;
  ...
  }, [formilyField, isValueEmpty, rules, t, validationOptions]);
    ...
    const newRules = rules.map((rule) => {
      if (rule.key === ruleKey) {
        ...
        return {
          ...rule,
          args: nextArgs,
          paramsType: option?.paramsType ?? rule.paramsType,
        };
      }
      return rule;
    });
  ...
  const handleToggleExpand = (ruleKey: string) => {
    if (expandedKeys.includes(ruleKey)) {
            ...
            >
              {param.componentType === 'checkbox' ? (
                ...
                <div>
                  ...
                  {param.options?.map((item) => {
                    if (item.value === currentValue && item.componentType) {
                      return (
                        <div key={`${item.value}-component`} style={{ marginTop: token.marginXS }}>
                          {item.componentType === 'text' ? (
                            (() => {
                              if (param.key === 'tlds' && (item.value === 'allow' || item.value === 'deny')) {
                                const tlds = rule.args?.tlds;
                                const arrValue: string[] = Array.isArray(
                                  item.value === 'allow' ? tlds?.allow : tlds?.deny,
                                )
                                  ? item.value === 'allow'
                                    ? tlds.allow
                                    : tlds.deny
                ...
                <Select
                  ...
                  allowClear
                  options={param.options?.map((item) => ({ label: t(item.label), value: item.value }))}
                />
                    ...
                    >
                      {hasParams && (
                        <Button
                          ...
                          style={{ color: token.colorTextSecondary, width: 16, height: 16, minWidth: 16, padding: 0 }}
                        />
                      )}
                    </div>
                    <span>
