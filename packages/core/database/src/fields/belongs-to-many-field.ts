...
export class BelongsToManyField extends RelationField {
  ...
  bind() {
    ...
    const autoGenPrimaryKeyField = Through.findField((f) => f.options.primaryKey && f.options._autoGenerated);
    if (autoGenPrimaryKeyField && !Through.model.rawAttributes[autoGenPrimaryKeyField.name]) {
      Through.removeField(autoGenPrimaryKeyField.name);
    }
