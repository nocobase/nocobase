...
export class Database extends EventEmitter implements AsyncEmitter {
  ...
  initListener() {
    ...
    this.on('afterDefineCollection', async (collection: Collection) => {
      ...
      const attrs = collection.model.rawAttributes;
      ...
      if (keys.length) {
        keys.forEach((key) => {
          ...
          if (
            ...
            if (collection.getField(key).getDefaultValue) {
              const value = collection.getField(key)?.getDefaultValue();
              value && (collection.model.rawAttributes[key].defaultValue = value);
            }
          }
        });
