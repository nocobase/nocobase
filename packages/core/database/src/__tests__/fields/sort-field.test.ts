...
describe('string field', () => {
  ...
  it('should init sort value on data already exits', async () => {
    ...
    const field = Test.addField('sort', {
      type: 'sort',
    });
    await field.sync({});
  ...
  it('scopeKey', async () => {
    ...
    await db.sync();
    const t1 = await Test.model.create({
      status: 'publish',
    });
    const t2 = await Test.model.create({
      status: 'publish',
    });
    const t3 = await Test.model.create({
      status: 'draft',
    });
    const t4 = await Test.model.create({
      status: 'draft',
    });
