...
新版手机登录用插件`sms-auth`的形式提供，移除了原**系统设置**中的**开启短信登录**选项。插件默认不启用，需要到插件管理中启用，插件启用后，认证器管理中会出现新的登录类型`SMS`，增加一个类型为`SMS`的认证器，即可启用短信认证功能（前提是已配置好验证码Provider）。插件可以配置手机号不存在的用户是否自动注册，用户表中已有手机号的用户可以直接登录。
