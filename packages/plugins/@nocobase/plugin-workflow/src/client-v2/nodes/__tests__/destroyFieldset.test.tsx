...
import { DestroyFieldset } from '../components/destroy';
...
describe('DestroyFieldset', () => {
  it('renders the delete filter for the selected collection', async () => {
    renderWithForm(
      { id: 1, config: { collection: 'posts' } },
      { config: { collection: 'posts', params: { filter: { title: { $eq: 'old' } } } } },
    );
  ...
  it('resets filter when collection changes', async () => {
