...
// `admin.workflow.*` is a shared namespace so sibling pages (e.g. executions
// at `/admin/workflow/executions/:id`) can line up under the same prefix,
// mirroring v1's `admin.workflow.workflows.id` route.
...
// Execution detail page, a sibling of the canvas under the same `admin.workflow`
// namespace — mirrors v1's `admin.workflow.executions.id` route.
