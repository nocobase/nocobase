...
import { AggregateNode, ClculationNode, CollectionTriggerNode, CreateRecordNode, QueryRecordNode, apiCreateWorkflow, apiCreateWorkflowNode, apiDeleteWorkflow, apiGetWorkflow, apiGetWorkflowNode, apiGetWorkflowNodeExecutions, apiUpdateWorkflowNode, apiUpdateWorkflowTrigger, appendJsonCollectionName, generalWithNoRelationalFields } from '@nocobase/plugin-workflow-test/e2e';
import { expect, test } from '@nocobase/test/e2e';
import { dayjs } from '@nocobase/utils';

test('Collection event add data trigger, Math engine, get trigger node single line text variable', async ({ page, mockCollections, mockRecords }) => {
    //数据表后缀标识
...
test('Collection event add data trigger, Formula engine, get trigger node single line text variable', async ({ page, mockCollections, mockRecords }) => {
    //数据表后缀标识
    const triggerNodeAppendText = faker.string.alphanumeric(5);
...
test('Collection event add data triggers, Math engine, get predecessor Math engine arithmetic node data', async ({ page, mockCollections, mockRecords }) => {
    //数据表后缀标识
    const triggerNodeAppendText = faker.string.alphanumeric(5);
    ...
    const preCalculationNodeData = {"type":"calculation","upstreamId":null,"branchIndex":null,"title":preCalculationNodeTitle,"config":{"engine":"math.js","expression":"{{$context.data.orgname}}"}};
...
test('Collection event add data triggers, Formula engine, get predecessor Formula engine arithmetic node data', async ({ page, mockCollections, mockRecords }) => {
    //数据表后缀标识
    const triggerNodeAppendText = faker.string.alphanumeric(5);
    ...
    //配置前置计算节点
    const preCalculationNodeTitle = 'calculation' + dayjs().format('YYYYMMDDHHmmss.SSS').toString();
    const preCalculationNodeData = {"type":"calculation","upstreamId":null,"branchIndex":null,"title":preCalculationNodeTitle,"config":{"engine":"formula.js","expression":"{{$context.data.orgname}}"}};
    const preCalculationNode = await apiCreateWorkflowNode(workflowId, preCalculationNodeData);
    const preCalculationNodeObj = JSON.parse(JSON.stringify(preCalculationNode));
    const preCalculationNodeId = preCalculationNodeObj.id;
    const getPreCalculationNode = await apiGetWorkflowNode(preCalculationNodeId);
...
test('Collection events add data triggers, static types, Math engine, get single line of text data for front query data nodes', async ({ page, mockCollections, mockRecords }) => {
    //数据表后缀标识
    const triggerNodeAppendText = faker.string.alphanumeric(5);
...
test('Collection events add data triggers, static types, Formula engine, get single line of text data for front query data nodes', async ({ page, mockCollections, mockRecords }) => {
    //数据表后缀标识
    const triggerNodeAppendText = faker.string.alphanumeric(5);
...
test('Collection event add data trigger, Math engine, get front added data node single line text data', async ({ page, mockCollections, mockRecords }) => {
    //创建触发器节点数据表
    const triggerNodeAppendText = faker.string.alphanumeric(5);
    const triggerNodeCollectionDisplayName = `自动>组织[普通表]${triggerNodeAppendText}`;
    const triggerNodeCollectionName = `tt_amt_org${triggerNodeAppendText}`;
    ...
    await mockCollections(appendJsonCollectionName(JSON.parse(JSON.stringify(generalWithNoRelationalFields)), createRecordNodeAppendText).collections);
    //添加工作流
    const workFlowName = faker.string.alphanumeric(5) + triggerNodeAppendText;
    const workflowData = { "current": true, "options": { "deleteExecutionOnStatus": [] }, "title": workFlowName, "type": "collection", "enabled": true };
    const workflow = await apiCreateWorkflow(workflowData);
    ...
    const preCreateRecordNodeData = {"type":"create","upstreamId":null,"branchIndex":null,"title":preCreateRecordNodeTitle,"config":{"params":{"values":{"orgname":"{{$context.data.orgname}}"},"appends":[]},"collection":createRecordNodeCollectionName}}
...
test('Collection event add data trigger, Formula engine, get front added data node single line text data', async ({ page, mockCollections, mockRecords }) => {
    //创建触发器节点数据表
    const triggerNodeAppendText = faker.string.alphanumeric(5);
    const triggerNodeCollectionDisplayName = `自动>组织[普通表]${triggerNodeAppendText}`;
    const triggerNodeCollectionName = `tt_amt_org${triggerNodeAppendText}`;
...
test('Collection event adds data triggers, static types, Math engine, fetches data from front-end aggregation query nodes',async ({ page, mockCollections, mockRecords }) => {
    //创建触发器节点数据表
    const triggerNodeAppendText = faker.string.alphanumeric(5);
    const triggerNodeCollectionDisplayName = `自动>组织[普通表]${triggerNodeAppendText}`;
    const triggerNodeCollectionName = `tt_amt_org${triggerNodeAppendText}`;
    ...
    const preAggregateNodeData = { "type": "aggregate", "upstreamId": null, "branchIndex": null, "title": preAggregateNodeTitle, "config": { "aggregator": "count", "associated": false, "params": { "field": "id", "filter": { "$and": [] } }, "collection": triggerNodeCollectionName } };
...
test('Collection event adds data triggers, static types, Formula engine, fetches data from front-end aggregation query nodes', async ({ page, mockCollections, mockRecords }) => {
    //创建触发器节点数据表
    const triggerNodeAppendText = faker.string.alphanumeric(5);
    const triggerNodeCollectionDisplayName = `自动>组织[普通表]${triggerNodeAppendText}`;
    const triggerNodeCollectionName = `tt_amt_org${triggerNodeAppendText}`;
...
test.skip('Collection event add data trigger, Math engine, get front manual node add form single line text data', async ({ page, mockCollections, mockRecords }) => {
    // 创建触发器节点数据表
    const triggerNodeAppendText = faker.string.alphanumeric(5);
    const triggerNodeCollectionDisplayName = `自动>组织[普通表]${triggerNodeAppendText}`;
    const triggerNodeCollectionName = `tt_amt_org${triggerNodeAppendText}`;
    ...
    await mockCollections(appendJsonCollectionName(JSON.parse(JSON.stringify(generalWithNoRelationalFields)), manualNodeAppendText).collections);
    //添加工作流
    const workFlowName = faker.string.alphanumeric(5) + triggerNodeAppendText;
    const workflowData = { "current": true, "options": { "deleteExecutionOnStatus": [] }, "title": workFlowName, "type": "collection", "enabled": true };
    const workflow = await apiCreateWorkflow(workflowData);
    ...
    await page.getByRole('menuitemcheckbox', { name: '公司名称(单行文本)' }).click();
    await expect(page.getByLabel('textbox')).toHaveText(`Node result / ${preManualNodeNodeTitle} / ${preManualNodeNodeCustomFormTitle} / 公司名称(单行文本)`);
    await calculationNode.submitButton.click();
...
test.skip('Collection event add data trigger, Formula engine, get front manual node add form single line text data', async ({ page, mockCollections, mockRecords }) => {
    // 创建触发器节点数据表
    const triggerNodeAppendText = faker.string.alphanumeric(5);
    const triggerNodeCollectionDisplayName = `自动>组织[普通表]${triggerNodeAppendText}`;
    const triggerNodeCollectionName = `tt_amt_org${triggerNodeAppendText}`;
