...
} from './pageobject/e2eTemplateJson';
...
test.describe('query data node', () => {
  test('Collection event add data trigger, no filter no sort query common table 1 record', async ({
    ...
    await page.waitForLoadState('networkidle');
    await page.getByLabel(`Query record-${preQueryRecordNodeName}`).getByRole('button', { name: 'check' }).click();
    await page.waitForLoadState('networkidle');
  ...
  test('Collection event add data trigger, no filtering and no sorting, query common table multiple records, set page 1 20 per page', async ({
    ...
    await preQueryRecordNode.collectionDropDown.click();
    await page.getByText(triggerNodeCollectionDisplayName).click();
    await preQueryRecordNode.allowMultipleDataBoxesForResults.check();
    // 设置过滤条件
    // await page.getByText('Add condition', { exact: true }).click();
    // await page.getByTestId('filter-select-field').getByLabel('Search').click();
    // await page.getByText('ID', { exact: true }).click();
    ...
    // await page.getByRole('menuitemcheckbox', { name: 'Trigger data' }).click();
    // await page.getByRole('menuitemcheckbox', { name: 'ID', exact: true }).click();
    await expect(preQueryRecordNode.pageNumberEditBox).toHaveValue('1');
    await expect(preQueryRecordNode.pageSizeEditBox).toHaveValue('20');
    await preQueryRecordNode.submitButton.click();
  ...
  test('Collection event add data trigger, no filter ID ascending, query common table with multiple records, set page X to Y per page', async ({
    ...
    await preQueryRecordNode.allowMultipleDataBoxesForResults.check();
    // 设置排序条件
    await preQueryRecordNode.addSortFieldsButton.click();
    ...
    await preQueryRecordNode.pageNumberEditBox.fill(pageNumber.toString());
    await preQueryRecordNode.pageSizeEditBox.fill(pageSize.toString());
    await preQueryRecordNode.submitButton.click();
  ...
  test('Collection event add data trigger, no filtering and no sorting, query the ordinary table 1 record, set the X page Y per page, query the results of the empty, exit the process', async ({
    ...
    await preQueryRecordNode.pageNumberEditBox.fill(pageNumber.toString());
    await preQueryRecordNode.pageSizeEditBox.fill(pageSize.toString());
    await preQueryRecordNode.exitProcessOptionsBoxWithEmptyResult.check();
    await expect(preQueryRecordNode.exitProcessOptionsBoxWithEmptyResult).toBeChecked();
    await preQueryRecordNode.submitButton.click();
    ...
    await page.waitForLoadState('networkidle');
    await page
      .getByLabel(`Query record-${preQueryRecordNodeName}`)
  ...
  test('Collection event add data trigger, filter to meet all conditions (status_singleselect=1 and staffnum>180) unsorted, query normal table 1 record, set page X to Y per page', async ({
    page,
    mockCollections,
    mockRecords,
    mockPage,
    ...
    //用例标题
    const caseTitle =
      'Collection event add data trigger, filter to meet all conditions (status_singleselect=1 and staffnum>180) unsorted, query normal table 1 record, set page X to Y per page';
    ...
    // const triggerNodeCollectionRecordNumber = faker.number.int({ min: 0, max: 19 });
    const triggerNodeCollectionRecords = await mockRecords(triggerNodeCollectionName, 0, e2e_GeneralFormsTableData);
...
test.describe('update data node', () => {});
test.describe('delete data node', () => {});
test.describe('aggregate node', () => {});
