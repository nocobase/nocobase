...
test.describe('Add new', () => {
  test('add new Form event', async ({ page }) => {
    ...
    await page.getByTitle('Custom action event').click();
...
test.describe('Edit', () => {
  test('edit Form event name', async ({ page }) => {
    //添加工作流
    const triggerNodeAppendText = Math.random().toString(36).substring(2, 20);
    let workFlowName = Math.random().toString(36).substring(2, 20);
    const workflowData = {
      current: true,
      options: { deleteExecutionOnStatus: [] },
      title: workFlowName,
    ...
    workFlowName = Math.random().toString(36).substring(2, 20);
