  ...
  test('supported options', async ({ page, mockPage }) => {
    ...
    await expectSettingsMenu({
      ...
      showMenu: async () => {
        ...
        await page.getByLabel(`block-item-CollectionField-general-form-general.sort-sort`).hover();
        await page
          .getByLabel(`designer-schema-settings-CollectionField-FormItem.Designer-general-general.sort`)
          .hover();
      },
      supportedOptions: [
        'Edit field title',
  ...
  test('set default value', async ({ page, mockPage }) => {
    await testDefaultValue({
      ...
      expectConstantValue: async () => {
        await expect(
          page.getByLabel('block-item-CollectionField-general-form-general.sort-sort').getByRole('spinbutton'),
        ).toHaveValue('112233');
      },
  ...
  test('pattern', async ({ page, mockPage }) => {
    await testPattern({
      ...
      expectEditable: async () => {
        // 默认情况下可以编辑
        await page
          .getByLabel('block-item-CollectionField-general-form-general.sort-sort')
      ...
      expectEasyReading: async () => {
        ...
        await expect(page.getByLabel('block-item-CollectionField-general-form-general.sort-sort')).toHaveText(
          'sort:112233',
        );
      },
    });
  ...
  test('pattern', async ({ page, mockPage, mockRecord }) => {
    ...
    await testPattern({
      ...
      expectEditable: async () => {
        ...
        await expect(
          page.getByLabel('block-item-CollectionField-general-form-general.sort-sort').getByRole('spinbutton'),
        ).toHaveValue(String(record.sort));
      },
      expectReadonly: async () => {
        // 只读模式下，输入框会被禁用
        await expect(
...
test.describe('table column & table', () => {
  test('supported options', async ({ page, mockPage, mockRecord }) => {
    ...
    await expectSettingsMenu({
      page,
      showMenu: async () => {
        await createColumnItem(page, 'sort');
        await showSettingsMenu(page, 'sort');
      },
