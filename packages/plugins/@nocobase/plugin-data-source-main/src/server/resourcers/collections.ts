...
import { applyCollectionDefinition, applyFieldDefinition } from '../modeling/service';

export async function syncCollectionFields(ctx, filterByTk: string, values: { fields?: any[] }) {
...
export default {
  ...
  async ['collections:apply'](ctx, next) {
    ...
    ctx.body = await applyCollectionDefinition(ctx, rawValues, syncCollectionFields);
    await next();
  },
  ...
  async ['fields:apply'](ctx, next) {
    const rawValues = ctx.action.params.values || ctx.request.body || {};
    ctx.body = {
      data: await applyFieldDefinition(ctx, rawValues),
    };
    await next();
  },
};
