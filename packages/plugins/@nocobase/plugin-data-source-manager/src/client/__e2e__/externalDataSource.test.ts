...
const PGDataSourceSetting = {
  key: PGDataSource,
  displayName: PGDataSource,
  type: 'postgres',
  options: {
    database: `${process.env.DB_DATABASE}_external_test`,
    host: 'localhost',
...
test.describe('external data source', () => {
  let dataSource;
  ...
  test.describe('add external data source', () => {
    test('pg external data source', async ({ page }) => {
      ...
      if (!(await page.getByLabel(`${PGDataSource}-Configure`).isVisible())) {
  ...
  test.describe('add association field', () => {
    ...
    test('o2m targetKey & forignkey', async ({ page }) => {
      ...
      await page
        .getByLabel('block-item-Input-fields-Field display name')
        .getByRole('textbox')
        .fill(o2mAssociationField);
