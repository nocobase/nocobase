 ...
 */
function toggleFullscreen(element: HTMLElement, isFullscreen: boolean): void {
  // Ensure element has an ID
  if (!element.id) {
    element.id = 'nb-fullscreen-element-' + Date.now();
  }
  ...
  return (
    <ActionContextProvider value={{ ...props }}>
      <Action
        ...
        {...props}
        onClick={(e) => {
          // Find the first parent with class 'nb-block-item'
          const blockParent = findParentByClassName(e.target as HTMLElement, 'nb-block-item');
          if (!blockParent) return; // Exit if no matching parent found
          const targetElement = blockParent.querySelector('.ant-card') as HTMLElement;
          ...
          toggleFullscreen(targetElement, targetState);
        }}
      />
