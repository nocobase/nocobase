...
import { flowSchemaManifestContribution as coreFlowSchemaManifestContribution } from '../../../../plugin-flow-engine/src/server/flow-schema-manifests';
...
describe('custom request flow schema manifests', () => {
  it('should expose a server manifest for the custom request event flow action', () => {
    const clientActionName = getClientCustomRequestActionName();
    const serverActionNames = (flowSchemaManifestContribution.actions || []).map((manifest) => manifest.name);
  ...
  it('should validate flow models that use the custom request event flow action', () => {
    ...
    service.registerActionManifests(coreFlowSchemaManifestContribution.actions || []);
    service.registerModelManifests(coreFlowSchemaManifestContribution.models || []);
