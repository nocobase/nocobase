...
import { useCancelAction } from '../hooks';
import { NAMESPACE } from '../../locale';

const getSchema = (schema: IField, record: any, compile, t) => {
  if (!schema) {
    return;
  }
  ...
  return {
    ...
    properties: {
      [uid()]: {
        ...
        title: t('Sync from remote table', { ns: NAMESPACE }),
        properties: {
          schema: {
            type: 'string',
            'x-hidden': true,
    ...
    <RecordProvider record={record}>
      <ActionContextProvider value={{ visible, setVisible, drawerProps: { width: 900 } }}>
        {children || (
          <Button
            onClick={(e) => {
              const schema = getSchema({}, record, compile, t);
              if (schema) {
                setSchema(schema);
                setVisible(true);
              }
          ...
          >
            {t('Sync from remote table', { ns: NAMESPACE })}
          </Button>
        )}
        <SchemaComponent
          schema={schema}
