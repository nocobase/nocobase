...
After a job is accepted, logging out, session expiry or revocation, and password rotation do not cancel it. Before each write boundary, the worker still requires the actor to exist, verifies that the request-selected role or union-role set remains active and compatible with the current role mode, and rechecks the current `create` permission. Git creation also rechecks `manageSyncSource` and `pullFromSyncSource` with that persisted request-selected role context; roles granted after acceptance are never borrowed.
...
`jsTemplateCreateJobs:list`, `jsTemplateCreateJobs:get`, and `jsTemplateCreateJobs:dismiss` are the public job API. Acceptance only confirms persistence. The list returns every active (`pending`, `running`, or `finalize-pending`) job for the current application and user, plus retained terminal history in stable newest-first order. The UI renders only active jobs. Successful jobs retain their safe `resultProjectId`; failed jobs retain safe error and request evidence but clear their source payload. Failed jobs are notified once and soft-dismissed automatically, while successful terminal history can also be soft-dismissed. Active jobs cannot be dismissed.
...
The UI closes the Starter, ZIP, or Git creation dialog after acceptance and merges each active job into the project table `dataSource` as a special creation row. The table contains created projects plus only `pending`, `running`, and `finalize-pending` creation rows. It polls every 2.5 seconds while an active job exists and retries an initial list error, but stops polling when the list is empty or terminal-only. Success refreshes `resultProjectId` and replaces the creation row with the project row. Failure produces one safe notification and is then soft-dismissed automatically, so there is no persistent Retry or Remove row. Another tab observes the dismissal after its own refresh or reload; permanent terminal polling is not a cross-tab synchronization mechanism. The server retains redacted audit and idempotency evidence while clearing retryable source payload after a terminal failure. Public summaries never expose source payloads, credentials, actor/request identity, claim tokens, owners, leases, or heartbeats.
