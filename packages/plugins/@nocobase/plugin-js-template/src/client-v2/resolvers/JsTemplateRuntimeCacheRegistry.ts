...
type JsTemplateRuntimeCacheInvalidator = {
  invalidateProject(projectId: string): void;
  clear(): void;
};

const runtimeCaches = new WeakMap<object, JsTemplateRuntimeCacheInvalidator>();
const cacheGenerations = new WeakMap<object, JsTemplateCacheGeneration>();
const selectableCatalogs = new WeakMap<object, SelectableCatalogState>();
const identityRegistrations = new WeakMap<object, JsTemplateRuntimeIdentityRegistration[]>();
const fallbackIdentityRegistrations = new WeakMap<object, JsTemplateRuntimeIdentityRegistration>();

type IdentityScalar = string | number | boolean | null | undefined;

export type JsTemplateRuntimeIdentitySnapshot = {
...
type JsTemplateRuntimeIdentityProvider = () => JsTemplateRuntimeIdentitySnapshot;
...
type JsTemplateRuntimeIdentityRegistration = {
  provider: JsTemplateRuntimeIdentityProvider;
  initialized: boolean;
  sessionEpoch: number;
  permissionGeneration: number;
  previous?: JsTemplateRuntimeIdentitySnapshot;
};

export type JsTemplateCacheGenerationSnapshot = {
  global: number;
  project: number;
};
...
export const JS_TEMPLATE_SELECTABLE_CATALOG_TTL_MS = 30_000;

export class JsTemplateCacheGeneration {
  private globalGeneration = 0;
  private catalogGeneration = 0;
  private readonly projectGenerations = new Map<string, number>();

  get(projectId: string): JsTemplateCacheGenerationSnapshot {
    return {
      global: this.globalGeneration,
      project: this.projectGenerations.get(projectId) || 0,
    };
  }

  isCurrent(projectId: string, snapshot: JsTemplateCacheGenerationSnapshot): boolean {
    const current = this.get(projectId);
    return current.global === snapshot.global && current.project === snapshot.project;
  }
...
export function getJsTemplateCacheGeneration(api: object): JsTemplateCacheGeneration {
  let generation = cacheGenerations.get(api);
  if (!generation) {
    generation = new JsTemplateCacheGeneration();
    cacheGenerations.set(api, generation);
  }
  return generation;
}

export function getOrCreateJsTemplateRuntimeCache<TCache extends JsTemplateRuntimeCacheInvalidator>(
  api: object,
  create: (generation: JsTemplateCacheGeneration) => TCache,
): TCache {
  let cache = runtimeCaches.get(api) as TCache | undefined;
  if (!cache) {
    cache = create(getJsTemplateCacheGeneration(api));
    runtimeCaches.set(api, cache);
  }
  return cache;
}

export function getJsTemplateRuntimeIdentity(api: object): string {
...
export function registerJsTemplateRuntimeIdentity(
  api: object,
  provider: JsTemplateRuntimeIdentityProvider,
  eventTarget?: EventTargetLike,
): () => void {
  const registration: JsTemplateRuntimeIdentityRegistration = {
    provider,
    initialized: false,
    sessionEpoch: 0,
...
export function registerJsTemplateRuntimeAuthSession(api: object, app?: object): () => void {
  return registerJsTemplateRuntimeIdentity(api, () => readRuntimeIdentity(api, app), getEventTarget(app));
}

export function getOrLoadJsTemplateSelectableCatalog<T>(api: object, load: () => Promise<T[]>): Promise<readonly T[]> {
  const generation = getJsTemplateCacheGeneration(api).getCatalog();
  ...
  if (state.cached?.key === key && state.cached.expiresAt > Date.now()) {
    return Promise.resolve(state.cached.templates as readonly T[]);
  }
  const existing = state.inFlight.get(key);
  if (existing) {
    return existing as Promise<readonly T[]>;
  }
  const request = load().then((templates) => {
    const currentGeneration = getJsTemplateCacheGeneration(api).getCatalog();
    const currentIdentity = getJsTemplateRuntimeIdentity(api);
    if (currentGeneration !== generation || currentIdentity !== identity) {
      return getOrLoadJsTemplateSelectableCatalog(api, load);
    }
    state.cached = {
      templates,
      expiresAt: Date.now() + JS_TEMPLATE_SELECTABLE_CATALOG_TTL_MS,
      key,
    };
    return templates;
  });
...
export function invalidateJsTemplateRuntimeCache(api: object, projectId?: string): void {
  const generation = getJsTemplateCacheGeneration(api);
...
function getFallbackIdentityRegistration(api: object): JsTemplateRuntimeIdentityRegistration {
...
function readRuntimeIdentity(api: object, app?: object): JsTemplateRuntimeIdentitySnapshot {
