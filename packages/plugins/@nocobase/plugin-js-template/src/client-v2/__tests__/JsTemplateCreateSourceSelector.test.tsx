...
describe('JsTemplateCreateSourceSelector', () => {
  it('defaults to starter and emits mutually exclusive starter, ZIP, and Git sources', async () => {
    const user = userEvent.setup();
    const onChange = vi.fn();
    renderSelector({ onChange, readZipFile: async () => 'zip-source-base64' });

    await waitFor(() => expect(onChange).toHaveBeenCalledWith({ mode: 'starter' }));
  ...
  it('clears ZIP file and error state when leaving ZIP mode', async () => {
    ...
    expect(onChange).toHaveBeenLastCalledWith({ mode: 'starter' });
  ...
  it('ignores a ZIP read that finishes after leaving ZIP mode', async () => {
    ...
    await waitFor(() => expect(onChange).toHaveBeenLastCalledWith({ mode: 'starter' }));
