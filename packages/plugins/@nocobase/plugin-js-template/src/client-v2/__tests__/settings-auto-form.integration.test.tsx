...
describe('SettingsAutoForm', () => {
  ...
  it('applies shared nested and array-item defaults without mutating the submitted settings', () => {
    const schema = {
      type: 'object',
      properties: {
        title: {
    ...
    expect(normalizeSettingsForSchema(schema, value)).toEqual({
      errors: [],
      value: {
        title: 'Untitled',
        options: {
  ...
  it('maps every shared validation issue to the existing UI messages and emitted order', async () => {
    const onChange = vi.fn();
    const schema = {
      type: 'object',
      properties: {
    ...
    const errors = [
      ...
      { label: 'contact', message: 'Must be one of the allowed values', path: 'contact' },
      { label: 'contact', message: 'Too short', path: 'contact' },
      { label: 'contact', message: 'Must match the required format', path: 'contact' },
      {
        code: 'settings_unknown_property',
        label: 'rows[0].extra',
        message: 'Unknown property',
      ...
      { label: 'rows[0].name', message: 'Too short', path: 'rows[0].name' },
    ];

    expect(normalizeSettingsForSchema(schema, value)).toEqual({ errors, value });
    const { container } = render(<SettingsAutoForm schema={schema} value={value} onChange={onChange} />);

    await waitFor(() => expect(onChange).toHaveBeenCalledWith(value, { errors, value }));
    const description = container.querySelector('.ant-alert-description');
    expect(description).toHaveTextContent('contact: Must be one of the allowed values');
    expect(description).toHaveTextContent('contact: Too short');
    expect(description).toHaveTextContent('contact: Must match the required format');
    expect(description).toHaveTextContent('rows[0].extra: Unknown property');
  });

  it('keeps prototype-sensitive unknown JSON properties as safe own data while rejecting them', () => {
    const value = JSON.parse(
      '{"enabled":true,"__proto__":{"polluted":true},"constructor":{"name":"own"},"prototype":{"flag":true}}',
    ) as Record<string, unknown>;
    const result = normalizeSettingsForSchema(
      {
        type: 'object',
        properties: {
          enabled: { type: 'boolean' },
    ...
    expect(Object.prototype.hasOwnProperty.call(result.value, 'prototype')).toBe(true);
    expect(Reflect.get(result.value, '__proto__')).toEqual({ polluted: true });
    expect(Reflect.get(result.value, 'constructor')).toEqual({ name: 'own' });
    expect(Reflect.get(result.value, 'prototype')).toEqual({ flag: true });
    expect(Reflect.get(Object.prototype, 'polluted')).toBeUndefined();
  });

  it('normalizes declared prototype-sensitive properties without changing any object prototype', () => {
    const schema = JSON.parse(`{
      "type":"object",
      "properties":{
        "__proto__":{"type":"object","properties":{"polluted":{"type":"boolean"}}},
        "constructor":{"type":"string"},
        "prototype":{"type":"array","items":{"type":"object","properties":{"value":{"type":"integer"}}}}
      }
    }`) as Record<string, unknown>;
    const value = JSON.parse('{"__proto__":{"polluted":true},"constructor":"own","prototype":[{"value":1}]}') as Record<
      string,
      unknown
    >;
    const result = normalizeSettingsForSchema(schema, value);

    expect(result.errors).toEqual([]);
    expect(result.value).toEqual(value);
    expect(Object.getPrototypeOf(result.value)).toBe(Object.prototype);
    expect(Object.getPrototypeOf(Reflect.get(result.value, '__proto__'))).toBe(Object.prototype);
    expect(Object.getPrototypeOf(result.value.prototype)).toBe(Array.prototype);
    expect(Reflect.get(Object.prototype, 'polluted')).toBeUndefined();
  });

  it('keeps prototype-sensitive unknown-property issues through SettingsSingleField normalization', async () => {
    const onChange = vi.fn();
    const value = JSON.parse(
      '{"known":"ok","__proto__":{"polluted":true},"constructor":"own","prototype":true}',
    ) as Record<string, unknown>;
    render(
      <SettingsSingleField
        fieldName="options"
    ...
    await waitFor(() => {
      expect(onChange).toHaveBeenCalledWith(
        expect.objectContaining({ known: 'ok' }),
        expect.objectContaining({
          errors: [
            {
              code: 'settings_unknown_property',
              label: 'options.__proto__',
              message: 'Unknown property',
              path: 'options.__proto__',
            },
            {
              code: 'settings_unknown_property',
              label: 'options.constructor',
              message: 'Unknown property',
              path: 'options.constructor',
            },
            {
              code: 'settings_unknown_property',
              label: 'options.prototype',
              message: 'Unknown property',
              path: 'options.prototype',
            },
          ],
        }),
      );
    });
    const submitted = onChange.mock.calls.at(-1)?.[0] as Record<string, unknown>;
    expect(Object.getPrototypeOf(submitted)).toBe(Object.prototype);
    expect(Object.prototype.hasOwnProperty.call(submitted, '__proto__')).toBe(true);
    expect(Reflect.get(Object.prototype, 'polluted')).toBeUndefined();
  });

  it('updates a declared __proto__ field as an own property in SettingsSingleField', async () => {
    const onChange = vi.fn();
    const fieldSchema = JSON.parse(
      '{"type":"object","properties":{"__proto__":{"type":"string","title":"Prototype value"}}}',
    ) as Record<string, unknown>;
    const value = JSON.parse('{"__proto__":"before"}') as Record<string, unknown>;
    render(<SettingsSingleField fieldName="options" fieldSchema={fieldSchema} value={value} onChange={onChange} />);

    await waitFor(() => expect(onChange).toHaveBeenCalled());
    onChange.mockClear();
    fireEvent.change(screen.getByRole('textbox', { name: 'Prototype value' }), { target: { value: 'after' } });

    await waitFor(() => expect(onChange).toHaveBeenCalled());
    const submitted = onChange.mock.calls.at(-1)?.[0] as Record<string, unknown>;
    expect(Object.getPrototypeOf(submitted)).toBe(Object.prototype);
    expect(Object.prototype.hasOwnProperty.call(submitted, '__proto__')).toBe(true);
    expect(Reflect.get(submitted, '__proto__')).toBe('after');
  });
