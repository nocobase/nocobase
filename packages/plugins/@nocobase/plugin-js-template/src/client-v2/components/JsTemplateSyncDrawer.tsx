...
import type { JsTemplateProject, JsTemplateSyncPlan, JsTemplateSyncSourceSummary } from '../../shared/types';
import {
...
export interface JsTemplateSyncDrawerProps {
  open: boolean;
...
export function JsTemplateSyncDrawer(props: JsTemplateSyncDrawerProps) {
  const { open, project, onClose, onProjectUpdated, onSyncSourceChanged, configurationPanel, onRequestConfigure } =
    props;
  const t = useT();
  ...
  const currentPlanRef = useRef<JsTemplateSyncPlan>();
  ...
  const setSafeError = useCallback((error: unknown) => {
    const code = error instanceof JsTemplateSyncHookError ? error.code : undefined;
    const reasonCode =
      error instanceof JsTemplateSyncHookError && typeof error.details?.reasonCode === 'string'
        ? error.details.reasonCode
        : undefined;
    setErrorKey(getJsTemplateSyncErrorTranslationKey(code, reasonCode) || 'Unable to complete sync operation');
  }, []);

  const isCurrentRequest = useCallback(
    (identity: RequestIdentity) =>
      currentOpenRef.current &&
      currentProjectIdRef.current === identity.projectId &&
  ...
  const loadSyncData = useCallback(
    async (identity: RequestIdentity) => {
      ...
      try {
        await sync.get({ projectId: identity.projectId });
        if (!isCurrentRequest(identity)) {
          return;
        }
        const planResult = await sync.plan({ projectId: identity.projectId });
        if (!isCurrentRequest(identity)) {
          return;
        }
  ...
  const startOperation = useCallback((name: SyncOperation): ActiveOperation | undefined => {
    ...
    const activeOperation = { id: Symbol(name), name, projectId: currentProjectIdRef.current };
  ...
  useEffect(() => {
    ...
    const identity = { projectId: currentProjectIdRef.current, version: requestVersionRef.current };
    if (!open) {
      setLoadState('idle');
      setSyncData(undefined);
      setErrorKey(undefined);
  ...
  }, [loadSyncData, open, project.id]);

  useEffect(() => {
    setConfirmation((current) => {
      if (
        !current ||
        (open && current.projectId === project.id && current.planFingerprint === syncData?.plan.fingerprint)
      ) {
        return current;
      }
      return undefined;
    });
  }, [open, project.id, syncData?.plan.fingerprint]);
  ...
  const runTestConnection = useCallback(async () => {
    ...
    setErrorKey(undefined);
    try {
      await sync.testConnection({ projectId: identity.projectId });
      if (!isCurrentRequest(identity)) {
        return;
      }
  ...
  const runDisconnect = useCallback(async () => {
    ...
    setErrorKey(undefined);
    try {
      await sync.disconnect({ projectId: identity.projectId });
      if (!isCurrentRequest(identity)) {
        return;
      }
  ...
  const confirmPush = useCallback(() => {
    ...
    setConfirmation({ projectId: currentProjectIdRef.current, planFingerprint: plan.fingerprint });
  }, []);
  ...
  const confirmPushOperation = useCallback(async () => {
    ...
    if (
      !currentConfirmation ||
      !plan?.canPush ||
      currentConfirmation.projectId !== currentProjectIdRef.current ||
  ...
  const currentProjectOperation = activeOperationRef.current?.projectId === project.id ? operation : undefined;
  ...
  return (
    <>
            ...
            {loadState === 'ready' && source && plan ? (
              <>
                ...
                <Space wrap>
                  ...
                  >
                    <Button danger disabled={actionsDisabled} loading={currentProjectOperation === 'disconnect'}>
                      {t('Disconnect')}
                    </Button>
                  </Popconfirm>
                </Space>
      ...
      <Modal
        ...
        cancelText={t('Cancel')}
        confirmLoading={currentProjectOperation === 'push'}
...
function getStatusContent(plan: JsTemplateSyncPlan, t: ReturnType<typeof useT>): StatusContent {
  ...
  if (plan.state === 'error') {
    const errorKey = getJsTemplateSyncErrorTranslationKey(plan.reasonCode);
    return {
...
export default JsTemplateSyncDrawer;
