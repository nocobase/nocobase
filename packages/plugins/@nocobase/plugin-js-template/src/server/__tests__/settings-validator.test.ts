...
describe('plugin-js-template settings condition validator', () => {
  ...
  it('rejects oversized component props', () => {
    const result = validateSettings({
      type: 'object',
      properties: {
        title: {
    ...
    expect(result.accepted).toBe(false);
    expect(result.diagnostics).toContainEqual(expect.objectContaining({ code: 'settings_field_invalid' }));
  });
...
function validateDescriptorSettings(descriptorSettings: Record<string, unknown>) {
  return new JsTemplateValidator().validateWorkspace({
    files: [
