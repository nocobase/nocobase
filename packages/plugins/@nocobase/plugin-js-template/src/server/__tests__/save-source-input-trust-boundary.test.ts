...
import { createJsTemplateFilesResource } from '../resources/jsTemplateFiles';
import type { JsTemplateFileService } from '../services/JsTemplateFileService';
...
describe('Save source input trust boundary', () => {
  it('drops client compile keys, affected hints, bundles, metafiles, and source maps before canonical Save', async () => {
    ...
    const resource = createJsTemplateFilesResource(
      {} as JsTemplateFileService,
      { saveSource } as unknown as JsTemplateCompileService,
    );
