...
const PROJECT_ROOT_FILE_NAMES = new Set(['README.md', 'js-template.json', 'tsconfig.json']);
...
export async function parseJsTemplateSourceArchive(
  ...
  const sourceEntries = entries.filter((entry) => !entry.dir && !isIgnoredMetadataPath(getOriginalTemplateName(entry)));

  if (!sourceEntries.length) {
    throwArchiveValidation('zip_empty', 'ZIP archive does not contain source files');
  }

  const rawPaths = sourceEntries.map((entry) => normalizeArchiveEntryPath(getOriginalTemplateName(entry)));
  ...
  const files: JsTemplateTreeEntryInput[] = [];
...
function decodeArchiveBase64(zipBase64: string, validator: JsTemplateValidator): Buffer {
...
function validateArchiveEntry(entry: JSZipObject): void {
  const originalName = getOriginalTemplateName(entry);
...
function validateArchiveEntriesBeforeExtraction(
  ...
  if (entries.length > limits.maxProjectFiles) {
    throwArchiveValidation('project_file_count_exceeded', 'ZIP archive contains too many source files');
  }
  ...
  for (let index = 0; index < entries.length; index += 1) {
    ...
    if (sourceBytes > limits.maxProjectBytes) {
      throwArchiveValidation('project_budget_limit_exceeded', 'ZIP source budget is exceeded');
    }
  }
...
function getZipEntrySize(entry: JSZipObject, key: keyof ZipEntrySizeData): number {
  ...
  if (typeof value !== 'number' || !Number.isSafeInteger(value) || value < 0) {
    throwArchiveValidation(
      'zip_invalid',
      'ZIP archive contains invalid entry metadata',
      getOriginalTemplateName(entry),
    );
  }
...
async function extractEntry(entry: JSZipObject): Promise<Buffer> {
  try {
    ...
    throwArchiveValidation('zip_invalid', 'Unable to extract ZIP source file', getOriginalTemplateName(entry));
  }
}

function getOriginalTemplateName(entry: JSZipObject): string {
...
function assertZipBudget(compressedBytes: number, uncompressedBytes: number, validator: JsTemplateValidator): void {
...
function throwArchiveValidation(code: string, message: string, path?: string): never {
  ...
  throw new JsTemplateError('JS_TEMPLATE_VALIDATION_FAILED', message, {
    status: 422,
