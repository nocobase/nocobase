...
export const JS_TEMPLATE_COMPILE_POOL_HARD_LIMITS = Object.freeze({
  queuedJobs: 1024,
...
export interface JsTemplateCompileWorkerHandle {
  readonly threadId: number;
  on(event: 'message', listener: (message: JsTemplateCompileWorkerResponse) => void): this;
  on(event: 'error', listener: (error: Error) => void): this;
  on(event: 'exit', listener: (code: number) => void): this;
  off(event: 'message', listener: (message: JsTemplateCompileWorkerResponse) => void): this;
  off(event: 'error', listener: (error: Error) => void): this;
  off(event: 'exit', listener: (code: number) => void): this;
  postMessage(value: JsTemplateCompileWorkerRequest): void;
  gracefulShutdown?(timeoutMs: number): Promise<void>;
  terminate(): Promise<number>;
}

export type JsTemplateCompileWorkerFactory = (workerId: number) => JsTemplateCompileWorkerHandle;

export interface JsTemplateCompileWorkerPoolOptions {
  ...
  workerFactory?: JsTemplateCompileWorkerFactory;
}

export interface JsTemplateCompileWorkerPoolMetrics {
  workerCount: number;
  active: number;
  maxActive: number;
  queueDepth: number;
...
export type JsTemplateCompilePoolErrorCode =
  | 'JS_TEMPLATE_COMPILE_POOL_CLOSED'
  | 'JS_TEMPLATE_COMPILE_JOB_TOO_LARGE'
  | 'JS_TEMPLATE_COMPILE_QUEUE_CAPACITY_EXCEEDED'
  | 'JS_TEMPLATE_COMPILE_INFLIGHT_BYTES_EXCEEDED'
  | 'JS_TEMPLATE_COMPILE_CAPACITY_WAITERS_EXCEEDED';

export class JsTemplateCompilePoolError extends Error {
  constructor(
    public readonly code: JsTemplateCompilePoolErrorCode,
    message: string,
  ) {
    super(message);
    this.name = 'JsTemplateCompilePoolError';
  }
}
...
interface CompileTask {
  ...
  startedAt?: number;
  attempt: number;
  resolve: (result: JsTemplateCompileResult) => void;
  reject: (error: Error) => void;
}

interface WorkerListeners {
  message: (message: JsTemplateCompileWorkerResponse) => void;
...
interface WorkerSlot {
  id: number;
  handle: JsTemplateCompileWorkerHandle;
...
export class JsTemplateCompileWorkerPool {
  ...
  private readonly failedWorkers = new WeakSet<JsTemplateCompileWorkerHandle>();
  ...
  constructor(options: JsTemplateCompileWorkerPoolOptions = {}) {
    this.options = normalizeOptions(options);
  }

  submit(job: JsTemplateCompileJob): Promise<JsTemplateCompileResult> {
    if (this.shuttingDown || this.closed) {
      return this.rejectSubmission(
        new JsTemplateCompilePoolError(
          'JS_TEMPLATE_COMPILE_POOL_CLOSED',
          'JS Template compile worker pool is shutting down',
        ),
      );
    }
    let prepared: PreparedCompileJob;
    try {
      prepared = prepareCompileJob(job);
    } catch (error) {
  ...
  private submitPrepared(prepared: PreparedCompileJob): Promise<JsTemplateCompileResult> {
    if (this.shuttingDown || this.closed) {
      return this.rejectSubmission(
        new JsTemplateCompilePoolError(
          'JS_TEMPLATE_COMPILE_POOL_CLOSED',
    ...
    try {
      this.ensureWorkerSlot();
    } catch (error) {
      return Promise.reject(error);
    }

    return new Promise<JsTemplateCompileResult>((resolve, reject) => {
  ...
  async submitWithBackpressure(job: JsTemplateCompileJob, signal?: AbortSignal): Promise<JsTemplateCompileResult> {
    const prepared = prepareCompileJob(job);
    let retry: boolean;
    do {
      retry = false;
      await this.waitForCapacity(prepared.byteSize, signal);
      try {
        return await this.submitPrepared(prepared);
      } catch (error) {
    ...
    throw new Error('Compile backpressure retry ended unexpectedly');
  }
  ...
  async waitForCapacity(byteSize: number, signal?: AbortSignal): Promise<void> {
    if (!Number.isSafeInteger(byteSize) || byteSize <= 0 || byteSize > this.options.maxJobBytes) {
      throw new JsTemplateCompilePoolError(
        'JS_TEMPLATE_COMPILE_JOB_TOO_LARGE',
  ...
  getMetrics(): JsTemplateCompileWorkerPoolMetrics {
    return {
  ...
  private createWorkerSlot(workerId: number): WorkerSlot {
    ...
    const slot = { id: workerId, handle, ready: false } as WorkerSlot;
    const listeners: WorkerListeners = {
      message: (message) => this.handleWorkerMessage(slot, handle, message),
      error: (error) => this.scheduleWorkerFailure(slot, handle, error, 'JS_TEMPLATE_COMPILE_WORKER_CRASH'),
      exit: (code) => {
        this.scheduleWorkerFailure(
          slot,
          handle,
          new Error(`Compile worker exited with code ${code}`),
          'JS_TEMPLATE_COMPILE_WORKER_CRASH',
        );
      },
    };
  ...
  private dispatch(): void {
    for (const slot of this.slots) {
      ...
      const request: JsTemplateCompileWorkerRequest = {
        type: 'compile',
        workerId: slot.id,
      ...
      try {
        ...
        slot.timeout = setTimeout(() => {
          ...
          this.scheduleWorkerFailure(
            ...
            'JS_TEMPLATE_COMPILE_JOB_TIMEOUT',
          );
  ...
  private handleWorkerMessage(
    slot: WorkerSlot,
    handle: JsTemplateCompileWorkerHandle,
    message: JsTemplateCompileWorkerResponse,
  ): void {
    ...
    try {
      aggregateJsTemplateCompileResults([task.job], [message.result]);
    } catch (error) {
      this.scheduleWorkerFailure(
        slot,
        handle,
        error instanceof Error ? error : new Error(String(error)),
        'JS_TEMPLATE_COMPILE_WORKER_PROTOCOL_ERROR',
      );
      return;
    }
    this.clearSlotTask(slot);
    const result: JsTemplateCompileResult = {
      ...message.result,
      observation: {
  ...
  private async handleWorkerFailure(
    ...
    if (!slot.ready) {
      this.rejectWorkerInitialization(slot, error);
    ...
    if (!this.shuttingDown && slotIndex >= 0) {
      ...
      try {
        ...
        this.slots.splice(slotIndex, 1);
        this.rejectQueuedTasks(failure instanceof Error ? failure : new Error(String(failure)));
      }
    }
    this.dispatch();
    this.notifyCapacityWaiters();
  ...
  private completeTask(task: CompileTask, result: JsTemplateCompileResult): void {
    this.inflightBytes = Math.max(0, this.inflightBytes - task.byteSize);
    this.completed += 1;
    task.resolve(result);
  }

  private rejectTask(task: CompileTask, error: Error): void {
    this.inflightBytes = Math.max(0, this.inflightBytes - task.byteSize);
    task.reject(error);
  }

  private rejectWorkerInitialization(slot: WorkerSlot, error: Error): void {
    const activeTask = slot.task;
    this.clearSlotTask(slot);
    if (activeTask) {
      this.rejectTask(activeTask, error);
    }
    this.rejectQueuedTasks(error);
  }
    ...
    for (const task of queued) {
      this.completeTask(
        ...
        createJsTemplateCompileInfrastructureFailure({
          ...
          runDurationMs: 0,
          failureCode: 'JS_TEMPLATE_COMPILE_POOL_SHUTDOWN',
    ...
    for (const task of this.queue.splice(0)) {
      this.completeTask(
        ...
        createJsTemplateCompileInfrastructureFailure({
          ...
          runDurationMs: 0,
          failureCode: 'JS_TEMPLATE_COMPILE_POOL_FAILED',
          message,
        }),
      );
    }
  ...
  private rejectSubmission(error: JsTemplateCompilePoolError): Promise<JsTemplateCompileResult> {
    this.rejected += 1;
    return Promise.reject(error);
  }
...
function prepareCompileJob(job: JsTemplateCompileJob): PreparedCompileJob {
  assertJsTemplateCompileJob(job);
  const payload = serialize(job);
  return Object.freeze({
    job: deserialize(payload) as JsTemplateCompileJob,
...
function isRetryableAdmissionError(error: unknown): boolean {
  return (
    error instanceof JsTemplateCompilePoolError &&
    (error.code === 'JS_TEMPLATE_COMPILE_QUEUE_CAPACITY_EXCEEDED' ||
      error.code === 'JS_TEMPLATE_COMPILE_INFLIGHT_BYTES_EXCEEDED')
  );
}

function normalizeOptions(options: JsTemplateCompileWorkerPoolOptions): NormalizedPoolOptions {
  ...
  assertIntegerLimit('maxQueueLength', normalized.maxQueueLength, 0, JS_TEMPLATE_COMPILE_POOL_HARD_LIMITS.queuedJobs);
  assertIntegerLimit('maxJobBytes', normalized.maxJobBytes, 1, JS_TEMPLATE_COMPILE_POOL_HARD_LIMITS.jobBytes);
  assertIntegerLimit(
    'maxInFlightBytes',
    normalized.maxInFlightBytes,
    normalized.maxJobBytes,
    JS_TEMPLATE_COMPILE_POOL_HARD_LIMITS.inflightBytes,
  );
  assertIntegerLimit(
    'maxCapacityWaiters',
    normalized.maxCapacityWaiters,
    0,
    JS_TEMPLATE_COMPILE_POOL_HARD_LIMITS.capacityWaiters,
  );
...
class NodeCompileWorkerHandle implements JsTemplateCompileWorkerHandle {
  ...
  on(
    ...
    listener:
      | ((message: JsTemplateCompileWorkerResponse) => void)
      | ((error: Error) => void)
      | ((code: number) => void),
  ): this {
  ...
  async gracefulShutdown(timeoutMs: number): Promise<void> {
    await new Promise<void>((resolve, reject) => {
      ...
      const onMessage = (message: JsTemplateCompileWorkerResponse) => {
        if (message.type === 'shutdown-complete') {
          finish();
        }
      };
