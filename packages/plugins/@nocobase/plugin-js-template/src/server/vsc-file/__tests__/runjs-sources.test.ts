...
describe('runJSSources resource', () => {
  ...
  it('returns canonical Inline settings descriptors and diagnostics from open and openLatest', async () => {
    ...
    const externalValidation = new JsTemplateValidator().validateWorkspace({
      files: [
        {
    ...
    const externalSchema = externalValidation.templates[0].settingsSchema;
    const externalHashes = buildJsTemplateSettingsHashes(externalSchema);
