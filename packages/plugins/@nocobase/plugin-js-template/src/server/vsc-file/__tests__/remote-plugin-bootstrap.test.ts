...
describe('vsc-file remote runtime bootstrap', () => {
  it('reloads the Git adapter identity-safely and unregisters the runtime on disable', async () => {
    ...
    const module = new JsTemplateRemoteSyncModule(app, app.db, new VscPermissionHookRegistry());
    ...
    for (const actionName of jsTemplateSyncAuditActionNames) {
      expect(registeredAuditActions).toContainEqual(expect.objectContaining({ name: `jsTemplateSync:${actionName}` }));
    }
  ...
  it('emits Pull job and reconcile audit events for successful owner recovery discovery', async () => {
    ...
    await runtime.getPullCoordinator().discover(
      ...
      { request: { requestSource: 'js-template-pull-recovery' } },
    );
  ...
  it('emits failed Pull recovery and persisted conflict audit events when discovery fails', async () => {
    ...
    await expect(
      runtime.getPullCoordinator().discover(
        ...
        { request: { requestId: 'recover:job_pull', requestSource: 'js-template-pull-recovery' } },
      ),
