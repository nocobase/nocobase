...
export const BASE_JS_TEMPLATE_TEMPLATE_FILES: readonly JsTemplateTreeEntryInput[] = [
  { path: 'README.md', content: DEFAULT_JS_TEMPLATE_README, language: 'markdown' },
  { path: 'tsconfig.json', content: JS_TEMPLATE_TSCONFIG_CONTENT, language: 'json' },
];
...
`,
  ...
  "settings": {
    ...
    "message": { "type": "string", "title": "Message", "default": "Build reusable UI with JS Templates.", "x-component": "Input.TextArea" }
  }
}
`,
    language: 'json',
...
export function createJsTemplateBaseTemplate(): JsTemplateTreeEntryInput[] {
  return BASE_JS_TEMPLATE_TEMPLATE_FILES.map((file) => ({ ...file }));
}

export function createDefaultJsTemplateTemplate(): JsTemplateTreeEntryInput[] {
  return DEFAULT_JS_TEMPLATE_TEMPLATE_FILES.map((file) => ({ ...file }));
}
