...
const createChatFacade = (sessionId?: string) => {
  ...
  const actions = {
    ...
    setEditorRef: (uid: string, editorRef: EditorRef) => useChatMessagesStore.getState().setEditorRef(uid, editorRef),
    setCurrentEditorRefUid: (uid: string) => useChatMessagesStore.getState().setCurrentEditorRefUid(uid),
    setFlowContext: (flowContext: any) => useChatMessagesStore.getState().setFlowContext(flowContext),
