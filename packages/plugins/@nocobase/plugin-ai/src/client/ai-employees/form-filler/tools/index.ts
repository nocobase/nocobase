...
type FormBlockModelLike = FormBlockModel & {
...
function normalizeSubTableRow(row: Record<string, unknown>): Record<string, unknown> {
  ...
  return {
    ...row,
    __is_new__: true,
    [SUB_TABLE_TEMP_ROW_KEY]: row[SUB_TABLE_TEMP_ROW_KEY] ?? uid(),
  };
}
...
export const formFillerTool: [string, ToolsOptions] = [
  'formFiller',
  {
    invoke: async (app, params) => {
      ...
      if (!uid || !isPlainRecord(data)) {
        return;
      }

      const model = app.flowEngine.getModel(uid, true) as FormBlockModelLike | undefined;
      if (!model) {
        return;
      }
