...
const docsIndexes = new Map<string, DocsIndexMeta>();
...
export function getDocIndexKeys() {
  return Array.from(docsIndexes.keys()).sort();
}

export function describeDocIndexKeys(emptyMessage = 'No document indexes available. Run ai:create-docs-index first.') {
  const keys = getDocIndexKeys();
  return keys.length ? `Available indexes: ${keys.join(', ')}` : emptyMessage;
}

export async function searchDocsIndex(indexKey: string, query: string, limit = 5) {
  const key = normalizeIndexKey(indexKey);
  const entry = docsIndexes.get(key);
  if (!entry) {
    throw new Error(`Document index "${key}" not found.`);
  }
...
export function createDocsSearchTool(options?: { description?: string }): ToolOptions {
  return {
    name: 'searchDocs',
    title: '{{t("Search documentation")}}',
    description:
      options?.description ??
      'Search docs indexes generated by ai:create-docs-index (use plugin package names such as @nocobase/plugin-flow-engine).',
    schema: z.object({
      indexKey: z
        .string()
        .min(1, 'indexKey is required')
        .describe('Plugin package path, e.g. @nocobase/plugin-flow-engine'),
      query: z.string().min(1, 'query is required').describe('Search keyword or phrase'),
      limit: z.number().int().min(1).max(20).optional().describe('Maximum number of hits (default 5, max 20)'),
    }),
    invoke: async (ctx, args) => {
      const available = getDocIndexKeys();
      if (!available.length) {
        return {
          status: 'error',
          content: 'No document indexes available. Run the ai:create-docs-index command first.',
        };
      }

      const indexKey = args?.indexKey?.trim();
      ...
      if (!indexKey || !query) {
        return {
          status: 'error',
          content: `Both indexKey and query are required. Available indexes: ${available.join(', ')}`,
        };
      }

      try {
        const result = await searchDocsIndex(indexKey, query, limit);
        return {
          status: 'success',
...
export function createReadDocEntryTool(options?: { description?: string }): ToolOptions {
  return {
    name: 'readDocEntry',
    title: '{{t("Read documentation file")}}',
    description:
      options?.description ??
      'Read files or list directories inside storage/ai/docs using canonical @nocobase/plugin-*/ relative paths.',
    schema: z.object({
      path: z
        .string()
        .min(1, 'path is required')
        .describe(
          'Canonical path like @nocobase/plugin-flow-engine/js-model/js-column.md. No wildcards, relative segments, or globbing.',
        ),
    }),
    invoke: async (ctx, args) => {
      const available = getDocIndexKeys();
...
function normalizeIndexKey(input: string) {
  const { pluginKey, relativePath } = parseDocPath(input);
  if (relativePath) {
    throw new Error('Index key should only contain the plugin package path, e.g. @nocobase/plugin-flow-engine.');
  }
  return pluginKey;
}
...
function resolveDocPath(input: string) {
  ...
  const meta = docsIndexes.get(pluginKey);
  if (!meta) {
    throw new Error(`Document index "${pluginKey}" not found.`);
  }
