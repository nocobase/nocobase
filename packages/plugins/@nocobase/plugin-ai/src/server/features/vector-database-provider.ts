...
export interface VectorDatabaseProviderFeature {
  ...
  beforeCreate<T>(providerName: string, connectParams: T): Promise<{ status: number; message?: string }>;
...
export type VectorDatabaseProvider<T, R> = {
  ...
  beforeCreate(connectParams: T): Promise<{ status: number; message?: string }>;
