...
// Deleted runtime-artifact-cache alias case -> direct owner below:
// reuses the resolved immutable artifact for settings-only executions -> same-named case below.
// deduplicates concurrent Artifact GET requests for the same hash -> same-named case below.
// fetches artifacts through the API client for a custom prefixed URL -> same behavior case below.
// does not execute cached code after repository invalidation -> same-named case below.
// re-resolves at most once after an Artifact 404 -> same-named case below.
