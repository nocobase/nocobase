...
describe('LightExtensionWorkspacePage', () => {
  ...
  it('edits Portal text files and shows binary files as downloadable read-only nodes', async () => {
    ...
    render(
      <MemoryRouter>
        <LightExtensionWorkspacePage repoId="ler_sales" />
      </MemoryRouter>,
    );

    await screen.findByTestId('runjs-code-tab');
    fireEvent.click(screen.getByRole('button', { name: 'index.html' }));
    expect(screen.getByLabelText('Edit file content')).toHaveValue('<main>Customer</main>\n');
    expect(screen.getByLabelText('Edit file content')).not.toHaveAttribute('readonly');
    fireEvent.change(screen.getByLabelText('Edit file content'), {
      target: { value: '<main>Updated customer</main>\n' },
    });

    fireEvent.click(screen.getByRole('button', { name: 'logo.png' }));
    expect(await screen.findByTestId('light-extension-binary-file')).toHaveTextContent('Binary file · 5 B');
    expect(screen.queryByTestId('runjs-code-tab')).not.toBeInTheDocument();
    fireEvent.click(screen.getByRole('button', { name: 'Download file' }));
    expect(mocks.archive.downloadLightExtensionWorkspaceArchive).toHaveBeenCalledWith(expect.any(Blob), 'logo.png');

    fireEvent.click(screen.getByRole('button', { name: 'index.html' }));
    fireEvent.click(screen.getByRole('button', { name: /Save/ }));
    await confirmSaveVersion('Update customer portal');
    await waitFor(() => expect(mocks.api.saveSource).toHaveBeenCalledTimes(1));
    expect(mocks.api.saveSource.mock.calls[0][0].files).toEqual([
      expect.objectContaining({
        path: 'src/client/js-portals/customer/index.html',
        content: '<main>Updated customer</main>\n',
        encoding: 'utf8',
        operation: 'upsert',
      }),
    ]);
  ...
  it('preserves imported Portal binary metadata when saving the repository workspace', async () => {
    mocks.api.pull.mockResolvedValueOnce({
      repo: { id: 'ler_sales' },
      commit: { id: 'commit-1' },
      tree: { hash: 'tree-1', entryCount: 2, byteSize: 100 },
      unchanged: false,
      files: [
        {
          path: 'src/client/js-blocks/sales-kpi/index.tsx',
    ...
    fireEvent.click(screen.getByRole('button', { name: 'logo.png' }));
    expect(await screen.findByTestId('light-extension-binary-file')).toHaveTextContent('Binary file · 7 B');
    fireEvent.click(screen.getByRole('button', { name: /Save/ }));
    await confirmSaveVersion('Replace customer logo');
    await waitFor(() => expect(mocks.api.saveSource).toHaveBeenCalledTimes(1));
    expect(mocks.api.saveSource.mock.calls[0][0].files).toEqual([
      expect.objectContaining({
        path: 'src/client/js-portals/customer/logo.png',
