/**
 * This file is part of the NocoBase (R) project.
 * Copyright (c) 2020-2024 NocoBase Co., Ltd.
 * Authors: NocoBase Team.
 *
 * This project is dual-licensed under AGPL-3.0 and NocoBase Commercial License.
 * For more information, please refer to: https://www.nocobase.com/agreement.
 */

export const PUBLIC_FORMS_NAMESPACE = 'public-forms';
export const PUBLIC_FORMS_SETTINGS_ROUTE_NAME = 'admin.settings.public-forms.layout';
export const PUBLIC_FORMS_SETTINGS_LAYOUT_UID = 'public-forms-settings-layout-model';
export const PUBLIC_FORMS_SETTINGS_LAYOUT_MODEL = 'PublicFormsSettingsLayoutModel';
export const PUBLIC_FORM_ROUTE_NAME = 'public-forms';
export const PUBLIC_FORM_LAYOUT_UID = 'public-form-layout-model';
export const PUBLIC_FORM_LAYOUT_MODEL = 'PublicFormLayoutModel';
export const PUBLIC_FORM_PAGE_MODEL = 'PublicFormPageModel';
export const PUBLIC_FORM_SUBMIT_ACTION_MODEL = 'PublicFormSubmitActionModel';
export const PUBLIC_FORM_TOKEN_KEY = 'NOCOBASE_FORM_TOKEN';
export const DEFAULT_SUCCESS_MESSAGE = '# Submitted successfully!\nThis is a demo text, **supports Markdown syntax**.';
