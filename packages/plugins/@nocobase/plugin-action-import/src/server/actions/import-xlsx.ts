...
export function readImportWorkbook(file: Buffer | string, sheetRows: number) {
...
async function importXlsxAction(ctx: Context, next: Next) {
  ...
  const uploadedFilePath = ctx.file.path;
  try {
    const workbook = readImportWorkbook(uploadedFilePath || ctx.file.buffer, readLimit);
    ...
    if (uploadedFilePath) {
      await fs.promises.rm(uploadedFilePath, { force: true });
    }
  }
}
