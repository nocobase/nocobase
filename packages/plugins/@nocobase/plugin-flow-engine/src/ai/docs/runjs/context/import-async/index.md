...
- `url`: ESM module address. Default format is `<module-name>@<version>` (e.g., `vue@3.4.0`), which will be prefixed by `NPM_MODULE_BASE_URL` (default `https://esm.sh`). Full URLs are also supported (e.g., `https://esm.sh/vue@3.4.0`).
