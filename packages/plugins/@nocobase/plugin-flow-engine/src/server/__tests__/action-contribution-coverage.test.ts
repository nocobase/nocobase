...
import { flowSchemaActionContributions } from '../flow-schema-contributions/actions';
...
describe('flow schema action contribution coverage', () => {
  it('should register server contributions for every core dynamic event flow action', () => {
    const clientActionNames = getCoreDynamicEventActionNames();
    const registeredActionNames = Array.from(
      new Set(
        flowSchemaActionContributions
          .map((contribution) => contribution.name)
          .filter((name) => clientActionNames.includes(name)),
      ),
    ).sort();
