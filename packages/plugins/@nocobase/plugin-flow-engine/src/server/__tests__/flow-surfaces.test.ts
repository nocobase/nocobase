...
const FLOW_SURFACE_TEST_VISIBLE_FIELD_MINIMUM_BLOCK_TYPES = new Set(['table', 'list', 'gridCard', 'details']);
...
describe('flowSurfaces resource', () => {
        ...
        blocks: [
          {
            ...
            fields: ['nickname', 'department.title', 'status'],
          },
        ],
      },
    });
        ...
        await rootAgent.resource('flowSurfaces').addBlock({
          values: {
            ...
            ...(FLOW_SURFACE_TEST_PUBLIC_DATA_BLOCK_TYPES.has(blockCase.type)
              ...
              : {}),
            ...(FLOW_SURFACE_TEST_VISIBLE_FIELD_BLOCK_TYPES.has(blockCase.type)
              ? { fields: buildFlowSurfaceTestVisibleFields(blockCase.resourceInit, blockCase.type) }
              : {}),
          },
        }),
      );
    ...
    for (const blockCase of [
      ...
      const addBlockRes = await rootAgent.resource('flowSurfaces').addBlock({
        values: {
          ...
          ...(blockCase.type === 'table'
            ? {
                fields: buildFlowSurfaceTestVisibleFields({ collectionName: blockCase.collectionName }, blockCase.type),
              }
            : {}),
          defaultFilter: blockCase.defaultFilter,
        },
  ...
  it('should auto-complete association field popups with defaultType edit and save them as reusable templates', async () => {
    ...
    expect(templatedPopup.name).not.toContain('待翻译');
    expect(templatedPopup.description).not.toContain('待翻译');
    ...
    expect(zhTemplatedPopup.name).not.toContain('待翻译');
    expect(zhTemplatedPopup.description).not.toContain('待翻译');
  ...
  it('should persist compiled auto-generated popup template metadata for flowModelTemplates lookups', async () => {
    ...
    try {
      ...
      if (departmentsCollection?.options) {
        departmentsCollection.options.title = '{{t("Organizations")}}';
      }
      departmentField.title = '{{t("Department")}}';
      if (departmentField?.options) {
        departmentField.options.title = '{{t("Department")}}';
      }

      const page = await createPage(rootAgent, {
        title: 'Persisted popup template metadata page',
        tabTitle: 'Persisted popup template metadata tab',
      });
      ...
      const templatedPopupUid = defaultViewPopupReadback.tree.popup?.template?.uid;
      expect(typeof templatedPopupUid).toBe('string');
      ...
      if (departmentsCollection?.options) {
        departmentsCollection.options.title = originalDepartmentsOptionsTitle;
      }
      departmentField.title = originalDepartmentFieldTitle;
      if (departmentField?.options) {
        ...
        blocks: [
          {
            ...
            fields: ['department', 'nickname', 'status'],
          },
        ],
      },
    });
  ...
  it('should preserve empty action popup payload semantics for external popup targets', async () => {
    ...
    expect(externalPopupFieldPaths).toEqual(['nickname', 'status', 'email']);
  });
  ...
  it('should create edit-form and details blocks with fields actions and event flow replacement', async () => {
    ...
    const detailsUid = await addBlock(
      ...
      {
        fields: ['department.title', 'nickname', 'status'],
      },
    );
  ...
  it('should compile i18n tokens in auto-generated default action popup template metadata before saving', async () => {
    const service = new FlowSurfacesService(app.pm.get('flow-engine') as any);
    const originalFindModelById = service.repository.findModelById.bind(service.repository);
    const originalResolvePopupBlockProfile = (service as any).resolvePopupBlockProfile;

    try {
      service.repository.findModelById = (async () => ({
        uid: 'action-host',
        use: 'ViewActionModel',
      })) as any;
      (service as any).resolvePopupBlockProfile = async () => ({
        currentCollection: {
          title: '{{t("Users")}}',
          name: 'users',
        },
      ...
      (service as any).resolvePopupBlockProfile = originalResolvePopupBlockProfile;
    }
  });
...
function buildFlowSurfaceTestVisibleFields(resourceInit?: Record<string, any>, type?: string) {
  const collectionName = String(resourceInit?.collectionName || '').trim();
  const paths = FLOW_SURFACE_TEST_DEFAULT_FILTER_FIELDS_BY_COLLECTION[collectionName] || ['title'];
  const fieldCount = FLOW_SURFACE_TEST_VISIBLE_FIELD_MINIMUM_BLOCK_TYPES.has(String(type || '').trim()) ? 3 : 1;
  return paths.slice(0, fieldCount);
}
...
async function addBlock(
  ...
  const defaultFieldsForVisibleDataBlock =
    ...
    !hasFlowSurfaceTestVisibleFieldConfig(extraValues)
      ? buildFlowSurfaceTestVisibleFields(normalizedResourceInit, type)
      : undefined;
