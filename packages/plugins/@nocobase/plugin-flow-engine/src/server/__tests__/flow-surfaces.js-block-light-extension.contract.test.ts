...
describe('flowSurfaces JS block light-extension contract', () => {
  ...
  it('should apply template-backed compose settings before deciding whether to bootstrap a workspace', async () => {
    ...
    const lightExtensionSource = getData(
      await rootAgent.resource('flowSurfaces').addBlock({
        values: {
          target: { uid: page.tabSchemaUid },
          type: 'jsBlock',
    ...
    const lightExtensionTemplate = getData(
      await rootAgent.resource('flowSurfaces').saveTemplate({
        values: {
          target: { uid: lightExtensionSource.uid },
          name: 'Light Extension JS block template',
        ...
        blocks: [
          ...
          },
          {
            ...
            settings: {
              ...
              sourceBinding: {
                ...LIGHT_EXTENSION_SOURCE_BINDING,
                entryId: 'entry_template_externalized',
              },
            },
          },
          {
            key: 'movedInline',
            template: { uid: lightExtensionTemplate.uid, mode: 'copy' },
