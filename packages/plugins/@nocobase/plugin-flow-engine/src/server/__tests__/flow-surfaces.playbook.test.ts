...
describe('flowSurfaces ai docs', () => {
  it('should keep skill.md as the stable main entry and cover recommended JS public capabilities', () => {
    ...
    expect(content).toContain('Do not write `{ "target": { "uid": "..." } }`');
    ...
    expect(content).toContain('truly available public capabilities');
    ...
    expect(content).toContain('association-field popup');
    expect(content).toContain('non-association-field popup');
    expect(content).toContain('plain popup');
    ...
    expect(content).toContain('`addAction` is only for non-record actions');
    expect(content).toContain('`addRecordAction` is only for record actions');
    expect(content).toContain('`addBlocks`:');
    expect(content).toContain('`addFields`:');
    expect(content).toContain('`addActions`:');
    expect(content).toContain('`addRecordActions`:');
    expect(content).toContain('`message`, `type`, `code`, `status`');
    expect(content).toContain('direct `add*` also supports inline `settings`');
    expect(content).toContain('the `settings` shape is the same as `configure.changes`');
    expect(content).toContain('popup-capable actions may include `popup` directly');
