...
type FlowSurfacesUsageInternals = {
...
function createHarness(initialOptions: FlowModelOptions, options: { providerAvailable?: boolean } = {}) {
  ...
  return {
    service: service as unknown as FlowSurfacesUsageInternals,
    repository,
    model,
    syncUsages,
    markMissing,
...
describe('flowSurfaces JS Template usage synchronization', () => {
  it('synchronizes usage creation and binding changes from the persisted canonical runJs state', async () => {
    ...
    expect(harness.syncUsages).toHaveBeenCalledTimes(1);
    expect(harness.syncUsages).toHaveBeenCalledWith(
