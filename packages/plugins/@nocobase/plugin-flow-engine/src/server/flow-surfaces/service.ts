...
export class FlowSurfacesService {
  ...
  private async resolveAutoGeneratedFieldPopupTemplateMetadata(
    ...
    try {
      ...
      const fieldInit =
        fieldNode?.stepParams?.fieldSettings?.init || wrapperNode?.stepParams?.fieldSettings?.init || {};
      const dataSourceKey = String(fieldInit.dataSourceKey || 'main').trim() || 'main';
      const sourceCollectionName = String(fieldInit.collectionName || '').trim() || undefined;
      ...
      if (!sourceCollectionName || !fieldPath) {
        return this.buildLegacyAutoGeneratedPopupTemplateMetadata(identity);
      }
      ...
      if (!associationContext.associationField || !associationContext.targetCollection) {
        return this.buildLegacyAutoGeneratedPopupTemplateMetadata(identity);
      }

      const sourceCollectionLabel = String(getCollectionTitle(sourceCollection) || '').trim() || sourceCollectionName;
      const associationFieldLabel =
        String(
          getFieldTitle(associationContext.associationField) || getFieldName(associationContext.associationField) || '',
        ).trim() || String(getFieldName(associationContext.associationField) || '').trim();
      const targetCollectionLabel =
        String(getCollectionTitle(associationContext.targetCollection) || '').trim() ||
        String(getCollectionName(associationContext.targetCollection) || '').trim();
  ...
  private inferPopupDefaultTypeFromTemplateTree(node: any): 'view' | 'edit' | undefined {
