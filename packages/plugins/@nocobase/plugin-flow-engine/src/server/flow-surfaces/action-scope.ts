...
export const RECORD_HISTORY_BLOCK_ACTION_CONTAINER_USES = ['RecordHistoryBlockModel'];
...
export const COMMENT_RECORD_ACTION_CONTAINER_USES = ['CommentItemModel'];
...
export const ACTION_CONTAINER_SCOPE_BY_USE: Record<string, FlowSurfaceActionScope> = Object.fromEntries([
  ...
  ...RECORD_HISTORY_BLOCK_ACTION_CONTAINER_USES.map((use) => [use, 'block'] as const),
  ...RECORD_ACTION_CONTAINER_USES.map((use) => [use, 'record'] as const),
  ...COMMENT_RECORD_ACTION_CONTAINER_USES.map((use) => [use, 'record'] as const),
