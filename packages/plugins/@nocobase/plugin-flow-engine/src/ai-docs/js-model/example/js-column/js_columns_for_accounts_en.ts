...
const industryPalette: Record<string, { color: string; label: string }> = {
...
function getRecord(ctx: ColumnCtx): AccountRecord {
  return (ctx.record || {}) as AccountRecord;
}
 ...
 */
export function renderAccountIdentityColumn(ctx: ColumnCtx) {
  ...
  const nameButton = container.querySelector<HTMLButtonElement>('button.account-name');
  if (nameButton) {
 ...
 */
export function renderAccountHealthScore(ctx: ColumnCtx) {
  const record = getRecord(ctx);
  const container = ctx.element as HTMLElement;
  if (!container) return;
 ...
 */
export function renderAccountRelationshipBadges(ctx: ColumnCtx) {
  const record = getRecord(ctx);
  const container = ctx.element as HTMLElement;
  if (!container) return;
  ...
  container.querySelectorAll<HTMLButtonElement>('.relation-pill').forEach((button) => {
    button.onclick = () => {
 ...
 */
export function renderAccountRevenuePotential(ctx: ColumnCtx) {
  const record = getRecord(ctx);
  const container = ctx.element as HTMLElement;
  if (!container) return;
  ...
  const pipelineRow = container.querySelector<HTMLDivElement>('.pipeline');
