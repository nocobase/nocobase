...
function getRecord(ctx: ColumnCtx): OpportunityRecord {
  return (ctx.record || {}) as OpportunityRecord;
}
 ...
 */
export function renderOpportunityStageTracker(ctx: ColumnCtx) {
 ...
 */
export function renderOpportunitySummary(ctx: ColumnCtx) {
  const record = getRecord(ctx);
  const container = ctx.element as HTMLElement;
  if (!container) return;
 ...
 */
export function renderOpportunityStakeholderColumn(ctx: ColumnCtx) {
  const record = getRecord(ctx);
  const container = ctx.element as HTMLElement;
  if (!container) return;
  ...
  const moreButton = container.querySelector<HTMLButtonElement>('button.view-more');
  if (moreButton) {
    moreButton.onclick = () => {
