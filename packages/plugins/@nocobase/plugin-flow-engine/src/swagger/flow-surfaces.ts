...
const PUBLIC_DATA_SURFACE_BLOCK_DEFAULT_FILTER_DESCRIPTION =
  'Supported only on direct `table`, `list`, `gridCard`, `calendar`, or `kanban` blocks. When omitted, backend authoring generates a default filter from live collection metadata, aiming for 3-4 scalar/filterable fields and never more than 4. Explicit values must contain at least one concrete caller-chosen filter item; invalid operators, unknown paths, and relation fields used directly fail authoring validation. Use scalar fields or relation child paths such as `department.title`, not the relation field itself. A valid explicit or generated value backfills the default `filter` action `settings.defaultFilter` when that action exists or is auto-created. If the filter action already provides `settings.defaultFilter`, the action-level value wins.';
const APPLY_BLUEPRINT_DATA_SURFACE_BLOCK_DEFAULT_FILTER_DESCRIPTION =
  'Supported only on direct `table`, `list`, `gridCard`, `calendar`, or `kanban` blocks. When omitted, applyBlueprint generates a default filter from live collection metadata, aiming for 3-4 scalar/filterable fields and never more than 4. Explicit values must contain at least one concrete caller-chosen filter item; invalid operators, unknown paths, and relation fields used directly fail authoring validation. Use scalar fields or relation child paths such as `department.title`, not the relation field itself. A valid explicit or generated value backfills the default `filter` action `settings.defaultFilter` when that action exists or is auto-created. If the filter action already provides `settings.defaultFilter`, the action-level value wins.';
const DIRECT_ADD_DEFAULT_FILTER_COMPAT_DESCRIPTION =
  'Public block-level `defaultFilter` is optional on direct non-template `table` / `list` / `gridCard` / `calendar` / `kanban` authoring creates. When no effective filter action or `defaultActionSettings.filter.defaultFilter` override is supplied, the backend auto-materializes one from live collection metadata, aiming for 3-4 scalar/filterable fields and never more than 4. Explicit values still validate strictly, and empty, invalid-operator, unknown-path, and relation-field-leaf payloads are reported through aggregate `errors[]`.';
...
const actionDocs: Record<string, any> = {
  ...
  addBlock: {
    ...
    description: valuesCompatibilityNote(
      'Creates a block by catalog key or an explicitly supported block use. It can also create from `template`, using `mode="reference"` or `mode="copy"`. Form templates may set `template.usage="fields"` to create a fresh host block and import only its grid fields. Popup-capable host nodes automatically receive the popup shell. For collection blocks under a popup, check `catalog.blocks[].resourceBindings` first, then pass the semantic `resource.binding`. The lower-level `resourceInit` is still accepted for compatibility, but the server validates it against popup semantics. `resource` and `resourceInit` are mutually exclusive. Direct `table` / `list` / `gridCard` / `calendar` / `kanban` creation may omit `defaultFilter`; backend authoring auto-generates one from live metadata, aiming for 3-4 fields and never more than 4. Explicit empty, invalid-operator, unknown-path, and relation-field-leaf payloads still fail through aggregate `errors[]`. Generated default popups for collections with more than 10 business fields require `defaults.collections.<collection>.fieldGroups`; otherwise authoring returns aggregate `errors[]` with ruleId `missing-default-field-groups`. A valid explicit or generated block-level value backfills the auto-created default filter action. `defaultActionSettings.filter.defaultFilter` remains supported for compatibility and wins over block-level `defaultFilter`. `skipDefaultActions` and `skipDefaultRecordActions` explicitly opt out of the auto-created default action groups; merely passing partial `actions` or `recordActions` no longer suppresses missing defaults. The `select / subForm / bulkEditForm` scene is currently recognized only, and popup collection block creation is not supported in that scene. Direct add does not accept raw `props` / `decoratorProps` / `stepParams` / `flowRegistry`. Use `settings` and reuse the public configuration semantics from `configure.changes` plus the catalog item/node `configureOptions`. Approval block keys are only exposed under approval grids and do not auto-create an approval root; bootstrap approval surfaces with `applyApprovalBlueprint` first.',
    ),
    requestBody: {
      required: true,
      content: {
  ...
  addBlocks: {
    ...
    description: valuesCompatibilityNote(
      'Creates multiple blocks sequentially under the same target. Each item may include `settings`, `defaultFilter`, `defaultActionSettings`, `template`, or inline `fields` / `fieldsLayout`, but raw `props` / `decoratorProps` / `stepParams` / `flowRegistry` are not accepted. Inline fields use the same public field semantics as compose/addField, including relation `fieldType`. Direct `table` / `list` / `gridCard` / `calendar` / `kanban` items may omit `defaultFilter`; backend authoring auto-generates one from live metadata, aiming for 3-4 fields and never more than 4. Explicit empty, invalid-operator, unknown-path, and relation-field-leaf payloads still fail through aggregate `errors[]`. Generated default popups for collections with more than 10 business fields require top-level `defaults.collections.<collection>.fieldGroups`; otherwise authoring returns aggregate `errors[]` with ruleId `missing-default-field-groups`. A valid explicit or generated block-level value backfills the auto-created default filter action. `defaultActionSettings.filter.defaultFilter` remains supported for compatibility and wins over block-level `defaultFilter`. `skipDefaultActions` and `skipDefaultRecordActions` explicitly opt out of the auto-created default action groups; merely passing partial `actions` or `recordActions` no longer suppresses missing defaults. Authoring validation is all-or-none before the batch starts: any aggregate `errors[]` response rejects the whole request before item creation begins. Once validation passes, runtime item failures are returned in input order as `index/key/ok/result/error`, and each `error` always includes `message/type/code/status`.',
    ),
  ...
  FlowSurfaceComposeBlockSpec: {
    ...
    properties: {
      ...
      skipDefaultActions: {
        type: 'boolean',
        description:
          'Explicitly skip auto-created block-level default actions such as filter/refresh/addNew. Partial actions[] no longer suppress missing defaults.',
      },
      skipDefaultRecordActions: {
        type: 'boolean',
        description:
          'Explicitly skip auto-created record default actions such as view/edit/delete. Partial recordActions[] no longer suppress missing defaults.',
      },
    },
    additionalProperties: false,
  },
