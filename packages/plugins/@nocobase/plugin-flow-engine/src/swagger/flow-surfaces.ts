...
const PUBLIC_DATA_SURFACE_BLOCK_DEFAULT_FILTER_DESCRIPTION =
  'Supported only on direct `table`, `list`, `gridCard`, or `calendar` blocks, and must contain at least one concrete filter item. Backfills the default `filter` action `settings.defaultFilter` when that action exists or is auto-created. If the filter action already provides `settings.defaultFilter`, the action-level value wins.';
const DIRECT_ADD_DEFAULT_FILTER_COMPAT_DESCRIPTION =
  'Public block-level `defaultFilter` is supported on direct `table` / `list` / `gridCard` / `calendar` creates. Legacy `defaultActionSettings.filter.defaultFilter` remains supported for compatibility and wins when both are provided.';
  ...
  FlowSurfaceComposeBlockSpec: {
    ...
    properties: {
      ...
      actions: {
        type: 'array',
        description:
          'Block-level actions. For table/list/gridCard/calendar, prefer block-wide collection actions here.',
