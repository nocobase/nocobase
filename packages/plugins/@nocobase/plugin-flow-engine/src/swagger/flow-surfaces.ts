...
const PUBLIC_DATA_SURFACE_BLOCK_DEFAULT_FILTER_DESCRIPTION =
  'Supported only on direct `table`, `list`, `gridCard`, `calendar`, or `kanban` blocks. Authoring writes must provide an explicit effective filter chosen by the caller; backend validation reports omitted, empty, invalid-operator, unknown-path, and relation-field-leaf payloads through aggregate `errors[]`. Use scalar fields or relation child paths such as `department.title`, not the relation field itself. A valid value backfills the default `filter` action `settings.defaultFilter` when that action exists or is auto-created. If the filter action already provides `settings.defaultFilter`, the action-level value wins.';
const APPLY_BLUEPRINT_DATA_SURFACE_BLOCK_DEFAULT_FILTER_DESCRIPTION =
  'Supported only on direct `table`, `list`, `gridCard`, `calendar`, or `kanban` blocks, and must contain at least one concrete caller-chosen filter item. Invalid operators, unknown paths, and relation fields used directly fail authoring validation; use scalar fields or relation child paths such as `department.title`. Backfills the default `filter` action `settings.defaultFilter` when that action exists or is auto-created. If the filter action already provides `settings.defaultFilter`, the action-level value wins.';
const DIRECT_ADD_DEFAULT_FILTER_COMPAT_DESCRIPTION =
  'Public block-level `defaultFilter` is required on direct non-template `table` / `list` / `gridCard` / `calendar` / `kanban` authoring creates unless an effective filter action or `defaultActionSettings.filter.defaultFilter` override is supplied. The backend reports missing, empty, invalid-operator, unknown-path, and relation-field-leaf payloads through aggregate `errors[]`; UI Builder chooses the filter fields.';
...
const actionDocs: Record<string, any> = {
  ...
  addBlocks: {
    ...
    description: valuesCompatibilityNote(
      'Creates multiple blocks sequentially under the same target. Each item may include `settings`, `defaultFilter`, `defaultActionSettings`, `template`, or inline `fields` / `fieldsLayout`, but raw `props` / `decoratorProps` / `stepParams` / `flowRegistry` are not accepted. Inline fields use the same public field semantics as compose/addField, including relation `fieldType`. Direct `table` / `list` / `gridCard` / `calendar` / `kanban` items must provide an explicit effective `defaultFilter`; backend authoring reports missing, empty, invalid-operator, unknown-path, and relation-field-leaf payloads through aggregate `errors[]` instead of choosing fields. A valid block-level value backfills the auto-created default filter action. Legacy `defaultActionSettings.filter.defaultFilter` is still supported for compatibility and wins over block-level `defaultFilter`. Partial-success semantics apply: a failure in one item does not roll back the others. Results are returned in input order as `index/key/ok/result/error`, and each `error` always includes `message/type/code/status`.',
    ),
