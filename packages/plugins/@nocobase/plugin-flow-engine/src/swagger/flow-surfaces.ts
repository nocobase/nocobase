...
const FLOW_SURFACES_READ_ACL_NOTE =
  'Read actions (`get` / `describeSurface` / `exportBlueprint` / `listNavigationTargets` / `capabilities` / `describeCapability` / `validateCapabilityCreate` / `diagnoseCapabilities` / `catalog` / `context` / `getReactionMeta` / `getEventFlowMeta` / `listTemplates` / `getTemplate`) are open to `loggedIn` by default. Write actions still require the `ui.flowSurfaces` snippet.';
