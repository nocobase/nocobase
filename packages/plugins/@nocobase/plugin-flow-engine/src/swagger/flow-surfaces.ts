...
const PUBLIC_DATA_SURFACE_BLOCK_DEFAULT_FILTER_DESCRIPTION =
  'Supported only on direct `table`, `list`, `gridCard`, `calendar`, or `kanban` blocks. When omitted, backend authoring generates a default filter from live collection metadata with exactly 4 scalar/filterable fields when enough candidates exist. Explicit values must contain at least 4 concrete caller-chosen filter fields; values with more than 4 fields are truncated to the first 4 before persistence. Invalid operators, unknown paths, and relation fields used directly fail authoring validation. Use scalar fields or relation child paths such as `department.title`, not the relation field itself. A valid explicit or generated value backfills the default `filter` action `settings.defaultFilter` when that action exists or is auto-created. If the filter action already provides `settings.defaultFilter`, the action-level value wins.';
const APPLY_BLUEPRINT_DATA_SURFACE_BLOCK_DEFAULT_FILTER_DESCRIPTION =
  'Supported only on direct `table`, `list`, `gridCard`, `calendar`, or `kanban` blocks. When omitted, applyBlueprint generates a default filter from live collection metadata with exactly 4 scalar/filterable fields when enough candidates exist. Explicit values must contain at least 4 concrete caller-chosen filter fields; values with more than 4 fields are truncated to the first 4 before persistence. Invalid operators, unknown paths, and relation fields used directly fail authoring validation. Use scalar fields or relation child paths such as `department.title`, not the relation field itself. A valid explicit or generated value backfills the default `filter` action `settings.defaultFilter` when that action exists or is auto-created. If the filter action already provides `settings.defaultFilter`, the action-level value wins.';
const DIRECT_ADD_DEFAULT_FILTER_COMPAT_DESCRIPTION =
  'Public block-level `defaultFilter` is optional on direct non-template `table` / `list` / `gridCard` / `calendar` / `kanban` authoring creates. When no effective filter action or `defaultActionSettings.filter.defaultFilter` override is supplied, the backend auto-materializes one from live collection metadata with exactly 4 scalar/filterable fields when enough candidates exist. Explicit values with fewer than 4 fields, empty values, invalid operators, unknown paths, and relation-field-leaf payloads are reported through aggregate `errors[]`; values with more than 4 fields are truncated to the first 4 before persistence.';
...
const actionDocs: Record<string, any> = {
  ...
  addBlock: {
    ...
    description: valuesCompatibilityNote(
      'Creates a block by catalog key or an explicitly supported block use. It can also create from `template`, using `mode="reference"` or `mode="copy"`. Form templates may set `template.usage="fields"` to create a fresh host block and import only its grid fields. Popup-capable host nodes automatically receive the popup shell. For collection blocks under a popup, check `catalog.blocks[].resourceBindings` first, then pass the semantic `resource.binding`. The lower-level `resourceInit` is still accepted for compatibility, but the server validates it against popup semantics. `resource` and `resourceInit` are mutually exclusive. Direct `table` / `list` / `gridCard` / `calendar` / `kanban` creation may omit `defaultFilter`; backend authoring auto-generates one from live metadata with exactly 4 fields when enough candidates exist. Explicit values with fewer than 4 fields, empty values, invalid operators, unknown paths, and relation-field-leaf payloads fail through aggregate `errors[]`; values with more than 4 fields are truncated before persistence. Generated default popups for collections with more than 10 business fields require `defaults.collections.<collection>.fieldGroups`; otherwise authoring returns aggregate `errors[]` with ruleId `missing-default-field-groups`. A valid explicit or generated block-level value backfills the auto-created default filter action. `defaultActionSettings.filter.defaultFilter` remains supported for compatibility and wins over block-level `defaultFilter`. Default action groups always merge with explicit actions; partial `actions` or `recordActions` do not suppress missing defaults. The `select / subForm / bulkEditForm` scene is currently recognized only, and popup collection block creation is not supported in that scene. Direct add does not accept raw `props` / `decoratorProps` / `stepParams` / `flowRegistry`. Use `settings` and reuse the public configuration semantics from `configure.changes` plus the catalog item/node `configureOptions`. Approval block keys are only exposed under approval grids and do not auto-create an approval root; bootstrap approval surfaces with `applyApprovalBlueprint` first.',
    ),
    requestBody: {
      required: true,
      content: {
  ...
  addBlocks: {
    ...
    description: valuesCompatibilityNote(
      'Creates multiple blocks sequentially under the same target. Each item may include `settings`, `defaultFilter`, `defaultActionSettings`, `template`, or inline `fields` / `fieldsLayout`, but raw `props` / `decoratorProps` / `stepParams` / `flowRegistry` are not accepted. Inline fields use the same public field semantics as compose/addField, including relation `fieldType`. Direct `table` / `list` / `gridCard` / `calendar` / `kanban` items may omit `defaultFilter`; backend authoring auto-generates one from live metadata with exactly 4 fields when enough candidates exist. Explicit values with fewer than 4 fields, empty values, invalid operators, unknown paths, and relation-field-leaf payloads fail through aggregate `errors[]`; values with more than 4 fields are truncated before persistence. Generated default popups for collections with more than 10 business fields require top-level `defaults.collections.<collection>.fieldGroups`; otherwise authoring returns aggregate `errors[]` with ruleId `missing-default-field-groups`. A valid explicit or generated block-level value backfills the auto-created default filter action. `defaultActionSettings.filter.defaultFilter` remains supported for compatibility and wins over block-level `defaultFilter`. Default action groups always merge with explicit actions; partial `actions` or `recordActions` do not suppress missing defaults. Authoring validation is all-or-none before the batch starts: any aggregate `errors[]` response rejects the whole request before item creation begins. Once validation passes, runtime item failures are returned in input order as `index/key/ok/result/error`, and each `error` always includes `message/type/code/status`.',
    ),
  ...
  FlowSurfaceApplyBlueprintNavigationGroup: {
    ...
    properties: {
      ...
      icon: {
        type: 'string',
        description:
          'Group icon used only when create mode actually creates a new menu group. Ignored when routeId is present or when title resolves to an existing group.',
      },
      tooltip: {
        type: 'string',
        description:
          'Group tooltip used only when create mode actually creates a new menu group. Ignored when routeId is present or when title resolves to an existing group.',
      },
      hideInMenu: {
        type: 'boolean',
        description:
          'Group hidden-state used only when create mode actually creates a new menu group. Ignored when routeId is present or when title resolves to an existing group.',
      },
    },
    additionalProperties: false,
  },
