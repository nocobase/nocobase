...
import {
  canAccessNestedFieldPath,
  expandRestrictedSelectionPaths,
  getActionCollection,
  normalizeFieldNames,
} from './utils/field-acl';
  ...
  async beforeLoad() {
    ...
    this.app.acl.use(
      async (ctx: Context, next) => {
        ...
        const requestedNestedFieldPaths = lodash
          .uniq(normalizeFieldNames(ctx.action.params?.fields))
          .filter((path) => path.includes('.'));

        const requestedNestedAppendPaths = lodash
          .uniq(normalizeFieldNames(ctx.action.params?.appends))
          .filter((path) => path.includes('.'));

        if (!requestedNestedFieldPaths.length && !requestedNestedAppendPaths.length) {
          return next();
        }

        const permissionParams = ctx.permission.can.params || {};
        const expandedSelectionPaths = expandRestrictedSelectionPaths(ctx, collection, permissionParams);

        if (expandedSelectionPaths.fields.length) {
          permissionParams.fields = expandedSelectionPaths.fields;
        }

        if (expandedSelectionPaths.appends.length) {
          permissionParams.appends = expandedSelectionPaths.appends;
        }

        const allowedNestedFields = requestedNestedFieldPaths.filter((path) =>
          canAccessNestedFieldPath(ctx, collection, path, permissionParams),
        );
        const allowedNestedAppends = requestedNestedAppendPaths.filter((path) =>
          canAccessNestedFieldPath(ctx, collection, path, permissionParams),
        );

        if (allowedNestedFields.length) {
          permissionParams.fields = lodash.uniq([
            ...normalizeFieldNames(permissionParams.fields),
            ...allowedNestedFields,
          ]);
        }
