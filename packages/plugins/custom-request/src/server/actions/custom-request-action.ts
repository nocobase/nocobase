...
import CustomRequestRepository, { GetJsonSchemaOptions, GetPropertiesOptions } from '../../repository';

const getRepositoryFromCtx = (ctx: Context) => {
  const repo = ctx.db.getCollection('customRequest').repository as CustomRequestRepository;
