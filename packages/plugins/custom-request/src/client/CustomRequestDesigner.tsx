...
import { useCustomRequestContext } from './CustomRequestProvider';

export const CustomRequestDesigner = () => {
  const { name, title } = useCollection();
  const field = useField();
  const fieldSchema = useFieldSchema();
  const dataSource = useCollectionFilterOptions(name);
  const { service } = useCustomRequestContext();
  const { t: cT } = useCustomRequestTranslation();
  ...
  return (
    <GeneralSchemaDesigner template={template} title={title || name}>
      ...
      <SchemaSettings.ModalItem
        title={cT('The default zoom level of the map')}
        schema={
          {
            type: 'object',
            title: cT('Set default zoom level'),
            properties: {
              zoom: {
                title: cT('Zoom'),
                default: defaultZoom,
