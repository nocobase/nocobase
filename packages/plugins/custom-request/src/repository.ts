...
export class CustomRequestRepository extends Repository {
  cache: Cache;
  ...
  protected async findNodeSchemaWithParent(uid, transaction) {
    const schema = await this.database.getRepository('customRequest').findOne({
      filter: {
        'x-uid': uid,
      },
      transaction,
  ...
  protected async isSingleChild(uid, transaction) {
    ...
    if (parentChildrenCount == 1) {
      const schema = await db.getRepository('customRequest').findOne({
        filter: {
          'x-uid': parent,
        },
        transaction,
  ...
  protected async insertBeside(
    ...
    const nodes = CustomRequestRepository.schemaToSingleNodes(schema);
