...
export function Result(props) {
  ...
  const fieldSchema = useFieldSchema();
  const { dataType, expression, currency, engine = 'math.js' } = useTargetCollectionField() ?? {};
  // console.log('*********************************************');
  // console.log(dataType, expression, engine);
  // console.log('*********************************************');
  ...
  useFormEffects(() => {
    onFormInputChange((form) => {
      ...
      try {
        ...
        v = toDbType(v, dataType, currency, props.ignoreDecimal, props.doNotAddOnly);
      } catch (error) {
        v = null;
      }
