...
const [DragDropManagerProvider, useDragDropManagerContext] = constate(useDragDropManager);
  ...
  onMouseUp((event: React.MouseEvent) => {
    ...
    while (dropElement) {
      ...
      if (dropContext && dropContext.accept === type) {
        if (!dropContext.shallow || (dropContext.shallow && dropIds.length === 0)) {
          // @ts-ignore
...
export function useDrop(options) {
  const { uid: dropId, accept, data, shallow, onDrop, onHover, canDrop = true } = options;
  const dropRef = useRef<HTMLDivElement>();
  const { onMouseEnter, onMouseLeave, onMouseMove, onMouseUp } = useMouseEvents(dropRef);
...
export function useColResizer(options?: any) {
  ...
  onMouseUp((event: React.MouseEvent) => {
    ...
    els.forEach((el: HTMLDivElement) => {
      ...
      const w2 = (100 * (el.clientWidth + gap + gap / els.length)) / parent.clientWidth;
      size.push(w2);
      el.style.width = `${w}%`;
    });
    console.log({ size });
