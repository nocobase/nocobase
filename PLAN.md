...
| 4. `refactor(admin-layout): migrate menu drag to flow dnd` | `87bac9f64a`, `c3530fe9a5` | `done` | `yarn test --client packages/core/client/src/route-switch/antd/admin-layout/__tests__/admin-layout-menu-models.test.ts`; `./node_modules/.bin/eslint packages/core/client/src/route-switch/antd/admin-layout/AdminLayoutMenuModels.tsx packages/core/client/src/route-switch/antd/admin-layout/AdminLayoutShell.tsx packages/core/client/src/route-switch/antd/admin-layout/__tests__/admin-layout-menu-models.test.ts`; `yarn build:client-v2` -> blocked: `rsbuild: command not found` | `manual ok` | 菜单拖拽已切到 v2 DnD：顶层 provider 改为 flow-engine `DndProvider`，菜单节点改为 `Droppable`，拖拽按钮改为 flow-engine `DragHandler`；浏览器回归后补提交流程默认拖拽图标修复。 |
