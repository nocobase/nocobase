...
keywords: "Database utama,MySQL、PostgreSQL、MariaDB、KingbaseES、OceanBase,penyinkronan tabel data"
---
# Database Utama

## Pendahuluan

Database yang dikonfigurasi melalui [Deploy NocoBase](/ai/install-nocobase-app), digunakan untuk menyimpan data tabel sistem NocoBase, dan juga mendukung penyimpanan tabel bisnis pengguna.
...
| KingbaseES | `@nocobase/plugin-data-source-kingbase` | Memerlukan lisensi komersial. Setelah dipasang, plugin diaktifkan secara default. |
| OceanBase | `@nocobase/plugin-data-source-oceanbase` | Memerlukan lisensi komersial. Setelah dipasang, plugin diaktifkan secara default. |

## Mengakses sumber data utama

1. Klik menu sumber data pada fungsi sistem untuk mengakses halaman utama sumber data.
2. Pilih sumber data **Main** dari daftar sumber data, lalu klik tindakan **Konfigurasi** untuk mengakses database utama dan mengelolanya.

![configure_main_datasource](https://static-docs.nocobase.com/configure_main_datasource.png)

## Pengelolaan sumber data utama

Database utama menyediakan fungsi pengelolaan tabel data, termasuk mencari, membuat, mengubah, dan menghapus tabel data, serta menyinkronkan kolom dari tabel data yang sudah ada di database; selain itu, tersedia pula pembuatan, perubahan, dan penghapusan kolom tabel data.
- **Filter**: mencari tabel data yang dikelola oleh database utama NocoBase
- **Buat tabel data**: menambahkan tabel data bisnis
...
Salah satu fitur penting sumber data utama adalah kemampuannya menyinkronkan tabel yang sudah ada di database ke NocoBase untuk dikelola. Artinya:

- **Melindungi investasi yang ada**: jika database Anda sudah memiliki banyak tabel bisnis, Anda tidak perlu membuatnya ulang dan dapat langsung menyinkronkannya untuk digunakan
...
- **Migrasi bertahap**: mendukung migrasi sistem yang ada ke NocoBase secara bertahap, tanpa perlu melakukan rekonstruksi sekaligus

Melalui fungsi «Muat dari database», Anda dapat:
...
- **Tabel turunan**: dapat membuat tabel induk, lalu menurunkan tabel anak dari tabel tersebut. Tabel anak akan mewarisi struktur tabel induk dan juga dapat menentukan kolomnya sendiri.
- **Tabel hierarki**: tabel dengan struktur pohon, saat ini hanya mendukung desain tabel adjacency;
...
### Mendukung pengelolaan kategori tabel data
...
NocoBase mendukung konversi tipe kolom secara fleksibel berdasarkan jenis database yang sama.

**Contoh: Opsi konversi kolom bertipe String**

Saat kolom dalam database bertipe String, kolom tersebut dapat dikonversi di NocoBase menjadi salah satu bentuk berikut:
...
- **Tipe lanjutan**: pengodean otomatis, pemilih tabel data, enkripsi
...
- **Menyesuaikan perubahan bisnis**: seiring perubahan kebutuhan bisnis, cara menampilkan dan berinteraksi dengan kolom dapat disesuaikan dengan cepat
- **Keamanan data**: proses konversi tidak memengaruhi integritas data yang sudah ada

### Sinkronisasi fleksibel pada tingkat kolom
...
1. **Sinkronisasi real-time**: ketika struktur tabel database berubah, kolom baru dapat disinkronkan kapan saja
2. **Sinkronisasi selektif**: dapat memilih kolom yang diperlukan untuk disinkronkan, bukan seluruh kolom
...
- **Mode pengelolaan hibrida**: sebagian kolom dikelola melalui NocoBase, sementara sebagian lainnya dikelola dengan cara tradisional, sehingga dapat dikombinasikan secara fleksibel
...
Untuk informasi lebih lanjut, lihat bab «[Kolom tabel data / Ikhtisar](../data-modeling/collection-fields/index.md)».
