:::tip Pemberitahuan Terjemahan AI
Dokumentasi ini telah diterjemahkan secara otomatis oleh AI.
:::


pkg: "@nocobase/plugin-block-grid-card"
---
# Kartu Grid

## Pendahuluan

Blok Kartu Grid menampilkan informasi ringkasan catatan data dalam format kartu. Blok ini mendukung konfigurasi jumlah kolom untuk ukuran layar yang berbeda, memastikan tampilan yang ramah pengguna di berbagai perangkat.

## Pengaturan Blok

![20251023205037](https://static-docs.nocobase.com/20251023205037.png)

### Cakupan Data

![20251023205425](https://static-docs.nocobase.com/20251023205425.png)

Untuk informasi lebih lanjut, lihat [Mengatur cakupan data](/interface-builder/blocks/block-settings/data-scope)

### Mengatur Jumlah Kolom per Baris

![20251023210137](https://static-docs.nocobase.com/20251023210137.gif)

Mendukung konfigurasi jumlah kolom untuk ukuran layar yang berbeda.

![20251023210303](https://static-docs.nocobase.com/20251023210303.png)

## Mengonfigurasi Bidang

### Bidang dari koleksi ini

![20251023210533](https://static-docs.nocobase.com/20251023210533.png)

### Bidang dari koleksi terkait

![20251023210629](https://static-docs.nocobase.com/20251023210629.png)

Untuk pengaturan bidang blok Kartu Grid, lihat [Bidang Detail](/interface-builder/fields/generic/detail-form-item)

## Mengonfigurasi Aksi

### Aksi Global

![20251023211330](https://static-docs.nocobase.com/20251023211330.png)

- [Filter](/interface-builder/actions/types/filter)
- [Tambah Baru](/interface-builder/actions/types/add-new)
- [Hapus](/interface-builder/actions/types/delete)
- [Segarkan](/interface-builder/actions/types/refresh)
- [Impor](/interface-builder/actions/types/import)
- [Ekspor](/interface-builder/actions/types/export)
- [Cetak Template](/template-print/index)
- [Perbarui Massal](/interface-builder/actions/types/bulk-update)
- [Ekspor Lampiran](/interface-builder/actions/types/export-attachments)
- [Picu alur kerja](/interface-builder/actions/types/trigger-workflow)
- [Karyawan AI](/interface-builder/actions/types/ai-employee)
- [Aksi JS](/interface-builder/actions/types/js-action)

### Aksi Baris

![20251023211054](https://static-docs.nocobase.com/20251023211054.png)

- [Edit](/interface-builder/actions/types/edit)
- [Hapus](/interface-builder/actions/types/delete)
- [Tautkan](/interface-builder/actions/types/link)
- [Pop-up](/interface-builder/actions/types/pop-up)
- [Perbarui Catatan](/interface-builder/actions/types/update-record)
- [Cetak Template](/template-print/index)
- [Picu alur kerja](/interface-builder/actions/types/trigger-workflow)
- [Karyawan AI](/interface-builder/actions/types/ai-employee)
- [Aksi JS](/interface-builder/actions/types/js-action)