...
Sebelum mengembalikan file lama `/storage/uploads/`, Nginx bawaan meminta NocoBase memeriksa login; akses anonim secara default mengembalikan `401`. Jika integrasi lama memerlukan akses anonim, tambahkan `LEGACY_LOCAL_STORAGE_PUBLIC_ACCESS=true` ke `app.environment` lalu restart kontainer. Tetap teruskan path ini ke kontainer dan jangan ekspos direktori upload secara langsung dengan `alias` lain.
