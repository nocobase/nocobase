...
AI Development Plugin cần chạy AI Agent trong thư mục gốc của dự án NocoBase được tạo bằng `nb init`. CLI hỗ trợ hai nguồn là npm và Git — **khuyến nghị sử dụng nguồn Git**, như vậy AI có thể trực tiếp tham chiếu mã nguồn lõi NocoBase, cho hiệu quả phát triển tốt hơn.

Cấu trúc thư mục dự án do `nb init` tạo ra như sau (tức `<app-path>`):
...
Khi mở AI Agent (như Claude Code, Codex, Cursor, v.v.), chuyển thư mục làm việc sang `<app-path>` là có thể bắt đầu phát triển plugin.
...
Sau khi cài đặt, mở AI Agent trong thư mục gốc của dự án (`<app-path>`), bạn có thể trực tiếp dùng ngôn ngữ tự nhiên để nói với AI bạn muốn phát triển plugin gì. Dưới đây là một vài tình huống thực tế, để bạn cảm nhận khả năng của AI Development Plugin.
...
Tại `<app-path>` gửi prompt sau cho AI:
...
Tương tự, tại `<app-path>` gửi prompt sau cho AI:
