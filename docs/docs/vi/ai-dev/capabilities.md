...
- **Chạy AI Agent ở thư mục gốc dự án do `nb init` tạo ra** — khuyến nghị sử dụng nguồn Git để tạo dự án, như vậy AI có thể trực tiếp tham chiếu mã nguồn lõi NocoBase, cho hiệu quả phát triển tốt hơn. Nếu bạn không ở thư mục gốc dự án, cần nói thêm với AI Agent về đường dẫn dự án.
...
Tất cả các prompt mẫu dưới đây đều giả định bạn mở AI Agent tại `<app-path>`.
...
Tại thư mục gốc dự án (`<app-path>`) mở AI Agent, gửi prompt:

```
Hãy giúp tôi tạo một plugin NocoBase, tên plugin là @my-scope/plugin-todo
```

AI sẽ chạy `nb scaffold plugin @my-scope/plugin-todo` và sinh thư mục chuẩn:
