# Gán Giá Trị Trường

## Giới Thiệu

Gán giá trị trường là việc đặt trước giá trị cho một trường trước khi thực hiện một thao tác. Khi cập nhật hoặc tạo mới một bản ghi, dữ liệu này sẽ được tự động điền vào mà không cần nhập thủ công. Điều này hữu ích trong các tình huống như cập nhật trạng thái đơn hàng hàng loạt, hoặc đặt thời gian giao hàng là thời gian hiện tại khi đơn hàng được giao.

![20251029143712](https://static-docs.nocobase.com/20251029143712.png)

![20251029144011](https://static-docs.nocobase.com/20251029144011.png)

![20251029144142](https://static-docs.nocobase.com/20251029144142.png)

## Hướng Dẫn Sử Dụng

### Cấu Hình Trường

Chỉ hỗ trợ cấu hình các trường của bộ sưu tập (collection) thuộc khối hiện tại.

![20251029144652](https://static-docs.nocobase.com/20251029144652.png)

### Hằng Số

![20251029144243](https://static-docs.nocobase.com/20251029144243.png)

### Biến

![20251029144547](https://static-docs.nocobase.com/20251029144547.png)

Để biết thêm thông tin về biến, vui lòng tham khảo [Biến](/interface-builder/variables)