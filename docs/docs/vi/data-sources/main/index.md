...
Cơ sở dữ liệu được cấu hình khi [triển khai NocoBase](/ai/install-nocobase-app), dùng để lưu trữ dữ liệu bảng hệ thống của NocoBase, đồng thời hỗ trợ lưu trữ dữ liệu bảng nghiệp vụ của người dùng.
...
KingbaseES chỉ hỗ trợ chế độ tương thích PostgreSQL, còn OceanBase chỉ hỗ trợ chế độ tương thích MySQL.
...
| KingbaseES | `@nocobase/plugin-data-source-kingbase` | Cần giấy phép thương mại; sau khi cài đặt, plugin được bật mặc định. |
| OceanBase | `@nocobase/plugin-data-source-oceanbase` | Cần giấy phép thương mại; sau khi cài đặt, plugin được bật mặc định. |

## Truy cập nguồn dữ liệu chính

1. Nhấp vào mục menu Nguồn dữ liệu trong các chức năng hệ thống để truy cập trang chủ nguồn dữ liệu.
2. Chọn nguồn dữ liệu **Main** trong danh sách nguồn dữ liệu, nhấp vào thao tác **Cấu hình** để truy cập cơ sở dữ liệu chính và thực hiện quản lý.

![configure_main_datasource](https://static-docs.nocobase.com/configure_main_datasource.png)

## Quản lý nguồn dữ liệu chính

Cơ sở dữ liệu chính cung cấp các chức năng quản lý bảng dữ liệu, cho phép tìm kiếm, tạo, thay đổi và xóa bảng dữ liệu, đồng thời đồng bộ các trường của những bảng dữ liệu đã tồn tại trong cơ sở dữ liệu; cung cấp khả năng tạo, thay đổi và xóa trường bảng dữ liệu.
- **Bộ lọc**: tìm kiếm các bảng dữ liệu được quản lý trong cơ sở dữ liệu chính của NocoBase
- **Tạo bảng dữ liệu**: thêm bảng dữ liệu nghiệp vụ
- **Chỉnh sửa**: thay đổi bảng dữ liệu nghiệp vụ
- **Xóa**: xóa bảng dữ liệu nghiệp vụ
- **Đồng bộ từ cơ sở dữ liệu**: đồng bộ cấu trúc của các bảng dữ liệu đã tồn tại trong cơ sở dữ liệu
- **Cấu hình trường**: tạo, thay đổi và xóa trường bảng dữ liệu
-  **+** : dấu **+** trên tab dùng để quản lý phân loại bảng dữ liệu, tạo, thay đổi và xóa danh mục
...
- **Bảo vệ khoản đầu tư hiện có**: nếu cơ sở dữ liệu của bạn đã có nhiều bảng nghiệp vụ, bạn không cần tạo lại mà có thể đồng bộ và sử dụng trực tiếp
- **Tích hợp linh hoạt**: có thể đưa các bảng được tạo bằng công cụ khác, chẳng hạn như tập lệnh SQL hoặc công cụ quản lý cơ sở dữ liệu, vào NocoBase để quản lý
...
4. Nhập vào NocoBase để quản lý chỉ bằng một cú nhấp
...
- **Bảng cây**: bảng có cấu trúc cây, hiện chỉ hỗ trợ thiết kế theo mô hình bảng kề;
- **Bảng lịch**: dùng để tạo bảng sự kiện liên quan đến lịch;
- **Bảng tệp**: dùng để quản lý việc lưu trữ tệp;
- **Bảng SQL**: không phải là bảng cơ sở dữ liệu thực tế mà là cách nhanh chóng trình bày có cấu trúc các truy vấn SQL;
- **Bảng khung nhìn**: kết nối với các khung nhìn cơ sở dữ liệu hiện có;

### Hỗ trợ quản lý phân loại bảng dữ liệu

![20240322231520](https://static-docs.nocobase.com/20240322231520.png)

### Cung cấp nhiều kiểu trường phong phú
...
NocoBase hỗ trợ chuyển đổi kiểu trường linh hoạt trên cùng một loại cơ sở dữ liệu.

**Các tùy chọn chuyển đổi của trường kiểu String**

Khi trường trong cơ sở dữ liệu có kiểu String, bạn có thể chuyển đổi trường đó trong NocoBase sang bất kỳ dạng nào sau đây:
...
- **Kiểu nội dung đa phương tiện**: Markdown, Markdown (Vditor), văn bản đa dạng thức, tệp đính kèm (URL)
...
- **Không cần thay đổi cấu trúc cơ sở dữ liệu**: kiểu lưu trữ ở tầng cơ sở của trường vẫn giữ nguyên, chỉ thay đổi cách thể hiện trong NocoBase
...
### Đồng bộ linh hoạt ở cấp trường

NocoBase không chỉ có thể đồng bộ toàn bộ bảng mà còn hỗ trợ quản lý đồng bộ chi tiết ở cấp trường:
...
2. **Đồng bộ có chọn lọc**: có thể chỉ đồng bộ các trường cần thiết thay vì toàn bộ trường
...
- **Mô hình quản lý kết hợp**: một số trường được quản lý bằng NocoBase, một số trường được quản lý theo cách truyền thống, cho phép kết hợp linh hoạt

Cơ chế đồng bộ linh hoạt này giúp NocoBase dễ dàng tích hợp vào kiến trúc công nghệ hiện có mà không cần thay đổi phương thức quản lý cơ sở dữ liệu ban đầu, đồng thời vẫn tận dụng được sự tiện lợi của hoạt động phát triển low-code do NocoBase mang lại.
