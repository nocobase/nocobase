...
Route của NocoBase v2 mặc định sẽ thêm tiền tố `/studio`. Ví dụ bạn đã đăng ký `path: '/hello'`, địa chỉ truy cập thực tế là `/studio/hello`:

```ts
this.router.add('hello', {
  path: '/hello', // Truy cập thực tế -> /studio/hello
...
- [Router](../router) — Đăng ký route và tiền tố `/studio`
