...
:::tip{title="AI 번역 알림"}
이 문서는 AI에 의해 번역되었습니다. 정확한 정보는 [영어 버전](/workflow/nodes/javascript)을 참조하세요.
:::
...
:::warning{title="보안 경고"}
비안전 모드에서는 스크립트가 제한된 모듈 화이트리스트를 가진 `vm` 샌드박스에서 실행되지만, Node.js `vm` 모듈은 보안 샌드박스 메커니즘이 아닙니다. 이 모드를 활성화하면 워크플로우 스크립트 편집 권한을 가진 모든 사용자를 신뢰하는 것을 의미합니다. 관리자는 보안 위험을 독립적으로 평가하고 모듈 화이트리스트 및 워크플로우 편집 권한을 엄격하게 제어해야 합니다.
:::
