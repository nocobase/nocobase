...
:::info
`v2.0.30`부터 보안상의 이유로 SQL 문에서 변수를 직접 텍스트 치환하는 방식은 더 이상 지원되지 않습니다. 대신 파라미터화된 쿼리를 사용해야 합니다.
:::

SQL 문에서 프로세스 컨텍스트의 변수를 사용할 수 있지만, `:variableName` 형식의 플레이스홀더를 사용해야 합니다. 예를 들어:
...
위의 SQL 문에서 `:userId`는 플레이스홀더입니다. 플레이스홀더의 치환은 "파라미터 목록"에서 설정해야 합니다. 변수 이름은 플레이스홀더의 이름(예: `userId`)을 사용하고, 변수 값은 변수 선택 도구를 사용하여 프로세스 컨텍스트에서 선택할 수 있습니다.
