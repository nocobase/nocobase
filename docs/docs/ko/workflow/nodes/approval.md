...
:::tip{title="AI 번역 알림"}
이 문서는 AI에 의해 번역되었습니다. 정확한 정보는 [영어 버전](/workflow/nodes/approval)을 참조하세요.
:::
...
워크플로우에서 플러스('+') 버튼을 클릭하여 '승인' 노드를 추가한 다음, 통과 모드 중 하나를 선택하여 승인 노드를 생성합니다.

![승인 노드_생성](https://static-docs.nocobase.com/20251107000938.png)
...
1.  직통 모드: 일반적으로 간단한 프로세스에 사용됩니다. 승인 노드의 통과 여부가 프로세스 종료를 결정하며, 통과하지 못하면 프로세스가 즉시 종료됩니다.
    ...
    ![승인 노드_통과 모드_직통 모드](https://static-docs.nocobase.com/20251107001043.png)

2.  분기 모드: 일반적으로 더 복잡한 데이터 로직에 사용됩니다. 승인 노드에서 어떤 결과가 나오든, 해당 결과 분기 내에서 다른 노드를 계속 실행할 수 있습니다.

    ![승인 노드_통과 모드_분기 모드](https://static-docs.nocobase.com/20251107001234.png)
...
![승인 노드_승인자](https://static-docs.nocobase.com/20251107001433.png)
...
1. 개별 승인(Anyone): 한 명이라도 승인하면 노드가 통과된 것으로 간주하며, 모든 사람이 거부해야 노드가 거부된 것으로 간주합니다.
2. 합의 승인(Countersign): 모든 사람이 승인해야 노드가 통과된 것으로 간주하며, 한 명이라도 거부하면 노드가 거부된 것으로 간주합니다.
3. 투표(Vote): 설정된 비율 이상의 인원이 승인해야 노드가 통과된 것으로 간주하며, 그렇지 않으면 노드가 거부된 것으로 간주합니다.
...
![승인 노드_거부 후 종료](https://static-docs.nocobase.com/20251107001839.png)

### 승인자 인터페이스 설정

승인자 인터페이스 설정은 승인 워크플로우가 이 노드에서 실행될 때 승인자에게 작업 인터페이스를 제공하는 데 사용됩니다. 설정 버튼을 클릭하여 팝업 창을 엽니다.

![승인 노드_인터페이스 설정_팝업](https://static-docs.nocobase.com/20251107001958.png)

설정 팝업 창에서 원본 제출 내용, 승인 정보, 처리 양식 및 사용자 정의 안내 문구 등의 블록을 추가할 수 있습니다.

![승인 노드_인터페이스 설정_블록 추가](https://static-docs.nocobase.com/20251107002604.png)

#### 원본 제출 내용

승인 내용 상세 블록은 시작자가 제출한 데이터 블록으로, 일반 데이터 블록과 유사하게 컬렉션의 필드 컴포넌트를 자유롭게 추가하고 배열하여 승인자가 확인해야 할 내용을 구성할 수 있습니다.

![승인 노드_인터페이스 설정_상세 블록](https://static-docs.nocobase.com/20251107002925.png)

#### 처리 양식

작업 양식 블록에는 이 노드가 지원하는 작업 버튼을 추가할 수 있습니다. 여기에는 '승인', '거부', '반려', '전달(Reassign)' 및 '추가 서명(Add Signer)'이 포함됩니다.

![승인 노드_인터페이스 설정_작업 양식 블록](https://static-docs.nocobase.com/20251107003015.png)
...
![승인 노드_인터페이스 설정_작업 양식_승인 내용 필드 수정](https://static-docs.nocobase.com/20251107003206.png)
...
![승인 노드_인터페이스 설정_작업 양식_처리 설정](https://static-docs.nocobase.com/20251107003314.png)
...
'전달'과 '추가 서명'은 승인 프로세스의 승인자를 동적으로 조정하는 데 사용되는 비결정적인 작업입니다. '전달'은 현재 사용자의 승인 작업을 다른 사용자에게 대신 처리하도록 넘기는 것이고, '추가 서명'은 현재 승인자 앞이나 뒤에 승인자를 추가하여 새로 추가된 승인자가 함께 승인을 계속 진행하도록 하는 것입니다.

'전달' 또는 '추가 서명' 작업 버튼을 활성화한 후, 버튼의 설정 메뉴에서 '지정 인원 범위'를 선택하여 새로운 승인자를 지정할 수 있는 범위를 설정해야 합니다.

![승인 노드_인터페이스 설정_작업 양식_지정 인원 범위](https://static-docs.nocobase.com/20241226232321.png)
...
### '내 승인' 카드 <Badge>2.0+</Badge>

할 일 센터의 '내 승인' 목록에 표시되는 작업 카드를 설정하는 데 사용됩니다.

![20260214141554](https://static-docs.nocobase.com/20260214141554.png)

카드에는 표시하고자 하는 비즈니스 필드(관계 필드 제외)나 승인 관련 정보를 자유롭게 설정할 수 있습니다.

승인이 해당 노드에 진입하면, 할 일 센터 목록에서 사용자 정의된 작업 카드를 확인할 수 있습니다.
