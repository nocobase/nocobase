...
2.  서비스 제공업체의 사전 서명 정보 가져오기
    ...
    > *   name: 파일 이름
    > *   size: 파일 크기 (바이트 단위)
    > *   type: 파일의 MIME 유형입니다. 다음을 참조하세요: [일반적인 MIME 유형](https://developer.mozilla.org/zh-CN/docs/Web/HTTP/MIME_types/Common_types)
...
3.  파일 업로드
    ...
    반환된 `putUrl`을 사용하여 `PUT` 요청을 보내고 파일을 본문으로 업로드합니다.
