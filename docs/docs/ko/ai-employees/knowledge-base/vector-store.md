:::tip{title="AI 번역 알림"}
이 문서는 AI에 의해 번역되었습니다. 정확한 정보는 [영어 버전](/ai-employees/knowledge-base/vector-store)을 참조하세요.
:::
...
- `LLM service`에서 이미 설정된 LLM 서비스를 선택합니다. 자세한 내용은 [LLM 서비스 관리](/ai-employees/features/llm-service)를 참조하세요.
