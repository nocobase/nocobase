...
NocoBase의 AI 직원은 챗봇이나 독립적인 에이전트가 아닙니다. 이들은 비즈니스 시스템에 원활하게 통합되어 비즈니스 컨텍스트를 이해하고 작업을 직접 실행하는 지능적인 동료입니다.

이들은 테이블 옆이나 폼 옆 등 필요한 곳 어디든 나타나므로, 언제든지 쉽게 불러낼 수 있습니다.

![20251102121036-2025-11-02-12-10-38](https://static-docs.nocobase.com/20251102121036-2025-11-02-12-10-38.png)

페이지 오른쪽 하단에 있는 직원 목록에서도 이들을 불러낼 수 있습니다.

![20251102121159-2025-11-02-12-12-01](https://static-docs.nocobase.com/20251102121159-2025-11-02-12-12-01.png)

이들은 데이터를 자동으로 컨텍스트로 가져옵니다. 예를 들어, 테이블 블록 옆의 **Viz**는 테이블의 데이터를 자동으로 가져와 적절한 도구를 호출하여 처리합니다. 따라서 데이터를 챗봇에 복사할 필요가 없습니다.
...
이들은 페이지 구조도 자동으로 컨텍스트로 가져올 수 있습니다. 예를 들어, 폼 블록 옆의 **Dex**는 폼의 필드 구조를 자동으로 가져와 적절한 도구를 호출하여 페이지를 조작합니다. 따라서 챗봇에서 데이터를 다시 복사해올 필요가 없습니다.
...
페이지에서 요소를 직접 선택하여 이들에게 보낼 수도 있으며, 그러면 이들은 해당 데이터와 페이지 구조를 자동으로 추출합니다.
...
각 AI 직원에 대해 현재 위치에서 작업을 미리 설정할 수 있습니다. 그러면 사용자는 클릭 한 번으로 작업을 트리거할 수 있으며, 한 글자도 입력할 필요가 없습니다.
