...
현지화 관리 플러그인은 NocoBase의 현지화 리소스를 관리하고 구현하는 데 사용됩니다. 이 플러그인을 통해 시스템 메뉴, 데이터 테이블, 필드 및 모든 플러그인을 번역하여 특정 지역의 언어와 문화에 맞게 조정할 수 있습니다.
...
:::info{title=참고}
모듈마다 동일한 원문 항목이 있을 수 있으므로, 각각 별도로 번역해야 합니다.
:::
...
번역을 완료한 후에는 변경 사항을 적용하려면 "발행" 버튼을 클릭해야 합니다.
