...
로컬 스토리지나 애플리케이션과 동일 출처 URL을 통해 직접 접근할 수 있는 public 스토리지의 경우, 능동형 콘텐츠 파일이 가져오는 위험에도 주의해야 합니다. `html`, `xhtml`, `svg` 와 같은 파일은 브라우저에서 직접 파싱되고 실행될 수 있습니다. 공격자가 이런 파일을 업로드하고 사용자가 이를 열도록 유도할 수 있다면, 신뢰된 애플리케이션 도메인에서 악성 페이지나 스크립트를 호스팅할 수 있습니다.

일반적으로 관리자에게 다음을 권장합니다.

- 사용자 업로드 파일이 메인 애플리케이션과 동일 출처에서 직접 제공되지 않도록 private 스토리지, 서명된 URL 또는 분리된 파일 도메인을 우선 사용하십시오.
- 업로드에는 엄격한 MIME type 허용 목록을 적용하고, 실제 업무에 필요한 파일 형식만 허용하십시오.
- `text/html`, `application/xhtml+xml`, `image/svg+xml` 과 같은 능동형 콘텐츠 타입 허용은 신중하게 결정하십시오. 시스템이 이러한 파일을 다운로드 방식으로 반환하더라도, 이것만으로 업로드 제한과 출처 격리를 완전히 대체할 수는 없습니다.
- 리버스 프록시, CDN, 오브젝트 스토리지 등 정적 파일 배포 계층 전체에 일관된 보안 설정을 적용하여, 애플리케이션 계층 보호를 우회해 위험한 파일이 인라인으로 반환되지 않도록 하십시오.
- 신뢰할 수 없는 웹 콘텐츠를 호스팅하는 용도로 로컬/public 스토리지를 사용하지 마십시오. 불가피한 경우에는 분리된 도메인을 사용하고, CSP, 다운로드 정책, 접근 제어를 별도로 검토하십시오.

관리자가 위험한 파일 형식 업로드를 명시적으로 허용하는 경우, 피싱, 동일 출처 스크립트 실행, 민감 정보 유출과 같은 위험을 스스로 평가해야 하며, 배포 경로 전반의 Web Server, 게이트웨이, CDN, 스토리지 서비스가 일관된 제한 정책을 적용하도록 해야 합니다.
