...
מסמך זה תורגם על ידי AI. למידע מדויק, אנא עיינו ב[גרסה באנגלית](/file-manager/development/index).
:::
...
לאחר הרישום, תצורת האחסון תופיע במשאב `storages` כמו הסוגים המובנים. ניתן להשתמש בתצורה שמספק `StorageType.defaults()` למילוי אוטומטי של טפסים או לאתחול רשומות ברירת מחדל.
...
עבור קבצים שהועלו, ניתן להציג תצוגות מקדימות שונות בממשק לפי סוג הקובץ. לשדה ה-Attachment של מנהל הקבצים יש תצוגה מקדימה מובנית המבוססת דפדפן (מוטמעת ב-iframe), התומכת ברוב הפורמטים (כמו תמונות, וידאו, אודיו ו-PDF) ישירות בדפדפן. כאשר פורמט הקובץ אינו נתמך בדפדפן או נדרשות אינטראקציות תצוגה מיוחדות, ניתן להרחיב את רכיב התצוגה לפי סוג הקובץ.
...
מאפיין `Previewer` באובייקט התיאור הוא הרכיב שמבצע את התצוגה המקדימה. כאשר הסוג תואם, רכיב זה יוצג בחלון התצוגה המקדימה. ניתן להחזיר כל תצוגת React (כגון iframe, נגן או תרשים).
...
מחזירה את כתובת ה-URL של התמונה הממוזערת ברשימת הקבצים. אם הערך מוחזר ריק, ייעשה שימוש בתמונת placeholder מובנית.
