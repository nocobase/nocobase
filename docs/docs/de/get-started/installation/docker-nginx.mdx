...
- Bevor Nginx eine historische `/storage/uploads/`-Datei per `alias` ausliefert, prüft NocoBase über `auth_request` den Anmeldestatus; anonyme Zugriffe erhalten standardmäßig `401`
...
Wenn ein bestehendes Programm anonymen Zugriff auf `/storage/uploads/` benötigt, füge `LEGACY_LOCAL_STORAGE_PUBLIC_ACCESS=true` zu `app.environment` hinzu und starte den Anwendungscontainer neu. Entferne `auth_request` nicht; der Authentifizierungsendpunkt wertet diese Variable aus. Die Berechtigungen für `/files/` bleiben unverändert.
