...
- `./storage` muss sowohl in `app` als auch in `caddy` gemountet werden, und der Proxy-Container sollte den Mount nur lesend verwenden
...
- Nach außen sollte nur der Port des Caddy-Containers freigegeben werden. Der Dienst `app` braucht innerhalb des Compose-Netzwerks nur `expose: "13000"`
