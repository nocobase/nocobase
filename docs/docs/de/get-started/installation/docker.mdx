...
Bevor das integrierte Nginx eine historische `/storage/uploads/`-Datei ausliefert, prüft NocoBase den Anmeldestatus; anonyme Zugriffe erhalten standardmäßig `401`. Falls eine bestehende Integration anonymen Zugriff benötigt, füge `LEGACY_LOCAL_STORAGE_PUBLIC_ACCESS=true` zu `app.environment` hinzu und starte den Container neu. Der Host-Nginx soll den Pfad weiterhin an den Container weiterleiten und das Upload-Verzeichnis nicht direkt per `alias` freigeben.
