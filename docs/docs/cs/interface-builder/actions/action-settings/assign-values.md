:::tip Upozornění na překlad AI
Tato dokumentace byla automaticky přeložena umělou inteligencí.
:::


# Přiřazení polí

## Úvod

Přiřazení polí umožňuje přednastavit hodnotu pole ještě před spuštěním akce. Při aktualizaci nebo vytváření záznamu se tato data automaticky vyplní, aniž byste je museli zadávat ručně. To se hodí například při hromadné aktualizaci stavů objednávek nebo nastavení aktuálního času doručení, jakmile je objednávka doručena.

![20251029143712](https://static-docs.nocobase.com/20251029143712.png)

![20251029144011](https://static-docs.nocobase.com/20251029144011.png)

![20251029144142](https://static-docs.nocobase.com/20251029144142.png)

## Použití

### Konfigurace pole

Konfigurovat lze pouze pole kolekce aktuálního bloku.

![20251029144652](https://static-docs.nocobase.com/20251029144652.png)

### Konstanta

![20251029144243](https://static-docs.nocobase.com/20251029144243.png)

### Proměnná

![20251029144547](https://static-docs.nocobase.com/20251029144547.png)

Více informací o proměnných naleznete v části [Proměnné](/interface-builder/variables).