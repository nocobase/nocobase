---
pkg: '@nocobase/plugin-users'
---

# Uživatelé

## Správa uživatelů

Tento plugin poskytuje rozhraní pro správu uživatelů v centru konfigurace. Po inicializaci aplikace je automaticky vytvořen neodstranitelný účet superadministrátora. Superadministrátor má oprávnění Root a má přístup ke všem zdrojům.

![](https://static-docs.nocobase.com/44bf40f56b45d4dd96c424fb08082cf6.png)

Kromě toho můžete také přidat různé bloky kolekce Uživatelé pro správu uživatelů, například tabulkové bloky:

![](https://static-docs.nocobase.com/76b5a465f869541a9e8f18a4568a7c9.png)

## Přidání uživatele

![](https://static-docs.nocobase.com/4f8ef9ffc1c17f275b62b462f6385b19.png)

![](https://static-docs.nocobase.com/437828173950bd7c21b40a6243ffe150.png)

## Úprava uživatelského profilu

![](https://static-docs.nocobase.com/d25e06872bd1d48ed8c1139728fa5ff3.png)

![](https://static-docs.nocobase.com/c140bcaab240385b9b5aca32a2ec2801.png)

## Změna uživatelského hesla

![](https://static-docs.nocobase.com/26c24c4cebda3d144dc4e9b728c2ede5.png)

![](https://static-docs.nocobase.com/23a2b2223cb5b387b3699cc6143302e8.png)

## Nastavení

:::info{title=Tip}
Vyžaduje NocoBase v1.3.34-beta nebo novější.
:::

Nastavte, zda povolit úpravu uživatelských profilů a změnu uživatelských hesel (platí pro všechny uživatele).

![20241021212438](https://static-docs.nocobase.com/20241021212438.png)

![20241021212659](https://static-docs.nocobase.com/20241021212659.png)