...
NocoBase v2 routes automatically add a `/studio` prefix. For example, if you registered `path: '/hello'`, the actual URL is `/studio/hello`:

```ts
this.router.add('hello', {
  path: '/hello', // actual URL -> /studio/hello
...
- [Router](../router) — Route registration and the `/studio` prefix
