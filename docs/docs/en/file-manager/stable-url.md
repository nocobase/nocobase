...
This is also a proxy URL. Earlier designs called it a “permanent URL,” but “permanent” only means that the saved URL no longer changes with temporary storage signatures. It does not mean that the file exists forever or is publicly accessible.
...
Logged-in requests use the current app credentials and role. Requests without a login use the anonymous role. After permission is granted, NocoBase responds with `302` and redirects to the local or object-storage URL.
...
- Reverse proxies must forward the `/files/` route under `APP_PUBLIC_PATH` to NocoBase
