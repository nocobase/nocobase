...
* `list`: file list
* `originalNode`: the built-in image preview node, which can be reused directly

`originalNode` is commonly used to reuse the built-in preview node for image types and avoid re-implementing it.
