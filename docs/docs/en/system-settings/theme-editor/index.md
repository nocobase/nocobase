...
> Note: The current theme feature is implemented based on antd 5.x version. It is recommended to read about the [Customizing Theme](https://ant.design/docs/react/customize-theme) concept before proceeding with this document.
...
Firstly, update NocoBase to the latest version (v0.11.1 or above). Then, search for the `Theme Editor` card in the `Plugin Manager`. Click on the Enable button at the bottom right of the card and wait for the page to refresh.
