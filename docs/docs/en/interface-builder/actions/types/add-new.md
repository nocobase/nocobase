# Add
## Introduction

The Add action adds a record to the current block's collection. By configuring a form block, you can enter new data records for the current block's collection.


![20251029152258](https://static-docs.nocobase.com/20251029152258.png)



![20251029152324](https://static-docs.nocobase.com/20251029152324.png)



## Action Configuration


![20251029152442](https://static-docs.nocobase.com/20251029152442.png)


- [Edit button](/interface-builder/actions/action-settings/edit-button)
- [Linkage rule](/interface-builder/actions/action-settings/linkage-rule)
- [Edit popup](/interface-builder/actions/action-settings/edit-popup)