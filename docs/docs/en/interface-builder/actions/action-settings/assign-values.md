# Field Assignment

## Introduction

Field assignment refers to presetting the value of a field before executing an action. When updating or creating a record, this data is automatically filled in without manual entry, such as in scenarios like bulk updating order statuses or setting the delivery time to the current time when an order is delivered.


![20251029143712](https://static-docs.nocobase.com/20251029143712.png)



![20251029144011](https://static-docs.nocobase.com/20251029144011.png)



![20251029144142](https://static-docs.nocobase.com/20251029144142.png)


## Usage

### Configure Field

Only fields of the current block's collection can be configured.


![20251029144652](https://static-docs.nocobase.com/20251029144652.png)


### Constant


![20251029144243](https://static-docs.nocobase.com/20251029144243.png)


### Variable


![20251029144547](https://static-docs.nocobase.com/20251029144547.png)



For more information about variables, see [Variables](/interface-builder/variables)