...
A constant table for workflow execution statuses, used to identify the current status of the corresponding execution.
...
A constant table for workflow node job statuses, used to identify the current status of the corresponding node job. The status generated by the node also affects the status of the entire execution.
