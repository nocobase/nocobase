...
- `./storage` must be mounted into both the `app` and `nginx` containers, and the proxy container should use a read-only mount
...
- Expose only the Nginx container port to the host. The `app` service only needs `expose: "13000"` inside the Compose network
