...
## Overview
The Reference block lets you reuse an existing block (list, form, chart, etc.) by pointing to its block UID. It shows the target block in your current page without reconfiguring it.

## Activate the plugin
This plugin is built‑in but disabled by default.
Open "Plugin manager" → find "Block: Reference" → click "Enable".

![Enable Reference block in Plugin Manager](https://static-docs.nocobase.com/block-ref-enable-20251102.png)

## Add the block
1) Add block -> Other blocks -> select "Reference block".  
2) In Reference settings, configure:
   - `Block UID`: the UID of the target block
   - `Reference mode`: choose `Reference` or `Copy` (see below)

![Reference block add and configure demo](https://static-docs.nocobase.com/20251102211459_rec_.gif)

### How to get Block UID
- Open the target block’s settings menu and click `Copy UID` to copy its UID.

![Copy UID from block settings](https://static-docs.nocobase.com/block-ref-copy-uid-20251102.png)
...
![Error state when target block is invalid](https://static-docs.nocobase.com/block-ref-error-20251102.png)
