---
title: 'nb env proxy'
description: 'nb env proxy topic reference: view the Nginx and Caddy proxy subcommands.'
keywords: 'nb env proxy,NocoBase CLI,nginx,caddy,reverse proxy,proxy configuration'
---

# nb env proxy

In NocoBase CLI, `nb env proxy` is now a topic. It does not generate a config by itself. You mainly use it to discover the Nginx and Caddy provider subcommands.
...
- `nb env proxy nginx` and `nb env proxy caddy` are the commands that actually generate configs
