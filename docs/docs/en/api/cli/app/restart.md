...
description: "nb app restart command reference: restart the NocoBase app for a selected env; local envs automatically complete install or upgrade preparation during restart, and Docker envs recreate the app container from saved env config when needed."
keywords: "nb app restart,NocoBase CLI,restart app,Docker"
---

# nb app restart

Stop and then start the NocoBase app for a selected env. Local envs reuse the `nb app stop` and `nb app start` flow and, by default, automatically complete install or upgrade preparation before starting again; Docker envs remove the current container first, then recreate the app container from saved env config.
...
By default, local envs automatically complete any required install or upgrade preparation before starting again. Whenever the CLI needs to wait for readiness, it checks `__health_check`: it prints one waiting line first, then one progress line every 10 seconds until the app becomes available or times out. If you pass `--no-daemon` for a local env, the app runs in the foreground, so the CLI does not keep waiting for the readiness check after startup.
