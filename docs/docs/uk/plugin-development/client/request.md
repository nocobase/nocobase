...
NocoBase пропонує `APIClient`, побудований на базі [Axios](https://axios-http.com/), який дозволяє надсилати HTTP-запити з будь-якого місця, де доступний `Context`.
...
`ctx.api.request()` — це найпоширеніший метод для надсилання запитів. Його параметри та значення, що повертаються, повністю відповідають [axios.request()](https://axios-http.com/docs/req_config).
...
Докладніше про доступні конфігурації дивіться в [Axios Default Config](https://axios-http.com/docs/config_defaults).
...
У реальній розробці ви можете використовувати хук `useRequest` від [ahooks](https://ahooks.js.org/hooks/use-request/index), щоб зручніше керувати життєвим циклом та станом запитів.
