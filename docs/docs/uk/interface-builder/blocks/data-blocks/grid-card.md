pkg: "@nocobase/plugin-block-grid-card"
---
# Сіткові картки

## Вступ

Блок сіткових карток відображає зведену інформацію про записи даних у форматі карток. Він дозволяє налаштовувати кількість стовпців для різних розмірів екрана, щоб забезпечити зручне відображення на різних пристроях.

## Налаштування блоку

![20251023205037](https://static-docs.nocobase.com/20251023205037.png)

### Область даних

![20251023205425](https://static-docs.nocobase.com/20251023205425.png)

Більше інформації дивіться в розділі [Налаштування області даних](/interface-builder/blocks/block-settings/data-scope)

### Налаштування кількості стовпців в одному рядку

![20251023210137](https://static-docs.nocobase.com/20251023210137.gif)

Підтримує налаштування кількості стовпців для різних розмірів екрана.

![20251023210303](https://static-docs.nocobase.com/20251023210303.png)

## Налаштування полів

### Поля з цієї колекції

![20251023210533](https://static-docs.nocobase.com/20251023210533.png)

### Поля з пов'язаних колекцій

![20251023210629](https://static-docs.nocobase.com/20251023210629.png)

Щодо налаштувань полів блоку сіткових карток, дивіться [Поле деталізації](/interface-builder/fields/generic/detail-form-item)

## Налаштування дій

### Глобальні дії

![20251023211330](https://static-docs.nocobase.com/20251023211330.png)

- [Фільтрувати](/interface-builder/actions/types/filter)
- [Додати новий](/interface-builder/actions/types/add-new)
- [Видалити](/interface-builder/actions/types/delete)
- [Оновити](/interface-builder/actions/types/refresh)
- [Імпорт](/interface-builder/actions/types/import)
- [Експорт](/interface-builder/actions/types/export)
- [Друк за шаблоном](/template-print/index)
- [Масове оновлення](/interface-builder/actions/types/bulk-update)
- [Експорт вкладень](/interface-builder/actions/types/export-attachments)
- [Запустити робочий процес](/interface-builder/actions/types/trigger-workflow)
- [AI-співробітник](/interface-builder/actions/types/ai-employee)
- [JS Дія](/interface-builder/actions/types/js-action)

### Дії для рядка

![20251023211054](https://static-docs.nocobase.com/20251023211054.png)

- [Редагувати](/interface-builder/actions/types/edit)
- [Видалити](/interface-builder/actions/types/delete)
- [Посилання](/interface-builder/actions/types/link)
- [Спливаюче вікно](/interface-builder/actions/types/pop-up)
- [Оновити запис](/interface-builder/actions/types/update-record)
- [Друк за шаблоном](/template-print/index)
- [Запустити робочий процес](/interface-builder/actions/types/trigger-workflow)
- [AI-співробітник](/interface-builder/actions/types/ai-employee)
- [JS Дія](/interface-builder/actions/types/js-action)