# Alan Ataması

## Giriş

Alan ataması, bir işlem yürütülmeden önce bir alanın değerini önceden belirleme işlemidir. Bu sayede, kayıtları güncellerken veya oluştururken veriler manuel giriş gerektirmeden otomatik olarak doldurulur. Örneğin, sipariş durumlarını toplu olarak güncellemek veya bir sipariş teslim edildiğinde teslimat zamanını mevcut zamana ayarlamak gibi durumlarda bu özellikten faydalanabilirsiniz.

![20251029143712](https://static-docs.nocobase.com/20251029143712.png)

![20251029144011](https://static-docs.nocobase.com/20251029144011.png)

![20251029144142](https://static-docs.nocobase.com/20251029144142.png)

## Kullanım

### Alan Yapılandırma

Yalnızca mevcut bloğun koleksiyonundaki alanlar yapılandırılabilir.

![20251029144652](https://static-docs.nocobase.com/20251029144652.png)

### Sabit

![20251029144243](https://static-docs.nocobase.com/20251029144243.png)

### Değişken

![20251029144547](https://static-docs.nocobase.com/20251029144547.png)

Değişkenler hakkında daha fazla bilgi için [Değişkenler](/interface-builder/variables) bölümüne bakabilirsiniz.