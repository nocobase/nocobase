# Açıklamayı Düzenle

## Giriş

Alan açıklaması, alan bileşeninin altında açık gri ve italik olarak gösterilen kısa bir metindir. Bu metin sayesinde kullanıcılar, alanın amacını ve giriş gereksinimlerini daha iyi anlayabilirler.

![20240411114048](https://static-docs.nocobase.com/20240411114048.png)

![20240423111043](https://static-docs.nocobase.com/20240423111043.png)

![20240411114433](https://static-docs.nocobase.com/20240411114433.png)