...
- アップロードには厳格な MIME type の許可リストを適用し、業務上本当に必要なファイルタイプだけを許可してください。
- `text/html`、`application/xhtml+xml`、`image/svg+xml` などのアクティブコンテンツ型を許可する場合は慎重に判断してください。システムがこれらをダウンロードとして返すようにしていても、それだけでアップロード制限やオリジン分離の代替にはなりません。
- リバースプロキシ、CDN、オブジェクトストレージなど、静的ファイル配信に関わる各レイヤーで一貫したセキュリティ設定を行い、アプリケーション層の保護を迂回して危険なファイルがインライン返却されないようにしてください。
- 信頼できない Web コンテンツのホスティングにローカル/public ストレージを使用しないでください。やむを得ず必要な場合は、分離されたドメインを使用し、CSP、ダウンロード挙動、アクセス制御を個別に評価してください。

管理者が危険なファイルタイプのアップロードを明示的に許可する場合は、フィッシング、同一オリジンでのスクリプト実行、機密情報漏えいなどのリスクを自ら評価し、Web Server、ゲートウェイ、CDN、ストレージサービスが配備経路全体で一貫した制限を適用していることを確認してください。
