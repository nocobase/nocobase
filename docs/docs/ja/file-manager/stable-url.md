...
以前は「永久 URL」と呼ばれていましたが、保存された URL が一時的なストレージ署名に左右されないという意味です。ファイルが永久に存在することや、公開アクセスできることを意味しません。
...
- リバースプロキシは `APP_PUBLIC_PATH` 配下の `/files/` を NocoBase に転送する必要があります
