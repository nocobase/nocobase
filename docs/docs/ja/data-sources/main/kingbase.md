...
KingbaseES のメインデータソースは PostgreSQL モードと互換性があるため、日常的な管理方法については[メインデータソース PostgreSQL](./postgresql.md)を参照してください。
...
3. データベースにすでに存在するテーブルを接続する場合は、メインデータベースの管理ページで「データベースから同期」を使用できます。
