---
title: '外部 NocoBase'
description: '別の NocoBase アプリを外部データソースとして現在のアプリに接続し、設定方法、利用可能な機能、およびワークフローの利用上の制限について説明します。'
...
プラグインを有効化した後、「データソース管理」で外部 NocoBase データソースを追加し、リモートアプリのアクセス情報を入力します。
...
| タイムアウト | リモートアプリへのリクエストのタイムアウト時間                                                                     |
...
外部 NocoBase データソースは、フロントエンドでボタンの表示状態を細かく制御するための権限メタデータを返しません。そのため、一部のボタンはメインデータソースのように権限に応じて自動的に非表示にならない場合があります。ボタンの表示・非表示にかかわらず、送信操作時には現在のアプリのサーバー側で権限チェックが行われ、権限のない操作は拒否されます。
...
| ---------------- | -------- | -------------- | -------------------------------------------------------------------------------------------- |
| リクエスト前イベント       | トリガー     | グローバルモードのみトリガー | 現在のアプリではグローバルモードでトリガーされ、ローカルモードでは現在のアプリのボタンへのバインドに従ってトリガーされます。リモートアプリではプロキシされたリクエストを受信した後、グローバルモードでのみトリガーされます |
| リクエスト後イベント       | トリガー     | グローバルモードのみトリガー | 現在のアプリではグローバルモードでトリガーされ、ローカルモードでは現在のアプリのボタンへのバインドに従ってトリガーされます。リモートアプリではプロキシされたリクエストを受信した後、グローバルモードでのみトリガーされます |
| カスタムアクションイベント   | トリガー     | トリガーされない         | 現在のアプリでバインドされた「ワークフローをトリガー」ボタンはローカルフローをトリガーします。プロキシされた CRUD リクエストは、リモートのカスタムアクションイベントをトリガーしません     |
...
| ノード     | 利用可否 | 説明                              |
| -------- | -------- | --------------------------------- |
| レコードの取得 | 利用可能     | リモートアプリのレコードを取得します              |
| レコードの作成 | 利用可能     | リモートアプリにレコードを作成します                |
| レコードの更新 | 利用可能     | リモートアプリのレコードを更新します              |
| レコードの削除 | 利用可能     | リモートアプリのレコードを削除します              |
| SQL ノード | 利用不可   | ワークフローの SQL ノードはデータベースデータソースのみをサポートします |
...
現在のアプリまたはリモートアプリでローカルファイルストレージを使用している場合は、Origin に対応するアプリの公開アクセスアドレスが入力されているか確認してください。Origin に API アドレスを指定しないでください。

### 現在のアプリでは権限があるのに操作に失敗する

リモートアプリの API key 権限を確認してください。外部 NocoBase データソースは、現在のアプリとリモートアプリの両方の権限の影響を受けます。

### リモートサービスの異常後にデータテーブルを使用できない

リモートアプリで 502 エラー、再起動、または一時的な利用不能が発生すると、現在のアプリでリモートデータテーブルのメタデータを一時的に読み取れなくなる場合があります。リモートサービスが復旧すると、現在のアプリは次回そのデータソースのデータテーブルにアクセスする際に、メタデータを自動的に再読み込みします。

### 現在のアプリでフィールドを設定できないのはなぜですか

外部 NocoBase データソースでは、リモートアプリのデータテーブル構造とフィールド設定を使用します。フィールドはリモートアプリで調整してから、現在のアプリに戻ってデータテーブルを再読み込みしてください。