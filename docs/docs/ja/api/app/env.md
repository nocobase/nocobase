...
サーバーが開始する外向き HTTP リクエストの許可先ホワイトリストです。ワークフローの HTTP リクエストノード、カスタムリクエスト、AI サービスなどのサーバー側リクエストに適用されます。

この変数が設定されていない場合、既存のデプロイとの互換性を維持するため、NocoBase は引き続き `http` / `https` リクエストを許可します。ただし、宛先が loopback、private、link-local、metadata アドレスの場合、またはドメインがそれらのアドレスに解決される場合、サーバーログに warning が出力されます。将来のバージョンでは、デフォルトの動作を段階的に厳しくする可能性があります。内部サービスにアクセスする必要があるデプロイでは、事前に明示的なホワイトリストを設定してください。

この変数を設定した場合、最初のリクエストと各リダイレクト先がホワイトリストのエントリに一致する必要があります。リダイレクト先が一致しないホストの場合、NocoBase はリダイレクトされたリクエストを送信する前に停止します。

サポートされるエントリ：

- 正確な IPv4 アドレス（例：`192.168.1.10`）
- IPv4 CIDR 範囲（例：`10.0.0.0/8`）
- 正確な IPv6 アドレス（例：`::1`）
- IPv6 CIDR 範囲（例：`fc00::/7`）
- 正確なドメイン（例：`api.example.com`）
- 単一レベルのワイルドカードサブドメイン（例：`*.example.com`）
