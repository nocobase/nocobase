...
JavaScript スクリプトノードは、実行待ちのスクリプトをタスクキューに追加し、個別の Worker スレッドで実行します。環境変数 `WORKFLOW_SCRIPT_WORKER_CONCURRENCY` を使用すると、NocoBase アプリケーションの各インスタンスが同時に実行できる JavaScript スクリプト数を制御できます。
...
- 未設定または無効な値の場合、デフォルトの同時実行数は `1`
- 正の整数を設定すると、同時に実行できる Worker スレッドの最大数になります
- `0` を設定すると同時実行数が制限されず、キュー内のすべてのタスクを同時に実行できます

同時実行数の上限に達すると、新しいタスクは Worker が利用可能になるまでキューで待機します。ほとんどのケースではデフォルト設定で十分です。同時実行数を増やすと CPU とメモリの使用量も増えるため、サーバーリソースとスクリプトの負荷に合わせて段階的に調整してください。本番環境では `0` に設定しないことを推奨します。
