...
ユーザー認証プラグインをインストールすると、ユーザーのユーザー名とメールアドレスに基づいた`パスワード`認証方式が初期化されます。
...
異なるタイプの認証器を追加することで、システムに適切な認証方式を追加できます。

既存のプラグインが提供する認証タイプに加えて、開発者は独自のユーザー認証タイプを拡張することも可能です。[開発者ガイド](./dev/index.md)を参照してください。
