...
AI プラグイン開発では、`nb init` で作成した NocoBase プロジェクトのルートディレクトリで AI Agent を実行する必要があります。CLI は npm と Git の 2 つのソースをサポートしています。**Git ソースの使用を推奨します**。これにより AI が NocoBase のコアソースコードを直接参照でき、より良い開発結果が得られます。
...
AI Agent（Claude Code、Codex、Cursor など）を開く際に、作業ディレクトリを `<app-path>` に切り替えれば、プラグイン開発を始められます。
...
インストールが完了したら、プロジェクトルートディレクトリ（`<app-path>`）で AI Agent を開き、自然言語で AI に開発したいプラグインを伝えるだけです。以下にいくつかの実際のユースケースを紹介します。AI プラグイン開発の実力を体験してください。

### 一言でウォーターマークプラグインを開発

一つのプロンプトで、AI がフロントエンドのレンダリングロジック、改ざん検知、バックエンドの設定保存 API、プラグイン設定ページを含む完全なウォーターマークプラグインを生成します。

`<app-path>` で以下のプロンプトを AI に送信してください：
...
同様に `<app-path>` で以下のプロンプトを AI に送信してください：
