...
組み込み Nginx が既存の `/storage/uploads/` ファイルを返す前に、NocoBase がログイン状態を確認します。匿名アクセスはデフォルトで `401` になります。既存の連携で匿名アクセスが必要な場合は、`app.environment` に `LEGACY_LOCAL_STORAGE_PUBLIC_ACCESS=true` を追加してコンテナを再起動してください。このパスは引き続きコンテナへ転送し、別の `alias` でアップロードディレクトリを直接公開しないでください。
