...
- Nginx が `alias` で既存の `/storage/uploads/` ファイルを返す前に、`auth_request` で NocoBase がログイン状態を確認します。匿名アクセスはデフォルトで `401` になります
...
既存プログラムが `/storage/uploads/` への匿名アクセスを必要とする場合は、`app.environment` に `LEGACY_LOCAL_STORAGE_PUBLIC_ACCESS=true` を追加してコンテナを再起動してください。`auth_request` は削除しないでください。認証 endpoint がこの変数を評価します。`/files/` の権限は変わりません。
...
自動生成された設定を使わずに Nginx を自分で管理する場合は、`/files/` と `APP_PUBLIC_PATH` 配下の対応するルートを、SPA フォールバックルールより前で NocoBase に転送してください。`alias` で `/storage/uploads/` を配信する場合は、ログインチェックを迂回しないよう `auth_request` も設定してください。完全な例は [Nginx リバースプロキシ](../../nocobase-cli/production/reverse-proxy/nginx.md)を参照してください。
