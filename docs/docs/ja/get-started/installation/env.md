...
既存のローカルファイル URL `/storage/uploads/` への匿名アクセスを許可するかを制御します。デフォルトは `false` で、ログイン済みユーザーのみがアクセスできます。

既存の連携がこれらの URL の公開アクセスに依存している場合は、互換モードを明示的に有効にします。

```bash
LEGACY_LOCAL_STORAGE_PUBLIC_ACCESS=true
```

変更後はアプリケーションを再起動してください。この変数は既存の `/storage/uploads/` URL のみに影響し、`/files/` のファイルレコード単位の権限は変更しません。公開アクセスによりアップロード済みファイルが露出する可能性があるため、公開可能であることを確認してから有効にしてください。
