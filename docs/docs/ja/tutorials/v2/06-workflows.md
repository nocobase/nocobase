...
| [**操作後イベント**](../../workflow/triggers/post-action.md) | ユーザーが画面で操作を実行した後にトリガー | フォーム送信後の通知送信、操作ログの記録 |
...
- **[データテーブル](/data-sources/data-modeling/collection)**：メインデータソース / 「チケット」を選択
- **トリガータイミング**：「データの追加または更新後」を選択
- **変更された[フィールド](/data-sources/data-modeling/collection-fields)**：「担当者（Assignee）」にチェック——担当者フィールドが変更されたときだけトリガーし、他のフィールドの変更による不要な通知を防ぎます（データ追加時はすべてのフィールドが「変更あり」と見なされるため、新規チケット作成時もトリガーされます）
