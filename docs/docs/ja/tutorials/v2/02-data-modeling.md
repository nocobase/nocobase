...
この章では、チケットと分類の 2 つの[データテーブル](/data-sources/main/collection)を作成し、[フィールドタイプ](/data-sources/field)（単行テキスト、ドロップダウン選択、[多対一](/data-sources/field/associations/m2o)リレーションなど）を設定して、テーブル間のリレーションを構築します。データモデルはシステムの基盤です。どのデータを保存するか、データ同士がどう関係しているかを先に整理しておけば、後のインターフェース構築や権限設定がスムーズに進みます。
...
各チケットには、問題を簡潔にまとめるタイトルが必要です。**「フィールドを追加」** → **[「単行テキスト」](/data-sources/field/basic/input)** を選択します。
...
> なぜドロップダウン単一選択を使うのか？ステータスは固定の選択肢であり、[ドロップダウン](/data-sources/field/choices/select)を使うことでユーザーが自由入力するのを防ぎ、データの一貫性を保つことができます。
...
インターフェースに表示されるのは名前（「ネットワーク問題」「田中さん」）ですが、裏では ID によるリレーションで紐づいています。**複数のチケットが同じ分類や同じユーザーを指すことができます**——このリレーションを[**多対一**](/data-sources/field/associations/m2o)と呼びます。
...
- [データテーブルフィールド](/data-sources/field) — すべてのフィールドタイプの詳細
- [多対一リレーション](/data-sources/field/associations/m2o) — リレーション設定の説明
