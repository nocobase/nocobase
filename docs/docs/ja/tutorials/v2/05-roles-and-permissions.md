...
この章では、システムに「入退室管理」を追加します。[ロール](/users-permissions/acl/role)を作成し、[メニュー権限](/users-permissions/acl/permissions)と[データ範囲](/users-permissions/acl/permissions)を設定して、**異なるユーザーに異なるメニューを表示し、異なるデータを操作させる**ことを実現します。

## 5.1 [ロール](/users-permissions/acl/role)（Role）を理解する

NocoBase では、**ロールとは[権限](/users-permissions/acl/role)のセット**です。各ユーザーに個別に権限を設定するのではなく、まずいくつかのロールを定義し、ユーザーを対応するロールに割り当てます。
...
重要な概念：**[データ範囲](/users-permissions/acl/permissions)（Data Scope）**。

ロールの権限設定で、**[データテーブル](/data-sources/data-modeling/collection)操作権限** タブに切り替えます。「チケット」テーブルを見つけてクリックし、個別に設定します。
...
1. 「チケット」テーブルの **閲覧** 権限を見つけます
2. データ範囲を → **自分のデータ** に選択します
3. ただし注意点があります。NocoBase の「自分のデータ」はデフォルトで作成者でフィルタリングされます。「担当者」でフィルタリングしたい場合は、グローバルの[操作権限](/users-permissions/acl/permissions)でさらに調整するか、フロントエンドページで**データ[ブロック](/interface-builder/blocks)のフィルタ条件**を組み合わせて実現できます。
...
- [ロールと権限](/users-permissions/acl/role) — ロール設定の説明
- [データ範囲](/users-permissions/acl/permissions) — データレベルの権限制御
