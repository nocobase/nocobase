:::tip{title="AI翻訳通知"}
このドキュメントはAIによって翻訳されました。正確な情報については[英語版](/ai-employees/knowledge-base/vector-store)をご参照ください。
:::
...
- `LLM service` で、すでに設定済みの LLM サービスを選択します。詳細は[LLM サービス管理](/ai-employees/features/llm-service)をご参照ください。
