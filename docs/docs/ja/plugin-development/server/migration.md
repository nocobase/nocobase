...
Migrationファイルは、プラグインディレクトリ内の `src/server/migrations/*.ts` に配置する必要があります。NocoBase CLIは、`nb scaffold migration` コマンドを提供しており、このコマンドを使ってMigrationファイルを素早く生成できます。
...
| パラメーター | 説明 |
|------|----------|
| `<name>` | migration 名称（必須） |
| `--pkg <pkg>` | プラグインのパッケージ名を指定します（必須） |
| `--on <timing>`  | 実行タイミングを指定します（`beforeLoad`、`afterSync`、`afterLoad` から選択可能） |
