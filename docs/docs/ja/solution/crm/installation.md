...
- CRM システムの対応するファイルをダウンロード済みであること：
  - **バックアップファイル**：[nocobase_crm_v2_backup_260523.nbdata](https://static-docs.nocobase.com/nocobase_crm_v2_backup_260523.nbdata) - 方法 1 に適用
  - **SQL ファイル**：[nocobase_crm_v2_sql_260523.zip](https://static-docs.nocobase.com/nocobase_crm_v2_sql_260523.zip) - 方法 2 に適用
...
* **オプション A：サーバーのコマンドライン経由（Docker を例に）**
  Docker で NocoBase とデータベースをインストールしている場合、`.sql` ファイルをサーバーにアップロードし、`docker exec` コマンドを使用してインポートを実行できます。PostgreSQL コンテナ名が `my-nocobase-db`、ファイル名が `nocobase_crm_v2_sql_260523.sql` であると仮定します：
