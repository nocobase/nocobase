...
- **Запускайте AI Agent в корневом каталоге проекта, созданного через `nb init`** — рекомендуется использовать Git-источник для создания проекта, так AI сможет напрямую ссылаться на исходный код ядра NocoBase, что даёт лучшие результаты. Если Вы не находитесь в корне проекта, нужно дополнительно сообщить AI Agent путь к проекту.

Структура каталогов проекта, созданного через `nb init` (то есть `<app-path>`):
...
├── plugins/              # Исходный код Ваших плагинов (nb scaffold plugin генерирует сюда)
└── .env                  # Файл переменных окружения приложения
```

Все приведённые ниже примеры промптов предполагают, что Вы запускаете AI Agent в каталоге `<app-path>`.
...
В корневом каталоге проекта (`<app-path>`) откройте AI Agent и отправьте:

```
Помогите мне создать плагин NocoBase, имя плагина: @my-scope/plugin-todo
```

AI выполнит `nb scaffold plugin @my-scope/plugin-todo` и сгенерирует стандартный каталог:
