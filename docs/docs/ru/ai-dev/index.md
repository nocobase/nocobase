...
- NocoBase сейчас переходит с `client` (v1) на `client-v2`, и `client-v2` пока находится в разработке. Клиентский код, сгенерированный AI-разработкой, основан на `client-v2` и работает только по пути `/studio/`. Это для предварительного ознакомления, не рекомендуется использовать в продакшене.
