...
1. Создали проект NocoBase через `nb init --ui` (рекомендуется Git-источник)
2. Открыли редактор с поддержкой AI Agent (например, Claude Code, Codex, Cursor и т.д.), рабочий каталог установлен на корень проекта (`<app-path>`)

Структура каталогов проекта, созданного через `nb init`:

```bash
<app-path>/
├── .nb/                  # Метаданные CLI для текущего окружения
├── source/               # Исходный код приложения (ядро NocoBase + встроенные плагины)
├── storage/              # Данные времени выполнения
...
В корневом каталоге Вашего проекта NocoBase (созданного через `nb init`, `<app-path>`) отправьте AI следующий промпт:
...
AI создаёт стандартную структуру каталогов плагина в `plugins/@my-project/plugin-watermark/`.

После создания scaffold запустите режим разработки в корневом каталоге проекта (`<app-path>`), чтобы изменения кода подхватывались горячей перезагрузкой:
