...
Убедитесь, что Вы выполняли `yarn pm create`, а не создавали каталог вручную. `yarn pm create`, помимо генерации файлов, обновляет `tsconfig.paths.json` и выполняет `yarn postinstall` — последний связывает плагин с `node_modules` и заново генерирует индекс клиентских плагинов, благодаря чему приложение может его обнаружить. При создании каталога вручную этот шаг пропускается — выполните `yarn nocobase upgrade`, чтобы повторно просканировать и зарегистрировать плагин в таблице `applicationPlugins`.
