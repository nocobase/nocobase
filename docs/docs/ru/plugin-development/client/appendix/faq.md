...
Убедитесь, что Вы выполняли `nb scaffold plugin`, а не создавали каталог вручную. `nb scaffold plugin`, помимо генерации файлов, регистрирует плагин в таблице `applicationPlugins` базы данных. Если каталог был создан вручную, можно выполнить `nb app upgrade` для повторного сканирования.

### После включения плагина страница не изменилась

Проверьте по порядку:

1. Убедитесь, что Вы выполнили `nb plugin enable <pluginName>`
...
| Добавление или изменение collection в `src/server/collections/` | Выполнить `nb app upgrade` |
...
### `nb source build --tar` ошибка "no paths specified to add to archive"

При выполнении `nb source build <pluginName> --tar` появляется ошибка:

```bash
TypeError: no paths specified to add to archive
```

Однако отдельное выполнение `nb source build <pluginName>` (без `--tar`) проходит нормально.
...
**Почему локально нет проблем:** при локальной разработке плагин находится в каталоге `plugins/`, у него нет собственного `node_modules/`, зависимости разрешаются в уже загруженные версии в корне проекта, конфликта не возникает.
