...
Определяет, разрешён ли анонимный доступ к устаревшим локальным URL `/storage/uploads/`. По умолчанию значение `false`, поэтому доступ есть только у авторизованных пользователей.

Если существующая интеграция зависит от публичного доступа по этим URL, явно включите режим совместимости:

```bash
LEGACY_LOCAL_STORAGE_PUBLIC_ACCESS=true
```

После изменения перезапустите приложение. Переменная влияет только на устаревшие URL `/storage/uploads/` и не изменяет разрешения на уровне записи для `/files/`. Публичный доступ может раскрыть загруженные файлы; включайте его только после проверки возможности их публикации.
