...
Проброс `13000:80` нужен только для удобного локального тестирования, чтобы не занимать стандартные порты хоста напрямую. В production обычно не оставляют `13000:80`; вместо этого внешний контейнер Nginx напрямую публикует порты хоста `80` и `443`.
```yml
networks:
  nocobase:
    driver: bridge
...
- Перед возвратом устаревшего файла `/storage/uploads/` через `alias` NocoBase проверяет вход с помощью `auth_request`; анонимный доступ по умолчанию получает `401`
...
Если существующей программе нужен анонимный доступ к `/storage/uploads/`, добавьте `LEGACY_LOCAL_STORAGE_PUBLIC_ACCESS=true` в `app.environment` и перезапустите контейнер. Не удаляйте `auth_request`: endpoint аутентификации оценивает эту переменную. Разрешения `/files/` не изменяются.
