...
Перед возвратом устаревшего файла `/storage/uploads/` встроенный Nginx просит NocoBase проверить вход; анонимный доступ по умолчанию получает `401`. Если существующей интеграции нужен анонимный доступ, добавьте `LEGACY_LOCAL_STORAGE_PUBLIC_ACCESS=true` в `app.environment` и перезапустите контейнер. Продолжайте передавать этот путь в контейнер и не открывайте каталог загрузок напрямую через другой `alias`.
