...
- `./storage` нужно смонтировать и в `app`, и в `caddy`, при этом для прокси-контейнера лучше использовать режим только для чтения
- Контейнер `caddy` должен дождаться создания `nocobase.caddy`, а затем с помощью `ln -sf` связать его с `/etc/caddy/Caddyfile`
- На хост нужно публиковать только порт контейнера Caddy. Для сервиса `app` внутри сети Compose достаточно `expose: "13000"`
