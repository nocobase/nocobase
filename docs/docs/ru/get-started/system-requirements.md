...
Для production-сред рекомендуется использовать Linux-серверы, например Ubuntu LTS, Debian, Rocky Linux или AlmaLinux. Docker, обратный прокси, управление процессами, логи и резервное копирование более зрелы в Linux и проще интегрируются с типичными облачными серверными средами.
...
macOS обычно больше подходит для локальной разработки и тестирования и не рекомендуется в качестве production-сервера. Если вы настраиваете NocoBase локально для разработки или использования с AI Agent, см. [Local Development Setup](../ai/local-development-setup.md).
