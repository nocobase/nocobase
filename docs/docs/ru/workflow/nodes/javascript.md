...
Скрипт выполняется в рабочем потоке на сервере приложения NocoBase. По умолчанию используется безопасная песочница (QuickJS на базе WebAssembly), которая не поддерживает `require` и встроенные API Node.js. Подробнее см. в разделах [Движок выполнения](#движок-выполнения) и [Список возможностей](#список-возможностей).
...
Когда `WORKFLOW_SCRIPT_MODULES` **не настроена**, скрипты выполняются с использованием движка [QuickJS](https://bellard.org/quickjs/) на базе WebAssembly. Этот движок выполняет код в изолированной среде выполнения JavaScript со следующими характеристиками:
