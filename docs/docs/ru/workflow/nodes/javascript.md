...
Узлы JavaScript-скриптов помещают ожидающие выполнения скрипты в очередь задач и запускают их в отдельных потоках Worker. Переменная среды `WORKFLOW_SCRIPT_WORKER_CONCURRENCY` позволяет управлять количеством JavaScript-скриптов, которые каждый экземпляр приложения NocoBase может выполнять одновременно:
...
- Если переменная не задана или ее значение недопустимо, параллельность по умолчанию равна `1`
...
Когда лимит параллельности достигнут, новые задачи остаются в очереди до освобождения Worker. Настройки по умолчанию достаточно для большинства сценариев. Увеличение параллельности повышает нагрузку на процессор и память, поэтому изменяйте ее постепенно с учетом ресурсов сервера и нагрузки скриптов. В производственной среде не рекомендуется устанавливать значение `0`.
