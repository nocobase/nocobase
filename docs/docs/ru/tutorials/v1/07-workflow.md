...
Сначала переключитесь на роль **Root** — это администратор системы со всеми правами. Затем перейдите в [**модуль рабочих процессов**](https://docs.nocobase.com/handbook/workflow).
...
- **Триггер**: выберите «Событие коллекции».

![](https://static-docs.nocobase.com/Solution/151734250935202415162212.png)

#### 7.1.2 Пояснение к выбору типа триггера:

1. [**Событие коллекции**](https://docs.nocobase.com/handbook/workflow/triggers/collection): срабатывает при изменении данных в коллекции (добавление, изменение, удаление). Идеально подходит для отслеживания изменений в полях задачи, например смены ответственного.
2. [**Расписание**](https://docs.nocobase.com/handbook/workflow/triggers/schedule): автоматический запуск в заданное время — больше подходит для операций, связанных с расписанием.
3. [**Событие после действия**](https://docs.nocobase.com/handbook/workflow/triggers/post-action): привязывается к кнопке операции, срабатывает после её выполнения. Например, после нажатия кнопки «Сохранить» запускается задача.

В дальнейшем Вы откроете и другие триггеры: «Событие до действия», «Пользовательское событие», «Согласование» и др. — все они подключаются через соответствующие плагины.

В нашем сценарии используем [**Событие коллекции**](https://docs.nocobase.com/handbook/workflow/triggers/collection), чтобы отслеживать изменения «Ответственного» в коллекции задач. Сохраните рабочий процесс, нажмите **Конфигурация** и перейдите к настройкам процесса.
...
- **Коллекция**: выберите «Коллекция задач». (Какая коллекция запускает процесс — её данные будут синхронно подгружены в процесс. Логично, что мы хотим запускать процесс именно при изменениях в коллекции задач.)
...
Перед этим создадим [канал «Внутренних сообщений»](https://docs.nocobase.com/handbook/notification-in-app-message) для отправки уведомлений.

- Перейдите в Менеджер плагинов, выберите «Управление уведомлениями» и создайте уведомление о задачах.
...
Гибкость и расширяемость NocoBase открывают перед Вами безграничные возможности: в дальнейшем Вы можете глубже изучать другие плагины, создавать кастомные функции или строить более сложную бизнес-логику. После этих учебных материалов Вы уже владеете базовыми приёмами и ключевыми концепциями NocoBase.

С нетерпением ждём Ваших новых идей! Если возникнут вопросы — обратитесь к [официальной документации NocoBase](https://docs.nocobase.com/) или присоединяйтесь к [сообществу NocoBase](https://forum.nocobase.com/).
