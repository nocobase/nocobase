...
Для NocoBase вход в производственном окружении — не просто `reverse_proxy`. Кроме API нужны загрузки, статика фронтенда, маршруты `.well-known`, WebSocket и резервная страница SPA.
...
- `v` — редирект `/v` на `/v/`;
- `uploads` — каталог загрузок;
- `dist` — каталог сборки фронтенда;
- `oauth well-known` — пути обнаружения OAuth;
- `openid well-known` — пути обнаружения OpenID;
- `api` — пересылка `/api/` на приложение;
- `ws` — WebSocket на приложение;
- `spa v2` — вход и возврат для `/v/`;
- `spa v1` — вход и возврат для `/`.
...
Так реже пропускают WebSocket, статику, загрузки, `.well-known` или резервные страницы SPA.
