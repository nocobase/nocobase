---
title: "Resource API"
description: "Справочник API ресурсов NocoBase FlowEngine: полные сигнатуры методов MultiRecordResource и SingleRecordResource, форматы параметров и синтаксис фильтра."
keywords: "Resource,MultiRecordResource,SingleRecordResource,FlowResource,CRUD,filter,NocoBase"
---

# Resource API

NocoBase FlowEngine предоставляет два класса Resource для работы с данными на стороне клиента: `MultiRecordResource` — для списков и таблиц (несколько записей), `SingleRecordResource` — для форм и страниц деталей (одна запись). Они инкапсулируют вызовы REST API и обеспечивают реактивное управление данными.
...
| `destroySelectedRows()` | - | Удалить все выбранные строки |
| `refresh()` | - | Обновить данные (вызов действия `list`). Несколько вызовов в одном цикле событий объединяются |
...
| Метод | Описание |
|------|------|
| `setSelectedRows(rows)` | Установить выбранные строки |
| `getSelectedRows()` | Получить выбранные строки |

### Пример: использование в CollectionBlockModel

При наследовании от `CollectionBlockModel` необходимо создать ресурс через `createResource()`, а затем читать данные в `renderComponent()`:
...
Полный пример см. в [FlowEngine → Расширение блоков](../../plugin-development/client/flow-engine/block.md).

### Пример: вызов CRUD из кнопки действия

В обработчике `registerFlow` модели `ActionModel` через `ctx.blockModel?.resource` можно получить ресурс текущего блока и вызывать CRUD-методы:
...
NewTodoActionModel.registerFlow({
  ...
  steps: {
    openForm: {
      async handler(ctx) {
        // Получаем ресурс текущего блока
          ...
          content: (view) => (
            <MyForm
              onSubmit={async (values) => {
                // Создаём запись; ресурс автоматически обновится после создания
...
Полный пример см. в [Создание плагина управления данными с интеграцией фронтенда и бэкенда](../../plugin-development/client/examples/fullstack-plugin.md).
...
Используется для форм, страниц деталей и других сценариев с одной записью. Импортируется из `@nocobase/flow-engine`.
...
| `destroy(options?)` | - | Удалить текущую запись (используя ранее установленный filterByTk) |
| `refresh()` | - | Обновить данные (вызов действия `get`); пропускается, если `isNewRecord = true` |
...
async handler(ctx) {
  ...
  // Внутри save вызывается действие create, после чего данные автоматически обновляются
  ...
  // Внутри save вызывается действие update
...
| Метод | Описание |
|------|------|
| `setFilter(filter)` | Напрямую установить объект фильтра |
...
| `getFilter()` | Получить итоговый фильтр; несколько групп автоматически объединяются через `$and` |
...
| `setAppends(appends)` | Установить appends для полей связей |
...
| Метод | Описание |
|------|------|
| `setResourceName(name)` | Установить имя ресурса, например `'users'` или ресурса связи `'users.tags'` |
| `setSourceId(id)` | Установить ID родительской записи для ресурса связи |
...
Для управления условиями фильтрации в ресурсе рекомендуется использовать `addFilterGroup`:
...
| Возможность | MultiRecordResource | SingleRecordResource |
|------|-------|-------|
| Что возвращает getData() | `TDataItem[]` (массив) | `TData` (один объект) |
| Действие для обновления по умолчанию | `list` | `get` |
| Постраничная навигация | Поддерживается | Не поддерживается |
| Выбранные строки | Поддерживаются | Не поддерживаются |
...
| Типичные сценарии | Списки, таблицы, канбан-доски | Формы, страницы деталей |

## Связанные ссылки

- [Создание плагина управления данными с интеграцией фронтенда и бэкенда](../../plugin-development/client/examples/fullstack-plugin.md) — полный пример: практическое использование `resource.create()` в пользовательской кнопке действия
- [FlowEngine → Расширение блоков](../../plugin-development/client/flow-engine/block.md) — использование `createResource()` и `resource.getData()` в CollectionBlockModel
- [Менеджер ресурсов (ResourceManager)](../../plugin-development/server/resource-manager.md) — определение ресурсов REST API на сервере; именно эти интерфейсы вызывает клиентский Resource
