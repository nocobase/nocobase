...
description: "Справочник по команде nb app restart: перезапуск приложения NocoBase указанного env; локальные env автоматически выполняют необходимую подготовку установки или обновления во время перезапуска, а для Docker env контейнер приложения пересоздаётся из сохранённой конфигурации."
keywords: "nb app restart,NocoBase CLI,перезапуск приложения,Docker"
---

# nb app restart

Остановка и повторный запуск приложения NocoBase указанного env. Локальные env повторно используют поток `nb app stop` и `nb app start` и по умолчанию автоматически выполняют необходимую подготовку установки или обновления перед повторным запуском; Docker env сначала удаляют текущий контейнер, а затем пересоздают контейнер приложения на основе сохранённой конфигурации env.
