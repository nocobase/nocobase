...
Управление локальным проектом исходного кода NocoBase. Команды `nb source` необходимо выполнять в каталоге исходного кода (`<app-path>/source/`). Для окружений типов npm/Git используется локальный каталог исходного кода; для Docker-окружений обычно достаточно [`nb app`](../app/index.md) для управления средой выполнения.
