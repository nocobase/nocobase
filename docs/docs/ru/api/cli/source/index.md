...
Управление локальным проектом исходного кода NocoBase. Команды `nb source` необходимо выполнять в каталоге исходного кода (`<app-path>/source/`). Для env типа npm/Git используется локальный каталог исходного кода; для Docker env обычно достаточно [`nb app`](../app/index.md) для управления состоянием выполнения.
