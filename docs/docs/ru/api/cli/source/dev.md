...
Запускает режим разработки для окружений с исходным кодом npm или Git. Команду необходимо выполнять в каталоге исходного кода (`<app-path>/source/`). Для Docker-окружений используйте [`nb app logs`](../app/logs.md) для просмотра логов среды выполнения.
