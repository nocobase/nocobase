...
Запускает тесты в каталоге выбранного приложения. Перед выполнением тестов CLI заново создаёт встроенную тестовую базу данных Docker и внедряет внутренние переменные окружения `DB_*`.
...
| --- | --- | --- |
| `[paths...]` | string[] | Пути к тестовым файлам или glob-шаблоны, передаваемые тестовому раннеру |
| `--cwd`, `-c` | string | Каталог приложения для запуска тестов; по умолчанию — текущий каталог |
| `--watch`, `-w` | boolean | Запустить Vitest в режиме наблюдения |
| `--run` | boolean | Однократный запуск без режима наблюдения |
| `--allowOnly` | boolean | Разрешить тесты с `.only` |
| `--bail` | boolean | Остановить выполнение после первого сбоя |
| `--coverage` | boolean | Включить отчёт о покрытии |
| `--single-thread` | string | Передать режим одного потока базовому тестовому раннеру |
...
| `--db-dialect` | string | Диалект встроенной тестовой базы данных: `postgres`, `mysql`, `mariadb` или `kingbase` |
