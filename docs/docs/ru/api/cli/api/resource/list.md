...
Вывод списка записей выбранного ресурса. Для управления запросом можно использовать `--filter`, `--fields`, `--sort`, `--page` и связанные параметры.
...
| `--appends` | string[] | Связанные поля для подгрузки; можно передавать несколько раз или JSON-массивом |
