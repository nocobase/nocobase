---
title: "nb env update"
description: "Справка по команде nb env update: обновление сохранённой конфигурации API, аутентификации, исходного кода, приложения и базы данных."
...
| --- | --- | --- |
| `[name]` | string | Имя настроенного env, которое нужно обновить. Если не указано, используется текущее env |
...
| `--username` | string | Имя пользователя, сохраняемое для `basic`-аутентификации. Используйте его только если текущее env уже использует `basic`, или вместе с `--auth-type basic` |
...
| `--db-schema` | string | Схема базы данных. Обычно это используется только для PostgreSQL |
