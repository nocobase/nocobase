...
:::warning Поведение локального хранилища
...
При использовании собственного Nginx настройте `auth_request` и для `/storage/uploads/`. Полную конфигурацию см. в разделе [Обратный прокси Nginx](../../nocobase-cli/production/reverse-proxy/nginx.md).
...
| Markdown, внешней странице или стороннему сервису нужен публичный доступ к файлу | URL NocoBase | Включено |
| Клиент не поддерживает перенаправления `302` или должен напрямую использовать адрес хранилища | Исходный URL | Не применяется |

:::warning Примечание

[Локальное хранилище](./local), [Amazon S3](./amazon-s3), [Aliyun OSS](./aliyun-oss) и [Tencent COS](./tencent-cos) не создают временные подписанные URL. Исходный адрес может обойти разрешения записи файла. Устаревшие локальные URL по умолчанию требуют входа; доступ к исходному URL облачного хранилища зависит от его настроек публичного чтения.
