---
pkg: "@nocobase/plugin-file-manager"
---


# Coleção de Arquivos

## Introdução

Arquivos são registros em uma coleção com uma estrutura especial, conhecida como coleção de arquivos. Ela é usada para armazenar metadados de arquivos e pode ser gerenciada através do Gerenciador de Arquivos.

## Manual do Usuário

Criando uma Coleção de Arquivos

![20240324090414](https://static-docs.nocobase.com/20240324090414.png)

Campos Predefinidos em uma Coleção de Arquivos

![20240324090527](https://static-docs.nocobase.com/20240324090527.png)

Usando em Blocos

![20240324090848](https://static-docs.nocobase.com/20240324090848.png)

Campo de Associação

![20240324091529](https://static-docs.nocobase.com/20240324091529.png)

Blocos de Associação

![20240324091321](https://static-docs.nocobase.com/20240324091321.png)