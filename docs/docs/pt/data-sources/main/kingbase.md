...
keywords: "fonte de dados principal,人大金仓,KingbaseES,banco de dados principal,modo compatível com PostgreSQL,mapeamento de campos,NocoBase"
---
...
Atualmente, são compatíveis apenas os bancos de dados KingbaseES executados no modo compatível com PostgreSQL.
...
Para consultar o processo de instalação, veja [Instalar a aplicação NocoBase](/ai/install-nocobase-app). A principal diferença está nas variáveis de ambiente do banco de dados.
...
A fonte de dados principal KingbaseES é compatível com o modo PostgreSQL. Para a administração diária, consulte [Fonte de dados principal PostgreSQL](./postgresql.md).
...
4. Ao configurar os campos das tabelas, consulte os diretórios [Tabela](../data-modeling/collection.md) e [Campo](../data-modeling/collection-fields/index.md) para selecionar os tipos de campo e os componentes de campo.

## Mapeamento de tipos de campo

Ao criar campos por meio de uma página do NocoBase no banco de dados principal, o NocoBase cria os campos correspondentes do KingbaseES de acordo com a configuração do campo. Ao conectar tabelas existentes usando «Sincronizar do banco de dados», o NocoBase identifica os tipos de campo do KingbaseES de acordo com a lógica de compatibilidade com PostgreSQL e os mapeia automaticamente para o Field type e o Field interface adequados. Você pode ajustar a forma de exibição na configuração do campo.
...
Os tipos de campo do KingbaseES não compatíveis serão exibidos separadamente na configuração dos campos. Esses campos só poderão ser usados no NocoBase como campos comuns após o desenvolvimento de uma adaptação.

:::

Para ver mais configurações gerais, consulte [Introdução à fonte de dados principal](./index.md).
