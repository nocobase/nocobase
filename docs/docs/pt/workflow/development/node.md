---
title: "Estendendo tipos de nó"
description: "Estendendo tipos de nó: desenvolvimento de nós personalizados, configuração de nós, lógica de execução, API e ciclo de vida."
keywords: "fluxo de trabalho,extensão de nós,nós personalizados,desenvolvimento de nós,NocoBase"
---
...
Assim como os gatilhos, estender os tipos de nós também se divide em duas partes: lado do servidor e lado do cliente. O lado do servidor precisa implementar a lógica para a instrução registrada, enquanto o lado do cliente deve fornecer a configuração da interface para os parâmetros do nó onde a instrução está localizada.
...
2.  O nó que retorna um status de falha está dentro de um fluxo de trabalho de ramificação. Neste caso, a responsabilidade de determinar o próximo estado do fluxo de trabalho é transferida para o nó que abriu a ramificação. A lógica interna desse nó decidirá o estado do fluxo de trabalho subsequente, e essa decisão se propagará recursivamente até o fluxo de trabalho principal.

No final, o próximo estado de todo o fluxo de trabalho é determinado nos nós do fluxo de trabalho principal. Se um nó no fluxo de trabalho principal retornar uma falha, todo o fluxo de trabalho termina com um status de falha.

Se qualquer nó retornar um status "pendente" após a execução, todo o processo de execução será temporariamente interrompido e suspenso, aguardando que um evento definido pelo nó correspondente acione a retomada do fluxo de trabalho. Por exemplo, o Nó Manual, quando executado, pausará nesse nó com um status "pendente", aguardando intervenção manual para decidir se aprova. Se o status inserido manualmente for de aprovação, os nós subsequentes do fluxo de trabalho continuarão; caso contrário, será tratado de acordo com a lógica de falha descrita anteriormente.

Para mais status de retorno de instrução, consulte a seção Referência da API do Fluxo de Trabalho.
...
Para as definições dos vários parâmetros para a definição de tipos de nós, consulte a seção Referência da API do Fluxo de Trabalho.

## Lado do Cliente

Assim como os gatilhos, o formulário de configuração para uma instrução (tipo de nó) precisa ser implementado no lado do cliente.
...
Por exemplo, se precisarmos fornecer uma interface de configuração para o nó do tipo string de número aleatório (`randomString`) definido no lado do servidor acima, que possui um item de configuração `digit` representando o número de dígitos para o número aleatório:
...
Aqui, `FieldsetLoader` é uma função que retorna `Promise<{ default: ComponentType }>`, implementando o carregamento lazy via `import()` dinâmico. O componente para o qual aponta é um componente funcional React padrão que constrói o formulário usando o `Form.Item` do antd:
...
Um nó pode fornecer múltiplas interfaces de configuração para diferentes cenários:

- `FieldsetLoader` — Formulário da gaveta de configuração do nó (o mais comumente usado)
![FieldsetLoader](https://static-docs.nocobase.com/20260701153106.png)

- `PresetFieldsetLoader` — Formulário predefinido ao criar um nó (geralmente contém apenas os campos obrigatórios)
![PresetFieldsetLoader](https://static-docs.nocobase.com/20260701153041.png)

- `ComponentLoader` — Renderização personalizada do nó no canvas (usado para nós de ramificação e outros casos que requerem renderização especial)
...
Registre o tipo de nó com a instância do plugin de fluxo de trabalho dentro do plugin estendido:
...
Ou seja, o primeiro nível é a `key` e o título do nó. Por exemplo, na [referência de código](https://github.com/nocobase/nocobase/blob/develop/packages/plugins/%40nocobase/plugin-workflow/src/client-v2/nodes/calculation.tsx) do nó de cálculo, ao usar o resultado do nó de cálculo, as opções da interface são as seguintes:
...
Para um exemplo completo do mundo real, consulte: [Código-fonte do CalculationInstruction](https://github.com/nocobase/nocobase/blob/develop/packages/plugins/%40nocobase/plugin-workflow/src/client-v2/nodes/calculation.tsx)

Para as definições dos vários parâmetros para a definição de tipos de nós, consulte a seção [Referência da API do Fluxo de Trabalho](./api).
