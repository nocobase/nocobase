...
Antes de retornar um arquivo legado `/storage/uploads/`, o Nginx integrado pede ao NocoBase que verifique o login; o acesso anônimo retorna `401` por padrão. Se uma integração existente precisar de acesso anônimo, adicione `LEGACY_LOCAL_STORAGE_PUBLIC_ACCESS=true` a `app.environment` e reinicie o contêiner. Continue encaminhando esse caminho ao contêiner e não exponha diretamente o diretório de uploads com outro `alias`.
