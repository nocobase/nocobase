...
- Antes de retornar um arquivo legado `/storage/uploads/` por `alias`, o NocoBase verifica o login via `auth_request`; o acesso anônimo retorna `401` por padrão
...
Se um programa existente precisar de acesso anônimo a `/storage/uploads/`, adicione `LEGACY_LOCAL_STORAGE_PUBLIC_ACCESS=true` a `app.environment` e reinicie o contêiner. Não remova `auth_request`; o endpoint de autenticação avalia essa variável. As permissões de `/files/` não mudam.
...
Se você mantiver o Nginx por conta própria em vez de usar a configuração gerada, certifique-se de que `/files/` e a rota correspondente sob `APP_PUBLIC_PATH` sejam encaminhadas ao NocoBase antes das regras de fallback da SPA. Ao servir `/storage/uploads/` por `alias`, configure também `auth_request` para não ignorar a verificação de login. Consulte [Proxy reverso Nginx](../../nocobase-cli/production/reverse-proxy/nginx.md) para ver um exemplo completo.
