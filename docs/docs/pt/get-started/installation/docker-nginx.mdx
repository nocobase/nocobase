...
- `./storage` precisa ser montado em `app` e `nginx`, e o contêiner de proxy deve usar montagem somente leitura
...
- Exponha ao host apenas a porta do contêiner Nginx. O serviço `app` só precisa de `expose: "13000"` dentro da rede do Compose
