...
- O proxy reverso deve encaminhar ao NocoBase a rota `/files/` sob `APP_PUBLIC_PATH`. Em implantações em subcaminhos, mantenha também a rota compatível `/files/` na raiz. As configurações geradas pela CLI do NocoBase incluem ambas as regras automaticamente; para configurações mantidas manualmente, consulte [Proxy reverso Nginx](../nocobase-cli/production/reverse-proxy/nginx.md)
