...
Se o env atual salvou um hook com `nb init --hook-script`, `nb app start` executa `afterAppStart(context)` depois que a app realmente inicia e passa no `__health_check`. Envs instalados usam `context.phase = 'app-start'` e `context.command = 'app:start'`. Se a app já estiver rodando, esse comando não executa o hook.

Para envs prepared criados com `--prepare-only`, o primeiro `nb app start` executa `beforeAppInstall(context)`, conclui a primeira instalação e startup, e depois executa `afterAppStart(context)`. Os dois hooks recebem `context.phase = 'init'` e `context.command = 'app:start'`.
