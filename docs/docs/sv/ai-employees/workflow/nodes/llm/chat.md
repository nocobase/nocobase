...
Detta dokument har översatts av AI. För korrekt information, se [den engelska versionen](/ai-employees/workflow/nodes/llm/chat).
:::
...
Välj först en ansluten LLM-tjänst. Om ni ännu inte har anslutit någon LLM-tjänst behöver ni först lägga till en konfiguration för LLM-tjänsten. Se: [LLM-tjänsthantering](/ai-employees/features/llm-service)
...
- Svaret blir en JSON-sträng. Användaren behöver sedan tolka den med hjälp av andra Noder i Workflow för att kunna använda det strukturerade innehållet. Ni kan också använda funktionen [Strukturerad utdata](/ai-employees/workflow/nodes/llm/structured-output).
