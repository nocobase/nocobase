:::tip AI-översättningsmeddelande
Denna dokumentation har översatts automatiskt av AI.
:::


# Redigera

## Introduktion

Redigera-åtgärden är en radknapp som har den aktuella posten som kontext. Den presenteras som en popup-ruta där ni kan anpassa och konfigurera block.

![20251029165442](https://static-docs.nocobase.com/20251029165442.png)

Konfigurera block efter behov.

![20251029165514](https://static-docs.nocobase.com/20251029165514.png)

## Åtgärdsinställningar

![20251029165620](https://static-docs.nocobase.com/20251029165620.png)

- [Länkregler](/interface-builder/actions/action-settings/linkage-rule): Visa/dölj knappen dynamiskt;
- [Redigera knapp](/interface-builder/actions/action-settings/edit-button): Redigera knappens titel, typ och ikon;
- [Redigera popup-ruta](/interface-builder/actions/action-settings/edit-popup): Öppningsläge och storlek.