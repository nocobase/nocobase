---
pkg: "@nocobase/plugin-file-manager"
---

```yaml
pkg: "@nocobase/plugin-file-manager"
---

# Collection de fichiers

## Introduction

Les fichiers sont des enregistrements au sein d'une collection structurée de manière spécifique, appelée collection de fichiers. Cette collection est utilisée pour stocker les métadonnées des fichiers et peut être gérée via le Gestionnaire de fichiers.

## Guide d'utilisation

Créer une collection de fichiers

![20240324090414](https://static-docs.nocobase.com/20240324090414.png)

Champs prédéfinis dans une collection de fichiers

![20240324090527](https://static-docs.nocobase.com/20240324090527.png)

Utilisation dans les blocs

![20240324090848](https://static-docs.nocobase.com/20240324090848.png)

Champ d'association

![20240324091529](https://static-docs.nocobase.com/20240324091529.png)

Blocs d'association

![20240324091321](https://static-docs.nocobase.com/20240324091321.png)