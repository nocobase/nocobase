...
La mise en page bureau est le choix par défaut. Elle convient aux tâches quotidiennes d'administration et de traitement des données sur ordinateur. Si vous avez besoin d'une navigation et de pages indépendantes pour les appareils mobiles, vous pouvez aussi configurer la mise en page mobile.
...
Les mises en page bureau et mobile utilisent les mêmes sources de données et les mêmes données métier. Vous pouvez donc construire dans chaque mise en page des pages adaptées à différents appareils à partir de la même table de données.
