...
- `./storage` doit être monté dans `app` et `nginx`, et le conteneur proxy devrait utiliser un montage en lecture seule
...
- Exposez uniquement le port du conteneur Nginx à l'hôte. Le service `app` n'a besoin que de `expose: "13000"` dans le réseau Compose
