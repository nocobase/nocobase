...
- Avant de renvoyer un ancien fichier `/storage/uploads/` via `alias`, NocoBase vérifie la connexion avec `auth_request` ; l’accès anonyme renvoie `401` par défaut
...
Si un programme existant nécessite un accès anonyme à `/storage/uploads/`, ajoutez `LEGACY_LOCAL_STORAGE_PUBLIC_ACCESS=true` à `app.environment` et redémarrez le conteneur. Ne supprimez pas `auth_request` ; le endpoint d’authentification évalue cette variable. Les autorisations de `/files/` restent inchangées.
...
Si vous gérez Nginx vous-même au lieu d'utiliser la configuration générée, vérifiez que `/files/` et la route correspondante sous `APP_PUBLIC_PATH` sont transmises à NocoBase avant les règles de fallback de la SPA. Lorsque `/storage/uploads/` est servi via `alias`, configurez aussi `auth_request` pour ne pas contourner le contrôle de connexion. Consultez [Proxy inverse Nginx](../../nocobase-cli/production/reverse-proxy/nginx.md) pour un exemple complet.
