...
Les routes de NocoBase v2 sont préfixées par `/studio` par défaut. Par exemple, si vous enregistrez `path: '/hello'`, l'adresse réelle est `/studio/hello` :

```ts
this.router.add('hello', {
  path: '/hello', // accès réel -> /studio/hello
...
- [Router](../router) — enregistrement de routes et préfixe `/studio`
