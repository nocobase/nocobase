...
- Le proxy inverse doit transmettre à NocoBase la route `/files/` située sous `APP_PUBLIC_PATH`. Pour un déploiement dans un sous-chemin, conservez également la route compatible `/files/` à la racine. Les configurations générées par la CLI NocoBase incluent automatiquement ces deux règles ; pour les configurations maintenues manuellement, consultez [Proxy inverse Nginx](../nocobase-cli/production/reverse-proxy/nginx.md)
