...
- 外部数据库：接入已经存在的业务数据库，会读取外部数据库中的数据表和字段，在页面区块、权限、工作流和 API 中使用，支持的数据库：MySQL、PostgreSQL、MariaDB、KingbaseES、OceanBase、MSSQL、Oracle、ClickHouse、Doris。
- REST API 数据源：接入第三方 HTTP API，把 RESTful 资源映射成 NocoBase 数据表，在页面区块、权限和工作流中使用它。
- 外部 NocoBase 数据源：把另一个 NocoBase 应用接入当前应用，当前应用可以使用远端应用中的数据表、字段、标题字段和关系字段等元数据，不需要重新配置。

## 如何选择数据源

- 如果搭建全新的业务系统，建议使用 **主数据库**，可以使用 NocoBase 完整的能力。
- 如果是历史系统的功能补充、报表 / BI 分析、无 API，建议使用 **外部数据库**，集成第三方系统历史数据库，操作数据库表的数据进行增删改查。
