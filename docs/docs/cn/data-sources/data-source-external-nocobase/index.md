...
- 导入：当前应用接收上传的导入文件，并代理到远端应用执行导入；
- 同步导出：当前应用代理请求远端应用生成文件，并将远端返回的文件流式返回给浏览器下载；
- 异步导出：当前应用创建本地异步任务，任务执行时会在远端应用发起导出，并将远端进度同步到本地任务。下载结果时，当前应用会从远端应用流式获取导出文件。
- 导出附件：当前应用代理请求远端应用导出附件。同步模式会将远端返回的附件包流式返回给浏览器下载；异步模式会返回远端应用创建的任务信息。
...
如果远端应用出现 502、重启或短暂不可用，当前应用可能无法读取远端数据表元数据。远端服务恢复后，请在当前应用中重新加载该数据源的数据表。
