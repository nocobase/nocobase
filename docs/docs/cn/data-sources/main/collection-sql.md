...
编写 SQL 查询语句形成 SQL 数据表，不会在数据库创建真实数据库表，是读取 SQL 查询结果，让查询结果可以在表格、详情、图表和工作流中使用。适用场景：汇总数据、统计报表。
...
如果 SQL 查询语句没有变化，但底层数据表结构或字段发生变化，可以进入「Configure fields」，点击「Sync from database」重新执行 SQL 并同步字段。字段映射参考[「创建 SQL 数据表」](#字段映射)。
...
在数据表列表中，点击 SQL 数据表右侧的「Delete」，只会删除 NocoBase 中的 SQL 数据表配置和字段，不会删除底层来源表，也不会删除来源表中的数据。
