...
编写 SQL 查询语句形成 SQL 数据表，不会在数据库创建真实数据库表，而是读取 SQL 查询结果，再配置字段元数据，让查询结果可以在表格、详情、图表和工作流中使用。适用场景：汇总数据、统计报表。

:::warning 注意

SQL 数据表只支持 `SELECT` 语句或 `WITH ... SELECT` 语句，只支持数据查询展示，不支持数据的新增、编辑和删除。
...
| SQL | SQL 表使用的查询语句。NocoBase 会执行这段 SQL，基于查询结果配置字段元数据，再把查询结果作为数据表使用。 |
...
字段映射是创建 SQL 数据表后必须确认的配置。SQL 查询结果本身只告诉 NocoBase 返回了哪些列。为了让这些列像普通字段一样在界面中使用，还需要确认 `Field source`或者配置 `Field interface` 和字段显示名称。
...
SQL 数据表创建后，在数据表列表中，点击 SQL 数据表右侧的「Configure fields」，可以进入字段配置页面。字段配置用于维护 SQL 数据表有哪些字段、字段在界面中如何显示，以及 SQL 查询结果如何映射为 NocoBase 的 Field interface。
[了解更多字段配置信息](../field/field.md)

### 切换UI类型

SQL 数据表创建后，仍然可以在字段配置中调整字段的界面配置。字段配置页主要用于切换 Field interface、修改显示名称、描述和字段专属配置。
![configure_field_sql](https://static-docs.nocobase.com/configure_field_sql.png)

适合在这里处理这些情况：

- 创建 SQL 数据表时，设 Field interface 不正确
...
如果 SQL 查询语句没有变化，但底层数据表结构或字段发生变化，可以进入「Configure fields」，点击「Sync from database」重新执行 SQL 并同步字段元数据。字段映射参考[「创建 SQL 数据表」](#字段映射)。
...
点击字段右侧的「Delete」可以删除单个字段。删除字段只会移除 NocoBase 中保存的字段元数据，不会删除 SQL 查询语句，也不会删除来源数据表里的真实列。
[了解更多字段配置信息](../field/field.md)
...
在数据表列表中，点击某个 SQL 数据表右侧的「Edit」，可以调整 SQL 数据表在 NocoBase 中的元信息和运行配置。编辑时的配置项和创建 SQL 数据表时基本一致，仅`Collection name` 不可修改。
...
在数据表列表中，点击 SQL 数据表右侧的「Delete」，只会删除 NocoBase 中的 SQL 数据表配置和字段元数据，不会删除底层来源表，也不会删除来源表中的数据。
也支持批量选择后统一删除。删除前需要检查页面区块、图表、权限、工作流和外部 API 是否还在使用这个 SQL 数据表。