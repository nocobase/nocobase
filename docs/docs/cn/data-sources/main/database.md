...
- **评论表** — 适合保存任务评论、文章评论、审批意见、客户反馈等围绕业务记录产生的讨论内容
- **文件表** — 适合保存合同附件、发票文件、产品图片、员工证件等文件元信息，文件实际由文件存储引擎保存；在业务表（普通表、树表、日历表）中创建[关系字段](../collection/field-relation.md)关联文件表，使用业务表创建区块配置关系字段上传文件，文件元信息会自动保存到文件表中。
- **[SQL 数据表](collection-sql)** — 适合把销售汇总、库存预警、跨表统计报表、运营看板等 SQL 查询结果作为数据表使用
- **[连接数据库视图](collection-view)** — 适合连接数据库里已有的 view，比如财务报表视图、过滤后的客户视图、跨系统同步后的聚合视图
...
如果删除时勾选自动删除依赖对象，NocoBase 会尝试一并删除依赖该数据表的对象（比如视图）。执行前请确认这些依赖对象也可以删除。
