---
title: "主数据库和外部数据库对比"
description: "对比 NocoBase 主数据库和外部数据库在数据库类型、表结构、字段管理、数据写入、备份迁移和适用场景上的差异。"
keywords: "主数据库,外部数据库,数据源对比,字段映射,只读连接,数据表同步,NocoBase"
---

# 主数据库和外部数据库对比

在 NocoBase 中，**主数据库** 和 **外部数据库** 都可以作为数据源使用，不过它们的定位不同。

主数据库是 NocoBase 应用运行时使用的核心数据库，用来保存系统表、应用配置和主数据库中的业务数据。外部数据库用于连接已经存在的业务数据库，NocoBase 读取它的表结构和数据，再把这些表作为 Collection 使用。
...
| --- | --- |
| 从零搭建一个新的业务系统。 | 主数据库。 |
| 在 NocoBase 中创建和维护业务表结构。 | 主数据库。 |
| 连接已有 ERP、MES、WMS、报表库等数据库。 | 外部数据库。 |
| 不迁移历史数据，只给已有数据库搭建管理界面。 | 外部数据库。 |
| 接入 ClickHouse、Doris 这类分析型数据库做查询和报表。 | 外部数据库。 |
