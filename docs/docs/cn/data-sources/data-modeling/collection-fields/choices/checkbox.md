---
title: "勾选"
description: "勾选字段用于保存是或否、启用或停用等布尔值。"
keywords: "勾选,checkbox,布尔值,boolean,NocoBase"
---
...
在 NocoBase 中，**勾选（Checkbox）** 用于保存二选一的布尔值。

勾选字段适合启用状态、是否默认、是否完成、是否需要审批等简单判断。它的语义比用文本保存“是/否”更清晰。

如果状态超过两个值，比如草稿、进行中、完成，选择[下拉单选](./select.md)更合适。
...
在数据表的「Configure fields」页面中，点击「Add field」，选择「勾选」可以创建勾选字段。

![20240512180122](https://static-docs.nocobase.com/20240512180122.png)

| 配置 | 说明 |
| --- | --- |
| Field interface | 字段的界面类型。勾选对应 `checkbox`，决定页面中如何录入和展示。 |
| Field display name | 字段在界面中显示的名称，比如「是否启用」「是否完成」「是否开票」。建议使用业务人员能直接理解的名称。 |
| Field name | 字段标识名称，用于 API、关系字段、权限、工作流等内部引用。创建后通常不再修改，只支持字母、数字和下划线，并且必须以字母开头。 |
| Field type | 字段在数据层的类型。勾选字段默认是 `boolean`。 |
| Default value | 默认值。新增记录时，如果用户没有填写，可以自动带出默认值。 |
| Validation rules | 校验规则。通常配置必填或默认值。 |
...
| 特性 | 说明 |
| --- | --- |
| 默认 Field interface | `checkbox`。 |
| 默认 Field type | `boolean`。 |
| 可选 Field type | `boolean`。 |
| 页面组件 | 编辑模式使用勾选框。 |
| 筛选 | 支持按是、否、为空筛选。 |
| 排序 | 支持按布尔值排序。 |
| 校验 | 支持必填、默认值等基础配置。 |
...
创建后，点击字段右侧的「Edit」可以编辑勾选字段配置。编辑字段主要用于调整字段在 NocoBase 中的展示和使用方式，比如修改显示名称、说明、默认值、校验规则或字段专属配置。
...
点击字段右侧的「Delete」可以删除勾选字段。主数据库中还可以勾选多个字段后批量删除。

删除主数据库中新建的勾选字段时，通常会同时删除数据库中的真实列及该列已有数据。删除从数据库同步或外部数据源映射出的字段时，影响范围取决于对应数据源和字段来源。
...
- [下拉单选](./select.md) — 保存多状态中的一个值
- [单选框组](./radio-group.md) — 以单选按钮方式展示选项
