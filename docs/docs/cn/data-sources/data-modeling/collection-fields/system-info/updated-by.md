---
title: "更新人"
description: "更新人字段用于自动记录最后一次更新记录的用户。"
keywords: "更新人,updatedBy,系统字段,用户,NocoBase"
---
...
在 NocoBase 中，**更新人（Updated by）** 用于自动记录最后更新记录的用户。

更新人通常由预设字段生成。它适合审计、责任追踪、筛选和工作流条件。

如果要表达业务负责人、处理人或审批人，建议单独创建用户关系字段。
...
在数据表的「Configure fields」页面中，点击「Add field」，选择「更新人」可以创建更新人字段。

![index-2025-11-01-00-51-12](https://static-docs.nocobase.com/index-2025-11-01-00-51-12.png)

| 配置 | 说明 |
| --- | --- |
| Field interface | 字段的界面类型。更新人对应 `updatedBy`，决定页面中如何录入和展示。 |
| Field display name | 字段在界面中显示的名称，比如「更新人」「最后修改人」。建议使用业务人员能直接理解的名称。 |
| Field name | 字段标识名称，用于 API、关系字段、权限、工作流等内部引用。创建后通常不再修改，只支持字母、数字和下划线，并且必须以字母开头。 |
| Field type | 字段在数据层的类型。更新人通常是指向用户表的 `belongsTo` 关系字段。 |
...
| 特性 | 说明 |
| --- | --- |
| 默认 Field interface | `updatedBy`。 |
| 默认 Field type | `belongsTo`。 |
| 可选 Field type | `belongsTo`。 |
| 页面组件 | 系统自动写入，页面通常以用户展示组件呈现。 |
| 筛选 | 支持按用户筛选。 |
| 排序 | 通常不按更新人排序。 |
| 校验 | 由系统自动写入。 |
...
创建后，点击字段右侧的「Edit」可以编辑更新人字段配置。编辑字段主要用于调整字段在 NocoBase 中的展示和使用方式，比如修改显示名称、说明、默认值、校验规则或字段专属配置。
...
点击字段右侧的「Delete」可以删除更新人字段。主数据库中还可以勾选多个字段后批量删除。

删除主数据库中新建的更新人字段时，通常会同时删除数据库中的真实列及该列已有数据。删除从数据库同步或外部数据源映射出的字段时，影响范围取决于对应数据源和字段来源。
