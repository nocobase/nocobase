---
title: "富文本"
description: "富文本字段用于保存带样式、图片、链接等格式化内容。"
keywords: "富文本,rich text,内容字段,NocoBase"
---
...
在 NocoBase 中，**富文本（Rich text）** 用于保存格式化内容。
...
在数据表的「Configure fields」页面中，点击「Add field」，选择「富文本」可以创建富文本字段。

![20240512181002](https://static-docs.nocobase.com/20240512181002.png)

| 配置 | 说明 |
| --- | --- |
| Field interface | 字段的界面类型。富文本对应 `richText`，决定页面中如何录入和展示。 |
| Field display name | 字段在界面中显示的名称，比如「公告正文」「邮件模板」「产品说明」。建议使用业务人员能直接理解的名称。 |
| Field name | 字段标识名称，用于 API、关系字段、权限、工作流等内部引用。创建后通常不再修改，只支持字母、数字和下划线，并且必须以字母开头。 |
| Field type | 字段在数据层的类型。富文本字段通常使用 `text` 保存内容。 |
...
创建后，点击字段右侧的「Edit」可以编辑富文本字段配置。编辑字段主要用于调整字段在 NocoBase 中的展示和使用方式，比如修改显示名称、说明、默认值、校验规则或字段专属配置。
...
点击字段右侧的「Delete」可以删除富文本字段。主数据库中还可以勾选多个字段后批量删除。

删除主数据库中新建的富文本字段时，通常会同时删除数据库中的真实列及该列已有数据。删除从数据库同步或外部数据源映射出的字段时，影响范围取决于对应数据源和字段来源。
