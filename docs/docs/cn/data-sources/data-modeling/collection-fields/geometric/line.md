---
title: "线"
description: "线字段用于保存路线、轨迹等线状空间数据。"
keywords: "线,LineString,路线,几何图形,NocoBase"
---
...
在 NocoBase 中，**线（LineString）** 用于保存线状空间数据。

线字段适合路线、轨迹、管线、巡检路径等业务数据。配合地图区块，可以展示路径。

如果只需要一个位置，选择[点](./point.md)。如果需要区域范围，选择[多边形](./polygon.md)。
...
在数据表的「Configure fields」页面中，点击「Add field」，选择「线」可以创建线字段。

![20240512181454](https://static-docs.nocobase.com/20240512181454.png)

| 配置 | 说明 |
| --- | --- |
| Field interface | 字段的界面类型。线对应 `lineString`，决定页面中如何录入和展示。 |
| Field display name | 字段在界面中显示的名称，比如「配送路线」「巡检轨迹」「管线」。建议使用业务人员能直接理解的名称。 |
| Field name | 字段标识名称，用于 API、关系字段、权限、工作流等内部引用。创建后通常不再修改，只支持字母、数字和下划线，并且必须以字母开头。 |
| Field type | 字段在数据层的类型。线字段默认是 `lineString`。 |
...
| 特性 | 说明 |
| --- | --- |
| 默认 Field interface | `lineString`。 |
| 默认 Field type | `lineString`。 |
| 可选 Field type | `lineString`。 |
| 页面组件 | 编辑模式使用地图绘制组件。 |
...
创建后，点击字段右侧的「Edit」可以编辑线字段配置。编辑字段主要用于调整字段在 NocoBase 中的展示和使用方式，比如修改显示名称、说明、默认值、校验规则或字段专属配置。
...
点击字段右侧的「Delete」可以删除线字段。主数据库中还可以勾选多个字段后批量删除。

删除主数据库中新建的线字段时，通常会同时删除数据库中的真实列及该列已有数据。删除从数据库同步或外部数据源映射出的字段时，影响范围取决于对应数据源和字段来源。
