---
title: "多边形"
description: "多边形字段用于保存区域、边界、服务范围等面状空间数据。"
keywords: "多边形,Polygon,区域,几何图形,NocoBase"
---
...
在 NocoBase 中，**多边形（Polygon）** 用于保存面状空间区域。

多边形字段适合行政片区、配送区域、销售区域、禁入区域等业务数据。配合地图区块，可以展示区域范围。

如果区域是简单圆形，选择[圆](./circle.md)。如果只保存一个位置，选择[点](./point.md)。
...
在数据表的「Configure fields」页面中，点击「Add field」，选择「多边形」可以创建多边形字段。

![20240512181547](https://static-docs.nocobase.com/20240512181547.png)

| 配置 | 说明 |
| --- | --- |
| Field interface | 字段的界面类型。多边形对应 `polygon`，决定页面中如何录入和展示。 |
| Field display name | 字段在界面中显示的名称，比如「销售区域」「配送范围」「风险区域」。建议使用业务人员能直接理解的名称。 |
| Field name | 字段标识名称，用于 API、关系字段、权限、工作流等内部引用。创建后通常不再修改，只支持字母、数字和下划线，并且必须以字母开头。 |
| Field type | 字段在数据层的类型。多边形字段默认是 `polygon`。 |
...
| 特性 | 说明 |
| --- | --- |
| 默认 Field interface | `polygon`。 |
| 默认 Field type | `polygon`。 |
| 可选 Field type | `polygon`。 |
| 页面组件 | 编辑模式使用地图绘制组件。 |
...
创建后，点击字段右侧的「Edit」可以编辑多边形字段配置。编辑字段主要用于调整字段在 NocoBase 中的展示和使用方式，比如修改显示名称、说明、默认值、校验规则或字段专属配置。
...
点击字段右侧的「Delete」可以删除多边形字段。主数据库中还可以勾选多个字段后批量删除。

删除主数据库中新建的多边形字段时，通常会同时删除数据库中的真实列及该列已有数据。删除从数据库同步或外部数据源映射出的字段时，影响范围取决于对应数据源和字段来源。
