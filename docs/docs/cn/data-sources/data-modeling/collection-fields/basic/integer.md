---
title: "整数"
description: "整数字段用于保存件数、人数、次数、天数等不带小数的数值。"
keywords: "整数,integer,数字字段,NocoBase"
---
...
在 NocoBase 中，**整数（Integer）** 用于保存不带小数的数值。

整数字段适合数量、次数、人数、排序号等业务数据。它可以参与筛选、排序、统计、权限和工作流条件。

如果需要保存小数、金额、重量或比例，选择[数字](./number.md)或[百分比](./percent.md)更合适。
...
在数据表的「Configure fields」页面中，点击「Add field」，选择「整数」可以创建整数字段。

![20240512175723](https://static-docs.nocobase.com/20240512175723.png)

| 配置 | 说明 |
| --- | --- |
| Field interface | 字段的界面类型。整数对应 `integer`，决定页面中如何录入和展示。 |
| Field display name | 字段在界面中显示的名称，比如「数量」「人数」「延期天数」。建议使用业务人员能直接理解的名称。 |
| Field name | 字段标识名称，用于 API、关系字段、权限、工作流等内部引用。创建后通常不再修改，只支持字母、数字和下划线，并且必须以字母开头。 |
| Field type | 字段在数据层的类型。整数字段默认是 `integer`，大范围整数可以选择 `bigInt`。 |
...
| 特性 | 说明 |
| --- | --- |
| 默认 Field interface | `integer`。 |
| 默认 Field type | `integer`。 |
| 可选 Field type | `integer`、`bigInt`。 |
| 页面组件 | 编辑模式使用数字输入框。 |
| 筛选 | 支持等于、不等于、大于、小于、区间、为空、不为空等数值筛选。 |
| 排序 | 支持在表格区块中排序。 |
| 校验 | 支持最小值、最大值、必填等数值校验。 |

## 编辑配置

创建后，点击字段右侧的「Edit」可以编辑整数字段配置。编辑字段主要用于调整字段在 NocoBase 中的展示和使用方式，比如修改显示名称、说明、默认值、校验规则或字段专属配置。
...
点击字段右侧的「Delete」可以删除整数字段。主数据库中还可以勾选多个字段后批量删除。

删除主数据库中新建的整数字段时，通常会同时删除数据库中的真实列及该列已有数据。删除从数据库同步或外部数据源映射出的字段时，影响范围取决于对应数据源和字段来源。
...
| 场景 | 用途 |
| --- | --- |
| 表单区块 | 录入不带小数的数量、次数、天数等。 |
| 表格区块 | 展示、排序和筛选整数。 |
| 图表区块 | 按数量、次数等字段做统计。 |
| 工作流和权限 | 作为条件字段参与判断，比如数量是否大于 0。 |
