---
title: "手机号"
description: "手机号字段用于保存手机号、联系电话等电话类文本，并提供格式校验。"
keywords: "手机号,phone,电话,联系方式,NocoBase"
---

# 手机号

## 介绍

在 NocoBase 中，**手机号（Phone）** 用于保存手机号或联系电话。

手机号字段适合客户电话、联系人电话、员工手机号等联系方式。它比普通文本更适合表达电话类数据。

如果需要保存多个电话或复杂联系信息，可以使用[多行文本](./textarea.md)或单独建联系人表。

## 适用场景

手机号适合这些业务场景：

- 客户手机号、联系人电话
- 员工手机号、备用电话
- 门店联系电话、服务热线
- 短信通知号码

## 创建配置

在数据表的「Configure fields」页面中，点击「Add field」，选择「手机号」可以创建手机号字段。

![20240512175526](https://static-docs.nocobase.com/20240512175526.png)

| 配置 | 说明 |
| --- | --- |
| Field interface | 字段的界面类型。手机号对应 `phone`，决定页面中如何录入和展示。 |
| Field display name | 字段在界面中显示的名称，比如「手机号」「联系电话」「服务热线」。建议使用业务人员能直接理解的名称。 |
| Field name | 字段标识名称，用于 API、关系字段、权限、工作流等内部引用。创建后通常不再修改，只支持字母、数字和下划线，并且必须以字母开头。 |
| Field type | 字段在数据层的类型。手机号字段默认是 `string`。 |
| Default value | 默认值。新增记录时，如果用户没有填写，可以自动带出默认值。 |
| Validation rules | 校验规则。可以配置长度、正则表达式或必填。 |
| Description | 字段说明。适合写字段含义、填写要求、数据来源或维护人。 |

:::warning 注意

字段名创建后会被页面区块、权限、工作流和 API 引用。创建前先确认命名，避免后续修改带来配置调整成本。

:::

## 字段特性

手机号字段的默认行为如下：

| 特性 | 说明 |
| --- | --- |
| 默认 Field interface | `phone`。 |
| 默认 Field type | `string`。 |
| 可选 Field type | `string`。 |
| 页面组件 | 编辑模式使用输入框，并可配置电话格式校验。 |
| 筛选 | 支持文本类筛选，比如包含、等于、为空、不为空。 |
| 排序 | 支持在表格区块中排序。 |
| 校验 | 支持长度、正则表达式、必填等校验。 |

## 编辑配置

创建后，点击字段右侧的「Edit」可以编辑手机号字段配置。编辑字段主要用于调整字段在 NocoBase 中的展示和使用方式，比如修改显示名称、说明、默认值、校验规则或字段专属配置。

如果字段来自主数据库中已经同步的表，编辑时通常是在做字段映射——把数据库字段映射为 NocoBase 的 Field type 和 Field interface。

| 配置 | 允许编辑 | 说明 |
| --- | --- | --- |
| Field display name | 是 | 修改字段在界面中的显示名称，不改变字段标识名称。 |
| Field name | 否 | 字段标识名称创建后通常不能在编辑表单中修改。 |
| Field interface | 条件支持 | 主数据库字段或同步字段在字段映射时可以调整。调整后会影响页面输入、展示和校验方式。 |
| Field type | 条件支持 | 主数据库字段或同步字段在字段映射时可以调整。调整前需要确认已有数据能否按新类型使用。 |
| Default value | 是 | 调整新增记录时的默认值。 |
| Validation rules | 是 | 调整字段校验规则。 |
| Description | 是 | 补充字段含义、填写要求、数据来源或维护人。 |

:::warning 注意

切换 Field type 或 Field interface 不等于简单改一个显示名称。它会影响字段的存储方式、输入组件、校验规则、筛选条件和工作流变量使用方式。已有数据较多时，先确认数据格式是否匹配。

:::

## 删除字段

点击字段右侧的「Delete」可以删除手机号字段。主数据库中还可以勾选多个字段后批量删除。

删除主数据库中新建的手机号字段时，通常会同时删除数据库中的真实列及该列已有数据。删除从数据库同步或外部数据源映射出的字段时，影响范围取决于对应数据源和字段来源。

:::danger 警告

删除字段可能影响页面区块、表单、筛选、权限、工作流、API、导入导出和已有数据。删除前先确认字段是否仍被业务配置引用。

:::

## 页面配置使用

手机号字段适合在表单、详情、筛选和通知场景中使用。
![20260709224555](https://static-docs.nocobase.com/20260709224555.png)

| 场景 | 用途 |
| --- | --- |
| 表单区块 | 录入手机号或联系电话。 |
| 详情区块 | 展示联系方式。 |
| 筛选区块 | 按手机号或号码片段筛选记录。 |
| 工作流和通知 | 作为短信、电话类通知的号码来源。 |

## 相关链接

- [字段](../index.md) — 了解字段的作用、分类和映射逻辑
- [普通表](../../../data-source-main/general-collection.md) — 在普通表中创建和管理字段
- [单行文本](./input.md) — 保存普通短文本
- [邮箱](./email.md) — 保存邮箱地址
