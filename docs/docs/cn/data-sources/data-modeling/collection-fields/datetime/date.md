---
title: "日期"
description: "日期字段用于保存生日、签约日、到期日等不包含具体时刻的日期。"
keywords: "日期,date,日期字段,NocoBase"
---
...
在 NocoBase 中，**日期（Date）** 用于保存不包含具体时刻的日期。

日期字段适合生日、签约日、到期日、计划日期等只关心年月日的业务数据。

如果需要保存具体小时、分钟和秒，选择[日期时间](./datetime.md)。如果只需要一天中的时间，选择[时间](./time.md)。
...
| 配置 | 说明 |
| --- | --- |
| Field interface | 字段的界面类型。日期对应 `date`，决定页面中如何录入和展示。 |
| Field display name | 字段在界面中显示的名称，比如「签约日期」「到期日期」「生日」。建议使用业务人员能直接理解的名称。 |
| Field name | 字段标识名称，用于 API、关系字段、权限、工作流等内部引用。创建后通常不再修改，只支持字母、数字和下划线，并且必须以字母开头。 |
| Field type | 字段在数据层的类型。日期字段默认是 `dateonly`。 |
| Default value | 默认值。新增记录时，如果用户没有填写，可以自动带出默认值。 |
| Validation rules | 校验规则。可以配置必填、日期范围等。 |
...
| 特性 | 说明 |
| --- | --- |
| 默认 Field interface | `date`。 |
| 默认 Field type | `dateonly`。 |
| 可选 Field type | `dateonly`。 |
| 页面组件 | 编辑模式使用日期选择器。 |
| 筛选 | 支持按日期、区间、为空、不为空筛选。 |
| 排序 | 支持按日期排序。 |
| 校验 | 支持必填和日期范围等校验。 |

## 编辑配置

创建后，点击字段右侧的「Edit」可以编辑日期字段配置。编辑字段主要用于调整字段在 NocoBase 中的展示和使用方式，比如修改显示名称、说明、默认值、校验规则或字段专属配置。
...
点击字段右侧的「Delete」可以删除日期字段。主数据库中还可以勾选多个字段后批量删除。

删除主数据库中新建的日期字段时，通常会同时删除数据库中的真实列及该列已有数据。删除从数据库同步或外部数据源映射出的字段时，影响范围取决于对应数据源和字段来源。
