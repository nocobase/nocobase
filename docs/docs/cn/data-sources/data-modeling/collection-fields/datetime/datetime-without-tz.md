---
title: "日期时间（不含时区）"
description: "日期时间（不含时区）字段用于保存不进行时区转换的日期和时间。"
keywords: "日期时间,datetime without timezone,不含时区,NocoBase"
---
...
在 NocoBase 中，**日期时间（不含时区）（Date time without timezone）** 用于保存不做时区转换的日期和时间。

日期时间（不含时区）适合排班、营业时间、课程时间等更关注本地显示值的场景。

如果需要表达一个全球一致的真实时间点，选择[日期时间（含时区）](./datetime.md)更合适。
...
在数据表的「Configure fields」页面中，点击「Add field」，选择「日期时间（不含时区）」可以创建日期时间（不含时区）字段。

![20260709232834](https://static-docs.nocobase.com/20260709232834.png)

| 配置 | 说明 |
| --- | --- |
| Field interface | 字段的界面类型。日期时间（不含时区）对应 `datetimeNoTz`，决定页面中如何录入和展示。 |
| Field display name | 字段在界面中显示的名称，比如「排班时间」「课程时间」「营业时间」。建议使用业务人员能直接理解的名称。 |
| Field name | 字段标识名称，用于 API、关系字段、权限、工作流等内部引用。创建后通常不再修改，只支持字母、数字和下划线，并且必须以字母开头。 |
| Field type | 字段在数据层的类型。日期时间（不含时区）通常使用 `datetimeNoTz`。 |
...
| 特性 | 说明 |
| --- | --- |
| 默认 Field interface | `datetimeNoTz`。 |
| 默认 Field type | `datetimeNoTz`。 |
| 可选 Field type | `datetimeNoTz`。 |
| 页面组件 | 编辑模式使用日期时间选择器。 |
...
创建后，点击字段右侧的「Edit」可以编辑日期时间（不含时区）字段配置。编辑字段主要用于调整字段在 NocoBase 中的展示和使用方式，比如修改显示名称、说明、默认值、校验规则或字段专属配置。
...
点击字段右侧的「Delete」可以删除日期时间（不含时区）字段。主数据库中还可以勾选多个字段后批量删除。

删除主数据库中新建的日期时间（不含时区）字段时，通常会同时删除数据库中的真实列及该列已有数据。删除从数据库同步或外部数据源映射出的字段时，影响范围取决于对应数据源和字段来源。
...
- [日期时间（含时区）](./datetime.md) — 保存带时区语义的时间点
- [日期](./date.md) — 只保存日期
