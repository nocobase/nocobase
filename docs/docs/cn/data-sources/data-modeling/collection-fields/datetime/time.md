---
title: "时间"
description: "时间字段用于保存一天中的时间，比如营业开始时间、提醒时间。"
keywords: "时间,time,时间字段,NocoBase"
---
...
在 NocoBase 中，**时间（Time）** 用于保存一天中的时间。

时间字段适合营业时间、提醒时间、排班时间段等不绑定具体日期的业务数据。

如果需要同时保存日期和时间，选择[日期时间](./datetime.md)。
...
在数据表的「Configure fields」页面中，点击「Add field」，选择「时间」可以创建时间字段。

![20240512181216](https://static-docs.nocobase.com/20240512181216.png)

| 配置 | 说明 |
| --- | --- |
| Field interface | 字段的界面类型。时间对应 `time`，决定页面中如何录入和展示。 |
| Field display name | 字段在界面中显示的名称，比如「开始时间」「提醒时间」「营业时间」。建议使用业务人员能直接理解的名称。 |
| Field name | 字段标识名称，用于 API、关系字段、权限、工作流等内部引用。创建后通常不再修改，只支持字母、数字和下划线，并且必须以字母开头。 |
| Field type | 字段在数据层的类型。时间字段默认是 `time`。 |
...
| 特性 | 说明 |
| --- | --- |
| 默认 Field interface | `time`。 |
| 默认 Field type | `time`。 |
| 可选 Field type | `time`。 |
| 页面组件 | 编辑模式使用时间选择器。 |
| 筛选 | 支持按时间、区间、为空、不为空筛选。 |
| 排序 | 支持按时间排序。 |
| 校验 | 支持必填和时间范围等校验。 |

## 编辑配置

创建后，点击字段右侧的「Edit」可以编辑时间字段配置。编辑字段主要用于调整字段在 NocoBase 中的展示和使用方式，比如修改显示名称、说明、默认值、校验规则或字段专属配置。
...
点击字段右侧的「Delete」可以删除时间字段。主数据库中还可以勾选多个字段后批量删除。

删除主数据库中新建的时间字段时，通常会同时删除数据库中的真实列及该列已有数据。删除从数据库同步或外部数据源映射出的字段时，影响范围取决于对应数据源和字段来源。
