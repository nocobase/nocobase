...
分布式架构中，除应用实例本身以外的其他系统组件，根据不同团队的运维需求，在符合当前约束的条件下，可由团队的运维人员自行选用。
...
使用 Docker 或 Kubernetes 部署时，请参考 [Kubernetes 部署：本地存储](./kubernetes#本地存储) 章节。
