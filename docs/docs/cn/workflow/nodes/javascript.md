...
JavaScript 脚本节点会将待执行的脚本放入任务队列，并在独立的 Worker 线程中执行。你可以通过环境变量 `WORKFLOW_SCRIPT_WORKER_CONCURRENCY`，控制每个 NocoBase 应用实例同时执行的 JavaScript 脚本数量：
...
达到并发上限后，新任务会继续排队，直到有可用的 Worker。默认配置适用于大部分场景。提高并发数会增加 CPU 和内存占用，建议根据服务端资源和脚本负载逐步调整。生产环境中不建议将并发数设置为 `0`。
