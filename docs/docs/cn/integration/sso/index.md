...
- **[SAML 2.0](/auth-verification/auth-saml/index.md)**: 基于 XML 的开放标准,广泛应用于企业级身份认证。适用于需要与企业身份提供商(IdP)集成的场景。

- **[OIDC (OpenID Connect)](/auth-verification/auth-oidc/index.md)**: 基于 OAuth 2.0 的身份认证层,提供现代化的认证和授权机制。支持与主流身份提供商(如 Google、Azure AD 等)集成。

- **[CAS (Central Authentication Service)](/auth-verification/auth-cas/index.md)**: 由耶鲁大学开发的单点登录协议,广泛应用于高校和教育机构。

- **[LDAP](/auth-verification/auth-ldap/index.md)**: 轻量级目录访问协议,用于访问和维护分布式目录信息服务。适用于需要与 Active Directory 或其他 LDAP 服务器集成的场景。

### 第三方平台认证

- **[企业微信](/auth-verification/auth-wecom/index.md)**: 支持企业微信扫码登录和企业微信内免登。

- **[钉钉](/auth-verification/auth-dingtalk/index.md)**: 支持钉钉扫码登录和钉钉内免登。

### 其他认证方式

- **[短信验证码](/auth-verification/auth-sms/index.md)**: 基于手机短信的验证码登录方式。

- **[用户名密码](/auth-verification/auth/index.md)**: NocoBase 内置的基础认证方式。
...
- [用户认证文档](/auth-verification/auth/index.md)
- [API 密钥认证](/integration/api-keys/index.md)
- [用户和权限管理](/plugins/@nocobase/plugin-users/index.md)
