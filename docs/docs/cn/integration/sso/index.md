...
- **[SAML 2.0](/plugins/@nocobase/plugin-auth-saml/)**: 基于 XML 的开放标准,广泛应用于企业级身份认证。适用于需要与企业身份提供商(IdP)集成的场景。

- **[OIDC (OpenID Connect)](/plugins/@nocobase/plugin-auth-oidc/)**: 基于 OAuth 2.0 的身份认证层,提供现代化的认证和授权机制。支持与主流身份提供商(如 Google、Azure AD 等)集成。

- **[CAS (Central Authentication Service)](/plugins/@nocobase/plugin-auth-cas/)**: 由耶鲁大学开发的单点登录协议,广泛应用于高校和教育机构。

- **[LDAP](/plugins/@nocobase/plugin-auth-ldap/)**: 轻量级目录访问协议,用于访问和维护分布式目录信息服务。适用于需要与 Active Directory 或其他 LDAP 服务器集成的场景。

### 第三方平台认证

- **[企业微信](/plugins/@nocobase/plugin-auth-wecom/)**: 支持企业微信扫码登录和企业微信内免登。

- **[钉钉](/plugins/@nocobase/plugin-auth-dingtalk/)**: 支持钉钉扫码登录和钉钉内免登。

### 其他认证方式

- **[短信验证码](/plugins/@nocobase/plugin-auth-sms/)**: 基于手机短信的验证码登录方式。

- **[用户名密码](/plugins/@nocobase/plugin-auth/)**: NocoBase 内置的基础认证方式。
...
- [用户认证插件文档](/plugins/@nocobase/plugin-auth/)
