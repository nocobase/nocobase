...
开始之前，确保你已经通过 NocoBase CLI（`nb init`）安装好了一个 NocoBase 应用。CLI 支持 npm 和 Git 两种来源，推荐使用 Git 来源（AI 开发时可直接参考源码）。详见 [AI Agent 接入指南](../ai/quick-start.mdx) 或 [使用 CLI 安装应用](../nocobase-cli/installation/cli.md)。
