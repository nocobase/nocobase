...
NocoBase Server 的中间件（Middleware）本质上是 **Koa 中间件**，你可以像在 Koa 里一样操作 `ctx` 对象处理请求和响应。不过由于 NocoBase 需要管理不同业务层的逻辑，如果所有中间件都放在一起，会非常难以维护。

为此，NocoBase 将中间件分为 **四个层级**：

1. **数据源级中间件**：`app.dataSourceManager.use()`
   只作用于某个数据源的请求，常用于该数据源的数据库连接、字段验证或事务处理等逻辑。

2. **资源级中间件**：`app.resourceManager.use()`
   只对已定义的资源（Resource）生效，适合处理资源级逻辑，比如数据权限、格式化等。
...
- [Context 请求上下文](./context.md) — 了解中间件中 `ctx` 对象的完整 API
- [ResourceManager 资源管理](./resource-manager.md) — 资源级中间件的注册与资源定义
- [ACL 权限控制](./acl.md) — 权限级中间件的使用与权限校验逻辑
- [Plugin 插件](./plugin.md) — 在插件的 `load` 方法中注册中间件
- [DataSourceManager 数据源管理](./data-source-manager.md) — 数据源级中间件的使用场景
- [服务端开发概述](./index.md) — 服务端整体架构与中间件在其中的位置
