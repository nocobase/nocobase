...
| **load()** | 插件加载时执行 | 所有插件的 `beforeLoad()` 执行完毕后才会开始执行 `load()`。适合注册资源、API 接口等核心业务逻辑。**注意：** `load()` 阶段数据库还没有完成同步，不能执行数据库查询或写入操作——数据库操作应放在 `install()` 或请求处理函数中。 |
| **install()** | 插件首次激活时执行 | 只在插件第一次被启用时执行一次，通常来说用于初始化数据库表结构、插入初始数据等安装逻辑。 |
