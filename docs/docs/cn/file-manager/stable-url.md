...
它也可以叫代理 URL。早期方案中使用过“永久 URL”这个名称，不过这里的“永久”只表示地址不再跟随存储引擎的临时签名变化，并不表示文件永远存在、任何人都能访问，或最终的存储地址永远不变。
...
通过 [HTTP API](./http-api.md) 上传或查询文件时，响应中的 `url` / `preview` 不再暴露本地路径、对象存储域名或预签名下载地址。调用方访问稳定 URL 时需要携带对应应用的登录凭证，或满足 anonymous 角色对该文件记录的查看权限。
...
访客在公开表单中上传文件后，NocoBase 会在当前公开表单会话中记录这些文件。上传者可以继续看到自己刚上传的图片或附件，不需要开放整个附件表的 anonymous 查看权限。
...
2. 未登录用户按照 anonymous 角色的文件查看权限处理。
3. 公开表单等插件可以对特定文件提供额外的受限授权。
4. 检查通过后，NocoBase 返回 `302`，跳转到本地存储或对象存储生成的实际地址。

因此，稳定 URL 隔离了业务数据和存储实现。切换存储域名、更新对象存储签名或调整缩略图规则时，业务字段中保存的地址通常不需要跟着修改。

## 使用注意

- 稳定不等于公开。复制链接给其他人后，对方仍需要登录并拥有文件查看权限，或拥有 anonymous 访问权限
...
- 反向代理需要把 `APP_PUBLIC_PATH` 下的 `/files/` 路径转发到 NocoBase。使用 NocoBase CLI 生成的代理配置时会自动包含该规则
- 如果通过 `<img>`、`<iframe>`、`fetch()` 或第三方客户端访问文件，需要确认它会携带凭证或满足 anonymous 权限，并能跟随 `302` 重定向
