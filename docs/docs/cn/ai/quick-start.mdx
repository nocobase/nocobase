...
如果你此时想要 AI Agent 参与操作 NocoBase， 需要确保已经在 CLI 初始化的工作目录里安装了 NocoBase Skills（如果你在第一步的配置里勾选了安装 Skills，则 CLI 会自动帮你安装好）。如果没有安装，可以在 CLI 初始化的目录里执行：
...
如果你已经有 NocoBase 应用在运行，或者已经初始化成功过，想要继续开发、调试或者管理应用，所有 `nb` 命令的完整参数说明请参阅 [CLI 命令参考](../api/cli/cli)。
...
- [CLI 命令参考](../api/cli/cli) — 所有命令的完整参数说明
