...
| 应用运行管理 | 切换目录、修改 compose、执行 yarn/pm2 命令 | `nb app start/stop/restart/logs`、`nb env remove` |
...
| 停止应用和内置数据库 | `nb app stop -e app1 --with-db` |
| 彻底移除 env 和本机托管资源 | `nb env remove app1 --purge --force` |

如果你只是想把应用和 CLI 托管的内置数据库一起停掉，默认直接用 `nb app stop -e app1 --with-db` 就行。

如果你确定这个 env 已经不再需要了，再用 `nb env remove app1 --purge --force`。它会移除托管运行资源、storage 数据，以及适用时由 CLI 下载管理的本地 app 文件。
