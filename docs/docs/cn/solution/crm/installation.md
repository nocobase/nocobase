...
> **维护说明**:为方便统一维护,本方案已并入 [一体化业务管理系统](../all-in-one/installation.md),下载链接已切换到新系统的备份。
...
- 您已经下载了 CRM 系统的相应文件：
  ...
  - **SQL文件**：[nocobase_all_in_one_sql_260521.zip](https://static-docs.nocobase.com/nocobase_all_in_one_sql_260521.zip) - 适用于方法二
...
* **选项 A：通过服务器命令行（以 Docker 为例）**
  如果您使用 Docker 安装 NocoBase 和数据库，可以将 `.sql` 文件上传到服务器，然后使用 `docker exec` 命令来执行导入。假设您的 PostgreSQL 容器名为 `my-nocobase-db`，文件名为 `nocobase_all_in_one_sql_260521.sql`：
