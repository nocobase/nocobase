...
一体化业务管理系统涵盖 **CRM 客户管理、销售管理、工单系统、项目管理、固定资产管理、HR 人事管理** 六大模块。这里提供两种还原方式,根据你的 NocoBase 版本和技术背景选一种就行。
...
- 已下载一体化方案的备份文件之一:
  - **nbdata 备份**:[nocobase_all_in_one_backup_260521.nbdata](https://static-docs.nocobase.com/nocobase_all_in_one_backup_260521.nbdata) — 适用于方法一
  - **SQL 压缩包**:[nocobase_all_in_one_sql_260521.zip](https://static-docs.nocobase.com/nocobase_all_in_one_sql_260521.zip) — 适用于方法二
...
通常来说,有备份管理器插件就选方法一,没有就选方法二。当前版本采用**备份还原**形式部署,后续版本会换成增量迁移,方便接入已有的 NocoBase 系统。

---

## 方法一:备份管理器还原(推荐专业/企业版用户)

这种方式通过 NocoBase 内置的「[备份管理器](https://docs-cn.nocobase.com/handbook/backups)」插件一键还原,UI 操作最简单。不过对环境和插件依赖有比较严格的要求。

### 核心特点

**优点:**

- **操作便捷** — UI 界面完成,包括插件配置在内的所有内容都能还原
- **完整还原** — 能还原所有系统文件,包括模板打印文件、附件字段上传文件、AI 员工头像等

**局限:**

- **专业/企业版限定** — 备份管理器是企业级插件,社区版不可用
- **环境要求严格** — 数据库版本、大小写敏感设置等都必须与备份源高度兼容
- **插件依赖强** — 备份里包含的商业插件,本地环境里必须也有,否则还原会失败
...
- **商业插件匹配** — 本地必须先开好备份里用到的所有商业插件,否则还原会中断。一体化方案涉及的商业插件包括:备份管理器、邮件管理、审计日志、AI 员工等

---

## 方法二:直接导入 SQL 文件(通用,更适合社区版)

直接操作数据库还原数据,绕过备份管理器,没有版本和插件限制。
...
- **高兼容性** — 不依赖应用内的 dump 工具,只要能连数据库就行
- **高容错性** — 备份里包含的商业插件,本地没装不会启用,但不影响其他模块的正常使用

**局限:**

- **需要数据库操作能力** — 比如知道怎么执行一个 `.sql` 文件
- **系统文件丢失** — 这种方式会丢失所有系统文件,包括模板打印文件、附件字段上传文件、AI 员工头像等

### 操作步骤

**第 1 步:准备一个干净的数据库**

为即将导入的数据准备一个全新的空数据库(PostgreSQL 16)。

**第 2 步:把 `.sql` 文件导入数据库**

下载的 `nocobase_all_in_one_sql_260521.zip` 解压后得到 `.sql` 文件,导入到上一步准备好的数据库中。执行方式有两种:

**选项 A:服务器命令行(Docker 示例)**

如果 NocoBase 和数据库都用 Docker 部署,把 `.sql` 文件上传到服务器后用 `docker exec` 导入。假设 PostgreSQL 容器名为 `my-nocobase-db`:
...
**选项 B:远程数据库客户端(Navicat 等)**

如果数据库暴露了端口,用任意图形化客户端(Navicat、DBeaver、pgAdmin 等)连接,然后:
...
配置 NocoBase 启动参数(`DB_HOST`、`DB_PORT`、`DB_DATABASE`、`DB_USER`、`DB_PASSWORD` 等),指向刚导入了数据的数据库,然后正常启动 NocoBase 服务。

### 注意事项

- **数据库权限** — 这种方式要求拥有直接操作数据库的账号和密码
- **插件状态** — 导入成功后,商业插件数据虽然在,但本地没装对应插件时相关功能用不了。应用不会崩
...
| :--- | :--- | :--- |
| **适用用户** | 专业/企业版 | 所有用户(含社区版) |
| **操作简易度** | ⭐⭐⭐⭐⭐(UI 操作) | ⭐⭐⭐(需要数据库知识) |
| **环境要求** | 严格,数据库、系统版本需高度兼容 | 一般,数据库兼容即可 |
| **插件依赖** | 强依赖,缺插件会还原失败 | 数据可独立导入,缺插件相关功能用不了 |
| **系统文件** | 完整保留(打印模板、上传文件、头像等) | 会丢失(打印模板、上传文件、头像等) |
| **推荐场景** | 企业用户、环境可控 | 缺部分插件、追求兼容、社区版 |
...
可以直接用,不会报错。Demo 里用到一些企业版插件(邮件管理、审计日志、AI 员工等),专业版缺这些插件时对应入口不会显示,不影响其他模块。比如审计日志入口会消失,但 CRM、销售、工单、项目、资产、HR 等核心模块完全正常。
...
SQL 方式安装后,文件上传可能报 OSS 相关错误。进入「插件管理 / 文件管理器」,把「Local Storage(本地存储)」设为默认存储,保存后就能正常上传。
...
不要启用 `ar-SA`,目前会导致 NocoBase 渲染异常。

:::

### 增量升级怎么办?

目前版本升级是全量替换,自定义修改会被覆盖。升级前务必备份。增量迁移方案正在规划中,会优先支持专业/企业版。社区版由于缺迁移管理插件,暂时较难支持。
