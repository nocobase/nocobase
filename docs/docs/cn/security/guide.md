...
对于本地存储或其他可通过应用同源 URL 直接访问的存储，还需要额外注意“主动内容文件”带来的风险。比如 `html`、`xhtml`、`svg` 等文件可能在浏览器中被直接解析和执行。如果攻击者能够上传此类文件，并诱导用户打开文件链接，就可能借助应用的可信域名承载恶意页面或脚本。
...
如果已有系统仍依赖历史地址的匿名访问，可以设置 `LEGACY_LOCAL_STORAGE_PUBLIC_ACCESS=true` 并重启应用，以恢复 `/storage/uploads/` 的公开访问。该兼容开关默认关闭，仅影响历史地址，不会改变 `/files/` 的文件记录级权限。公开访问可能暴露已上传文件，启用前应确认其中不包含敏感内容。
...
- 优先使用私有存储、签名 URL，或独立文件域名，避免用户上传文件与应用主站同源直接访问。
- 对上传文件启用严格的 MIME type 白名单，仅允许业务实际需要的文件类型。
- 谨慎允许 `text/html`、`application/xhtml+xml`、`image/svg+xml` 等主动内容类型上传。即使系统会对这类文件尽量采用下载方式返回，也不应将其视为可替代上传限制和访问隔离的完整防护。
- 对外部反向代理、CDN、对象存储等静态文件分发层进行一致的安全配置，避免绕过应用层策略直接以内联方式返回危险文件。
- 不要把本地/public 存储用于托管不受信任的网页内容；如确有此类需求，应使用隔离域名，并单独评估 CSP、下载策略和访问控制。

如果管理员显式放开危险文件类型上传，应自行评估由此带来的钓鱼、同源脚本执行、敏感信息泄露等风险，并确保部署链路中的 Web Server、网关、CDN 和存储服务采取一致的限制策略。
