...
这一章，我们来给系统加上"门禁"：创建[角色](/users-permissions/acl/role)、配置[菜单权限](/users-permissions/acl/permissions)和[数据范围](/users-permissions/acl/permissions)，实现**不同的人，看到不同的菜单，操作不同的数据**。

## 5.1 理解[角色](/users-permissions/acl/role)（Role）

在 NocoBase 里，**角色就是一组[权限](/users-permissions/acl/role)的集合**。你不需要给每个用户单独配权限，而是先定义好几个角色，再把用户丢进对应的角色里。
...
关键概念：**[数据范围](/users-permissions/acl/permissions)（Data Scope）**。

在角色的权限配置中，切换到 **[数据表](/data-sources/data-modeling/collection)操作权限** 选项卡。找到我们的"工单"表，点击进入单独配置。
...
1. 找到"工单"表的 **查看** 权限
2. 数据范围选择 → **自己的数据**
3. 但这里有个细节——NocoBase 的"自己的数据"默认是按创建人过滤的。如果我们希望按"处理人"过滤，可以在全局[操作权限](/users-permissions/acl/permissions)里进一步调整，或者在前端页面用 **数据[区块](/interface-builder/blocks)的筛选条件** 配合实现
...
- [角色与权限](/users-permissions/acl/role) — 角色配置说明
- [数据范围](/users-permissions/acl/permissions) — 数据级权限控制
