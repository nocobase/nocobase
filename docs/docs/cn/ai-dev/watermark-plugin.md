...
<!-- 需要一张启用水印插件后的页面截图，能看到半透明水印覆盖在正常内容上方 -->

<!-- 需要一张插件设置页的截图，能看到水印文字、透明度、字号的配置表单 -->
...
3. 打开了支持 AI Agent 的编辑器（比如 Claude Code、Cursor 等）
...
```bash
yarn pm create @nocobase-sample/plugin-watermark
```

AI 在 `packages/plugins/@nocobase-sample/plugin-watermark/` 下生成了标准的插件目录结构。
...
```bash
yarn pm enable @nocobase-sample/plugin-watermark
```
