...
阅读本页前，请确保你已按照 [AI 开发快速开始](./index.md) 完成了环境准备。

:::

AI 插件开发的能力基于 [nocobase-plugin-development](https://github.com/nocobase/skills/tree/main/skills/nocobase-plugin-development) Skill 实现。如果你已经通过 [NocoBase CLI](../get-started/nocobase-cli) 初始化（`nb init`），这个 Skill 会自动安装。
