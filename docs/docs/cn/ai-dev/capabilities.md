...
```
在插件 @my-scope/plugin-order 里定义一张"订单"表，字段包括：订单编号（自增）、
客户名称（字符串）、金额（小数）、状态（单选：待处理/处理中/已完成）、创建时间。
订单和客户是多对一关系。
```
...
```
帮我做一个"评分"字段的插件，插件名叫 @my-scope/plugin-rating，
用星星图标展示，支持 1-5 分，点击可以修改评分值。
```
...
```
在插件 @my-scope/plugin-order 里，帮我在订单表格上加一个"批量导出"按钮，
点击后调用后端 API 导出选中的订单为 CSV 文件。
```
...
```
帮我给插件 @my-scope/plugin-notify 做一个设置页，让用户配置以下参数：
- API Key（字符串，必填）
- Webhook URL（字符串，选填）
- 是否启用通知（开关）
...
```
在插件 @my-scope/plugin-order 里，帮我写一个 API，路径是 order:statistics，
返回订单的统计数据（总数、总金额、按状态分组的数量）。只有登录用户可以访问。
```
...
```
插件 @my-scope/plugin-order 的权限要求：普通用户只能看自己创建的订单，
管理员可以看所有订单，只有管理员可以删除订单。
```
...
```
帮插件 @my-scope/plugin-order 额外生成日文语言包
```
...
```
插件 @my-scope/plugin-order 要升级，需要给"订单"表新增一个"备注"字段
（长文本，选填），并且把现有订单的备注字段默认填上"无"。
```
