# NocoBase 2.1.0-beta.24 发布日志

NocoBase 2.1.0-beta.24 是一次围绕 AI 能力全面升级的 Beta 版本。

这一版本的重点，是把 NocoBase 从“可配置的低代码平台”进一步推进为“可与 AI Agent 协同工作的业务系统平台”。从 CLI 接入、AI 搭建，到 AI 员工、MCP 和 Skills，2.1.0-beta.24 初步打通了 AI 参与系统搭建、业务执行与环境管理的完整链路，形成了从环境接入、系统搭建到业务协作的完整能力框架。

如果你正在关注这几个方向，尤其值得试一下：

- 想让 AI Agent 直接参与 NocoBase 的搭建和配置
- 想用自然语言完成数据建模、页面搭建、工作流编排和权限配置
- 想在业务系统中引入能执行任务的 AI 员工
- 想通过 CLI 和标准化能力包提升团队协作效率

## 版本亮点

### NocoBase CLI 正式成为 AI Agent 接入入口

在 NocoBase 2.1.0-beta.24 中，NocoBase CLI（`nb`）是 AI Agent 连接 NocoBase 的核心入口。
...
为了让 AI 真正理解 NocoBase 的配置体系，这个版本提供了一组可安装到 AI Agent 中的领域知识包——NocoBase Skills。
...
AI 员工在之前的版本里已经存在。不过在 NocoBase 2.1.0-beta.24 中，AI 员工相关能力得到了增强，AI 内核也做了补齐：
...
如果你的团队需要在业务系统中引入智能协作、自动分析和任务执行能力，这是 2.1.0-beta.24 最值得关注的方向之一。
...
AI 之外，NocoBase 2.1.0-beta.24 也继续补充了面向实际业务场景的配置能力：
...
NocoBase 2.1.0-beta.24 适合这几类用户优先体验：

- 正在探索 AI Agent 如何参与业务系统搭建的团队
- 想提升实施效率、减少重复配置的交付团队
- 想把 AI 引入业务流程、报表生成和任务执行环节的企业用户
- 正在评估 NocoBase AI 能力路线的技术负责人和产品负责人

如果你已经在用 NocoBase 2.0，并且想引入 AI 协作能力，2.1.0-beta.24 的方向很明确：从“人工配置系统”走向“人和 AI 一起搭建和运行系统”，值得升级体验。