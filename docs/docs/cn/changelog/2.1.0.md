...
这个版本还新增了独立构建的 `/v2/` 分支，这个分支里将不再能创建 v1 的页面，只能创建和使用 2.0 的区块和页面。

你可以通过访问 `/v2/` 路径（比如 `http://localhost:13000/v2/`）来进入新的路由体系，体验基于只有 2.0 版本区块和页面的界面。目前这个部分还在适配中，功能还不完善，可以尝鲜使用。后续我们会逐步将更多核心功能和插件适配到 V2 路由中来。
