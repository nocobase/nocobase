...
本文内容适用于 NocoBase 2.1.0 及以上版本。

NocoBase 2.1.0 是一次**围绕 AI 能力与 V2 适配**的重要升级。在 2.1.0-beta 把 AI Agent 接入 NocoBase 的完整链路（CLI 接入、AI 搭建、AI 员工、AI 开发插件）打磨稳定之后，正式版在延续这条主线的同时，重点补齐了两个方向：

- **更完善的 AI 与系统能力**：新增 NocoBase CLI 的备份、发布等运维能力、本地化工程师 AI 员工 Lina、多应用单点登录（App SSO）、V2 安全与认证体系等。
- **更广泛的 V2 适配**：大量区块、字段、操作以及认证、管理后台插件完成了 client-v2（FlowEngine）适配，目前 V2 相关的适配已经能够进入 beta 阶段。

## 新特性

### NocoBase CLI 正式成为普通用户和 AI Agent的统一入口
...
正式版还补齐了一批运维相关命令与认证方式：

- 新增 `backup create`、`backup restore` 命令，支持通过 CLI 进行备份与恢复
- 新增发布相关命令，支持跨环境发布流程
- 支持 Basic Auth
- 补齐 `api` 命令的兼容规则
- 优化 `app upgrade` 流程、env 认证流程与 CLI 更新策略
...
App SSO 为 **多应用部署提供应用间自动登录能力**。用户在一个应用完成登录后，可以自动登录到其他关联应用，无需重复认证，适用于多应用、多环境的统一接入场景。

- 新增用于应用之间自动登录的 App SSO 插件
- 应用监管器支持配置子应用的 App SSO
...
在 2.1.0-beta 的基础上，又有一批区块、字段、操作以及认证、管理后台插件完成了 V2（client-v2 / FlowEngine）适配。
...
- [编码字段](https://docs.nocobase.com/cn/data-sources/field-sequence)
...
- [安全菜单与 Token 策略设置页](https://docs.nocobase.com/cn/security/token-policy)
- [修改密码（用户中心）](https://docs.nocobase.com/cn/users-permissions/user)
- [用户与角色权限设置页](https://docs.nocobase.com/cn/users-permissions/acl/role)
- [主题编辑器](https://docs.nocobase.com/cn/system-management/theme-editor)
- [Office 文件预览](https://docs.nocobase.com/cn/file-manager/file-preview/ms-office)
- [通知管理](https://docs.nocobase.com/cn/notification-manager)
- [文件管理器](https://docs.nocobase.com/cn/file-manager)
- [嵌入 V2 页面](https://docs.nocobase.com/cn/integration/embed)、[公开表单 V2 页面](https://docs.nocobase.com/cn/plugins/@nocobase/plugin-public-forms)
- [表单提交操作字段赋值](https://docs.nocobase.com/cn/interface-builder/actions/action-settings/assign-values)

认证与安全

- [用户认证设置页](https://docs.nocobase.com/cn/auth-verification/auth)
- [OIDC](https://docs.nocobase.com/cn/auth-verification/auth-oidc) / [LDAP](https://docs.nocobase.com/cn/auth-verification/auth-ldap) / [SAML 2.0](https://docs.nocobase.com/cn/auth-verification/auth-saml) / [CAS](https://docs.nocobase.com/cn/auth-verification/auth-cas) 认证
- [验证（短信验证码等）](https://docs.nocobase.com/cn/auth-verification/verification)
- [双因素身份认证（2FA）](https://docs.nocobase.com/cn/auth-verification/2fa)
- [密码策略](https://docs.nocobase.com/cn/security/password-policy)
- [自定义品牌](https://docs.nocobase.com/cn/system-management/custom-brand)
- [IP 限制](https://docs.nocobase.com/cn/security/ip-restriction)
- [授权与商业授权设置](https://docs.nocobase.com/cn/plugins/@nocobase/plugin-license)
