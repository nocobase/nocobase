...
NocoBase 2.1 是对 **AI 能力、多应用以及 2.0 版本适配** 的一次重要升级。我们推出了 NocoBase CLI，方便人和 AI Agent 连接和管理 NocoBase 应用。从创建应用、AI 搭建、AI 员工能力增强、到 AI 开发插件等，覆盖了环境接入、系统搭建和业务协作的完整流程。同时，我们继续大幅补齐 2.0 页面与核心能力的适配——更多区块、字段、操作和插件支持了 2.0 版本，并增加了版本管理功能。
...
版本管理功能帮助用户更好地管理应用的版本迭代和变更历史。用户可以通过版本管理功能创建、查看和恢复应用的不同版本，方便在开发和运维过程中进行版本切换和问题恢复。
