...
NocoBase CLI（`nb` 命令）是 NocoBase 的命令行管理工具，覆盖从安装、环境管理到 API 操作的完整流程。配合 [NocoBase Skills](../../ai-builder/index.md)，还可以让 AI Agent（比如 Claude Code、Cursor、Codex 等）理解并操作 NocoBase。
...
详细安装流程请参阅 [NocoBase CLI 快速开始](../../get-started/nocobase-cli)。
...
安装完成后，运行 `nb init` 初始化项目——过程中会引导你安装 NocoBase Skills、配置数据库并启动应用。所有命令都支持 `nb <command> --help` 查看完整参数说明。
...
- [NocoBase CLI 快速开始](../../get-started/nocobase-cli) — 安装引导和使用入门
