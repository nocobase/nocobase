---
title: "nb portal"
description: "nb portal 命令参考：管理 Portal 本地源码目录，包括配置、创建、开发、查看、拉取、推送、部署和删除。"
keywords: "nb portal,NocoBase CLI,Portal,多入口,本地源码目录,源码同步,部署"
---

# nb portal

管理 Portal 本地源码目录。Portal 可以有独立的前端源码、入口和部署结果，`nb portal` 负责把 NocoBase 中的 Portal 记录、本地源码目录和 source storage 串起来。

通常来说，Portal 的完整流程是：先创建本地源码目录，再进入开发模式；开发完成后，把源码推送到 source storage，最后构建并部署。如果你接手的是已有 Portal，则可以先 `pull` 到本地，再继续开发。
...
| 命令 | 说明 |
| --- | --- |
| [`nb portal config`](./config.md) | 更新 Portal 本地源码目录的源码配置，并尽量同步到远端 Portal 记录 |
| [`nb portal create`](./create.md) | 基于模板创建 Portal 本地源码目录，并创建或更新 Portal 记录 |
| [`nb portal deploy`](./deploy.md) | 构建并部署指定 Portal 本地源码目录 |
| [`nb portal destroy`](./destroy.md) | 删除 Portal 记录和本地源码目录 |
| [`nb portal dev`](./dev.md) | 启动指定 Portal 本地源码目录的开发模式 |
| [`nb portal info`](./info.md) | 查看指定 Portal 记录和本地源码目录详情 |
| [`nb portal list`](./list.md) | 列出 Portal 记录和本地源码目录同步状态 |
| [`nb portal pull`](./pull.md) | 从 source storage 拉取 Portal 源码到本地源码目录 |
...
源码配置会写入本地源码目录的 `portal.config.json`。`create`、`pull` 和 `config` 都会维护这个文件；`push` 和 `deploy` 会读取它，并按配置同步源码或部署产物。
...
| env 类型 | 说明 |
| --- | --- |
| `local` | 本地源码目录和应用 storage 在当前机器上，`pull`/`push` 对默认 `nocobase` 存储通常不需要做额外同步 |
| `docker` | 本地源码目录通过 Docker volume 和应用共享，`pull`/`push` 对默认 `nocobase` 存储通常不需要做额外同步 |
| `http` | 通过 API 同步源码和部署产物，`pull`/`push` 会下载或上传源码归档 |

`ssh` env 在当前版本暂不支持 Portal 本地源码目录管理。

## 本地源码目录路径

Portal 本地源码目录会放在当前 env 的 storage 下：
