...
description: "nb portal deploy 命令参考：构建并部署指定 Portal 本地源码目录。"
keywords: "nb portal deploy,NocoBase CLI,Portal,构建,部署"
---

# nb portal deploy

构建并部署指定 Portal 本地源码目录。通常在本地开发完成，并且需要把 Portal 更新到目标 env 时使用。

执行时会先刷新本地源码目录里的 `.env` 和 `.env.local`，再运行 `pnpm build`。构建产物需要包含 `dist/index.html`。
...
`deploy` 面向已经存在的 Portal 本地源码目录。如果本地还没有源码目录，先使用 [`nb portal create`](./create.md) 创建，或者使用 [`nb portal pull`](./pull.md) 从 source storage 拉取。

对于 `local` 和 `docker` env，部署会同步 Portal 记录，并直接使用本地或 volume 中的 `dist` 目录。对于 `http` env，CLI 会把 `dist` 打包上传到目标 NocoBase，再同步 Portal 记录。

部署时会读取本地源码目录的 `portal.config.json`，并把源码配置同步到远端 Portal 记录。
