...
description: "nb portal config 命令参考：更新 Portal 本地源码目录的 source storage 和 Git 源码配置。"
keywords: "nb portal config,NocoBase CLI,Portal,source storage,Git,portal.config.json"
---

# nb portal config

更新 Portal 本地源码目录的源码配置。它会写入本地源码目录的 `portal.config.json`，如果远端 Portal 记录存在，也会把配置同步到远端记录。
...
`config` 要求 Portal 本地源码目录已经存在。如果本地还没有源码目录，先使用 [`nb portal create`](./create.md) 创建，或者使用 [`nb portal pull`](./pull.md) 拉取已有源码。
