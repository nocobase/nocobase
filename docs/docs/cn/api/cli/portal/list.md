...
列表会显示名称、访问 URL、开发模式、source storage、本地路径、启用状态和本地同步状态。只有 `developmentMode` 为 `vibe-coding` 的 Portal 才会检查本地工作区；其他类型的 Portal，本地同步状态为空。

`--json-output` 输出的字段包括 `name`、`url`、`developmentMode`、`localPath`、`enabled`、`sourceStorage` 和 `localSynced`。
