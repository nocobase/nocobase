...
description: "nb app upgrade 命令参考：先停止应用，再下载并替换源码或镜像，然后重新启动指定 NocoBase 应用。"
...
如果传入 `--skip-download`，CLI 会跳过第 2 步和第 3 步，直接重启当前已保存的源码或镜像。此时如果同时传入 `--version`，CLI 不会在本次升级中下载该版本，而是在成功后仅把它保存为新的 `downloadVersion`，供后续升级或恢复使用。

第 4 步会等待应用通过 `__health_check`。等待期间，CLI 会先输出一条等待日志，之后每 10 秒输出一条进度提示，直到应用就绪或超时。
