...
只需两步，让 AI 帮你完成从安装到启动的全部流程。

### 第一步：安装 Skills

先安装 [NocoBase Skills](https://github.com/nocobase/skills)——这是 AI 理解 NocoBase 的基础：
...
将下方提示词复制给你的 AI 助手（Claude Code、Codex、Cursor、Trae 等），它会自动完成 CLI 安装、环境配置和应用启动：
...
确认用户已安装 Node.js v20 及以上版本（运行 `node -v` 检查）。如果没有，引导用户通过 [nvm](https://github.com/nvm-sh/nvm)、[fnm](https://github.com/Schniz/fnm) 或 [Node.js 官网](https://nodejs.org/) 安装。

### Step 2：安装 Skills 和 CLI

先确认 [NocoBase Skills](https://github.com/nocobase/skills) 是否已安装——这是 AI 搭建和 AI 开发的基础，后续步骤都依赖它。如果已安装则跳过，如果没有则执行安装：
...
安装 Skills 后，需要提示用户重启 AI Agent（比如重启 Claude Code 会话、重新打开 Cursor 等），Skills 才会被加载生效。重启后再继续后续步骤。
...
运行 `nb --version` 确认安装成功。然后创建项目目录：

```bash
mkdir my-nocobase && cd my-nocobase
```

### Step 3：询问用户是否已有 NocoBase

向用户确认：**你是否已经有一个运行中的 NocoBase？**

- **已有** → 跳转到 [Step 6：连接已有环境](#step-6连接已有环境)
- **没有** → 继续 Step 4
...
如果用户已经有运行中的 NocoBase，不需要执行 Step 4-5，直接添加远程环境。
