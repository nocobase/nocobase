...
如果你用的是内置 Nginx 镜像，通常来说不建议直接把 `13000` 暴露给公网。更常见的做法是把容器端口保留在宿主机本地，再由宿主机上的 Nginx 统一处理域名、HTTPS 和反向代理。
...
内置 Nginx 会在返回历史 `/storage/uploads/` 文件前调用 NocoBase 检查登录状态，匿名访问默认返回 `401`。如果升级前已有程序依赖这些历史地址的匿名访问，可以在 `app.environment` 中增加 `LEGACY_LOCAL_STORAGE_PUBLIC_ACCESS=true` 并重启容器。宿主机 Nginx 只需继续把该路径转发到容器，不要另外使用 `alias` 直接暴露宿主机上的上传目录，否则会绕过这一检查。
...
这里的路径要保留前后的 `/`。配置完成后，应用的访问地址会变成 `http://127.0.0.1:13000/nocobase/`。
...
如果你不想使用内置 Nginx，也可以看这两种单独的代理方案：

- [Docker 安装（外部 Nginx）](./docker-nginx.mdx)
- [Docker 安装（外部 Caddy）](./docker-caddy.mdx)
