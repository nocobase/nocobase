...
内置 Nginx 会在返回历史 `/storage/uploads/` 文件前调用 NocoBase 检查登录状态，匿名访问默认返回 `401`。如果升级前已有程序依赖这些历史地址的匿名访问，可以在 `app.environment` 中增加 `LEGACY_STORAGE_PUBLIC_ACCESS=true` 并重启容器。宿主机 Nginx 只需继续把该路径转发到容器，不要另外使用 `alias` 直接暴露宿主机上的上传目录，否则会绕过这一检查。
