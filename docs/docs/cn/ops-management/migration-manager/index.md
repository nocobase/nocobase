...
1.  **仅结构：** 只同步数据表结构，不涉及数据的插入或更新。
2.  **覆盖（清空并重新插入）：** 清空现有表记录，然后插入新数据。
3.  **插入或更新 (Upsert)：** 根据主键判断，记录存在则更新，不存在则插入。
4.  **插入时忽略重复：** 插入新记录，如果主键冲突则忽略（不更新现有记录）。
5.  **跳过：** 对该表不做任何处理。
...
- 如果需要在新环境保留这些文件，您需要手动拷贝 `storage` 目录下的相关文件夹。
...
- **关闭网关/入口：** 强烈建议在开始备份或迁移前停止用户访问。您可以通过 Nginx 或网关配置 **503 维护页面**，向用户提示系统正在维护中，并防止新的数据写入。
