---
title: "界面搭建概述"
description: "NocoBase 界面搭建：所见即所得配置模式，桌面端与移动端 UI 布局、页面、区块、字段和操作配置。"
keywords: "界面搭建,所见即所得,UI 布局,桌面端布局,移动端布局,区块,页面配置,NocoBase"
---

# 界面搭建

## 所见即所得

NocoBase 采用所见即所得的界面搭建模式。点击界面搭建按钮，可以在配置模式和使用模式之间切换。

使用模式：

![20251023215845](https://static-docs.nocobase.com/20251023215845.png)

配置模式：

![20251023215951](https://static-docs.nocobase.com/20251023215951.png)

## UI 布局

NocoBase 提供桌面端布局和移动端布局。桌面端布局适合常规管理操作，也能在窄屏中自动调整导航和页面内容；移动端布局提供独立的移动导航和页面配置。详细介绍见 [UI 布局](./ui-layout/index.md)。

<!-- 需要一张同一业务页面的桌面端和移动端并排对比截图，并突出两种布局的导航差异 -->

## 配置项

进入界面搭建模式后，界面上会出现橙色的可配置项。通常，每个可配置元素的配置项入口位于该元素的右上角。几乎所有元素都可以在界面上实时配置和查看效果，以下是几个示例：

菜单的配置项：

![20251023221617](https://static-docs.nocobase.com/20251023221617.png)

页面的配置项：

![20251023221649](https://static-docs.nocobase.com/20251023221649.png)

区块的配置项：

![20251023221748](https://static-docs.nocobase.com/20251023221748.png)

操作的配置项：

![20251023222109](https://static-docs.nocobase.com/20251023222109.png)

表格列的配置项：

![20251023221814](https://static-docs.nocobase.com/20251023221814.png)

## 相关链接

- [UI 布局](./ui-layout/index.md) — 了解桌面端布局和移动端布局
