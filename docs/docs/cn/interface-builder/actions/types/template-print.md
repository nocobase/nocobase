...
条形码的生成方式与[动态图片](/handbook/action-template-print#动态图片)相同，只需三步：

1. 在模板中插入一张临时图片，用于标记条码的位置

2. 编辑图片的“替代文字”，写入条码格式字段标签，例如 `{d.code:barcode(qrcode)}`，其中 `qrcode` 是条码的类型（详见下方[支持列表](/handbook/action-template-print#支持的条码类型)）
