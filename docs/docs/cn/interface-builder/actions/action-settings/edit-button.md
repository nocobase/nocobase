---
title: "编辑按钮"
description: "操作设置：配置操作按钮的显示文本、图标、样式等。"
keywords: "编辑按钮,按钮配置,操作按钮,界面搭建,NocoBase"
---
...
- **按钮类型（Button Type）**：按钮支持不同的类型设置，常见的按钮类型包括：
  ...
  - **链接类型（Link）**：以超链接样式显示的按钮，常用于轻量级行操作。
  - **文本类型（Text）**：只有文本没有背景色的按钮，通常用于轻量级操作或链接。

表格 **操作** 列中的行操作支持以上所有按钮类型。**Primary**、**Default** 和 **Dashed** 在单元格内渲染为带水平内边距的紧凑型填充按钮。**Link** 和 **Text** 仍保持无背景的紧凑文本样式。
