...
「当前项」用于子表单/子表格等“列表项”场景，表示当前行的数据对象。

常见使用场景：

- 子表单/子表格的字段值配置（例如默认值/固定值）
- 子表单联动规则
- 子关系字段的数据范围

你还可以使用「索引（从0开始）」获取当前项在列表中的位置，用于按行号计算或区分不同项。

![变量-当前项与索引（从0开始）](https://static-docs.nocobase.com/placeholder.png)

### 上级项

「上级项」表示当前项的父级对象（例如父表单记录或上一级关联对象）。

常见使用场景：

- 子表单字段值引用父表单字段（例如上级项/客户、上级项/项目等）
- 子表单联动规则中读取父级信息

![变量-上级项](https://static-docs.nocobase.com/placeholder.png)
