...
- Radio group：单选框
- Checkbox group：多选框
- Record select：关系记录选择器，用于筛选关联数据

#### Record select（自定义关系字段）

`Record select` 适合用于“按关联表记录筛选”的场景。比如在订单列表中，按“客户”筛选订单，或在任务列表中按“负责人”筛选任务。

配置项说明：

- **Target collection**：目标数据表。表示要从哪个数据表加载可选记录。
- **Title field**：标题字段。用于下拉选项和已选标签的展示文本（如名称、标题）。
- **Value field**：值字段。用于实际筛选时提交的值，通常选择主键字段（如 `id`）。
- **Multiple**：是否允许多选。开启后可以一次选择多条记录。
- **Operator**：操作符。定义筛选条件如何匹配（见下方 Operator 说明）。

推荐配置：

1. `Title field` 选择可读性高的字段（如“名称”），避免使用纯 ID 影响可用性。
2. `Value field` 优先选择主键字段，保证筛选稳定且唯一。
3. 单选场景通常关闭 `Multiple`，多选场景开启 `Multiple` 并配合合适的 `Operator`。

#### Operator（操作符）

`Operator` 用于定义“筛选表单字段值”与“目标区块字段值”的匹配关系。

常见理解方式：

- **等于 / 不等于**：常用于单值匹配（如选择一个客户）。
- **包含 / 不包含**：常用于文本或集合类型匹配。
- **属于（in）/ 不属于（not in）**：常用于多值匹配（如多选多个客户后筛选命中任一客户）。

选择建议：

1. 字段为单选时，优先选择“等于”。
2. 字段为多选时，优先选择“属于（in）”。
3. 若筛选结果异常，先检查 `Multiple` 与 `Operator` 是否匹配，再检查 `Connect fields` 的关联字段是否正确。
