...
description: "AppSupervisor 多应用管理：单应用、共享内存多应用、多环境混合部署，SaaS/多租户场景，业务模块拆分，应用创建、启动、停止、生命周期管理。"
keywords: "多应用,Multi App,AppSupervisor,共享内存,多环境部署,SaaS,多租户,业务模块拆分,NocoBase"
---
# 多应用管理
...
- 将一个大型系统按业务模块拆分为多个应用，例如 CRM、售后、运营后台、数据分析等模块分别独立维护
- 团队按业务线或模块并行开发，降低不同模块之间的配置和发布影响
- 为不同客户、租户或组织创建独立应用，实现数据和配置隔离
- 为演示、测试或沙箱环境批量创建独立应用
- 在应用数量增多或负载升高后，将不同应用分配到不同运行环境中
