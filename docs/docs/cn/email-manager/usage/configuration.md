...
在配置页面，点击 **Add block** 按钮，选择 **Mail messages (All)** 或 **Mail messages (Personal)** 区块以添加邮件消息区块：

* **Mail messages (All)** ：显示所有邮件消息  
* **Mail messages (Personal)**：显示当前登录人的邮件消息  
...
点击表格右侧的配置，选择 **Set the data scope**，可以设置筛选邮件的数据范围。  
...
首先在邮件消息区块字段上开启 **Enable click to open** 功能：  
...
邮件表单的每个字段与普通表单一致，可配置 **默认值** 或 **校验规则** 等。
