...
他们会自动获取数据作为上下文，比如表格区块上的 Viz 自动获取表格里的数据，并调用合适的工具对数据进行处理，这意味着你不需要复制这些数据发给聊天机器人。 
...
他们也能自动获取页面结构作为上下文，比如表单区块上的 Dex 自动获取表单的字段结构，并调用合适的工具对页面进行操作，这意味着你不需要从聊天机器人把数据复制回来。
...
你也可以直接选取页面中的元素发给它们，他们将会从中获取到对应的数据和页面结构。 
...
你可以为每个 AI 员工在当前位置上预设任务，这样用户就可以点击触发，甚至不需要输入一个字。
