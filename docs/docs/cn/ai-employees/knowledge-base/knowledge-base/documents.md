...
如果上传 ZIP 时遇到文件过大的错误，可以到「File manager」里检查对应文件存储的上传大小限制。知识库上传会使用知识库设置中的「File storage」，所以需要调整的是同一个文件存储。
...
点击列表上方的「Vectorization」可以对选中的文档重新执行向量化。没有选择文档时，会对当前知识库内的文档执行向量化。
