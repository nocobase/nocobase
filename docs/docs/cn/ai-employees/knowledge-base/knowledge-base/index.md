...
| External | 文档、向量数据和检索逻辑都由外部系统负责，NocoBase 不能直接维护文档或向量数据。开发者需要提供插件，在插件中实现外部知识库的检索逻辑，比如连接 NocoBase 暂不支持的向量数据库，或调用第三方检索 API，详细开发方式见 [External 知识库插件](../dev/external-knowledge-base) |
