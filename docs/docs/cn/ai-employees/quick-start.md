...
1.  进入 **系统设置** -> **AI 员工** 列表页。
2.  找到你需要的 **AI 员工**， 例如 **Viz (数据分析师)**。
3.  点击 **编辑**，在 **LLM 设置** 中选择刚才配置的模型服务商 (LLM Service) 和 模型 (Model)，推荐使用：**gemini-2.5**、**deepseek**、**qwen3-max**
...
2.  尝试发送指令： ```"请按 leads 来源分析线索的数量与质量（高意向比），并提供 2–3 条渠道优化建议。"```

你还可以 **添加上下文** 、**添加附件** 等让 AI 更好的理解当前环境和需求。
