---
title: "自定义品牌"
description: "自定义品牌：配置登录页底部品牌文字、右上角「关于」菜单、网站 favicon，支持 HTML 格式，Logo 在系统设置中配置。"
keywords: "自定义品牌,品牌配置,favicon,关于菜单,登录页品牌,系统管理,NocoBase"
---

# 自定义品牌

## 介绍

系统默认的品牌名称是 "NocoBase"，主要显示在以下三个位置：

![20240409111100](https://static-docs.nocobase.com/20240409111100.png)

登录页面底部：

![20240409111254](https://static-docs.nocobase.com/20240409111254.png)

位置 1 可通过[系统设置](/system-management/system-settings/index.md)中的 Logo 选项进行配置；位置 2 和 3 可以通过本插件进行配置。

## 使用说明

插件激活后，点击页面右上角的「自定义品牌」菜单进入配置页面。

![20240409113640](https://static-docs.nocobase.com/20240409113640.png)

- **品牌**：用于设置登录页面底部的文字（位置 3）
- **关于**：用于设置页面右上角的菜单内容（如下图所示）

![20240409113915](https://static-docs.nocobase.com/20240409113915.png)

配置支持使用 HTML 格式，便于灵活设置内容。如果只需简单修改文字，可以直接在现有内容基础上进行修改，如下图所示：

![20240409114740](https://static-docs.nocobase.com/20240409114740.png)

设置网站图标 favicon

![20250601143958_rec_](https://static-docs.nocobase.com/20250601143958_rec_.gif)
