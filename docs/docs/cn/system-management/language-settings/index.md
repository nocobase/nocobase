---
title: "语言设置"
description: "语言设置：在系统设置中配置启用的语言，列表首项为默认语言，用户可在个人中心设置语言偏好，配合本地化管理补充翻译。"
keywords: "语言设置,启用语言,默认语言,个人语言偏好,多语言,i18n,系统管理,NocoBase"
---

# 语言设置

在「系统设置」中，通过配置「启用的语言」，可以调整系统的全局语言环境，列表中的第一个语言将作为默认语言。

![20240324192005](https://static-docs.nocobase.com/20240324192005.png)

当启用多种语言时，用户可以在个人中心（页面右上角）设置当前用户的个人语言偏好。

![20240324192509](https://static-docs.nocobase.com/20240324192509.png)

如果当前使用的语言环境缺少翻译，可以使用「[本地化管理](/system-management/localization/index.md)」插件进行翻译和补充。

![20240324193055](https://static-docs.nocobase.com/20240324193055.png)
