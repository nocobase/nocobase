:::tip Avviso di traduzione IA
Questa documentazione è stata tradotta automaticamente dall'IA.
:::


# Collezione di File

## Introduzione

Una collezione di file è una collezione dedicata specificamente alla memorizzazione dei metadati dei file, con campi predefiniti come nome del file, dimensione, tipo e percorso di archiviazione.

## Creare una Collezione di File

![20240324090414](https://static-docs.nocobase.com/20240324090414.png)

## Campi Predefiniti nella Collezione di File

![20240324090527](https://static-docs.nocobase.com/20240324090527.png)

## Utilizzo in un Blocco

![20240324090848](https://static-docs.nocobase.com/20240324090848.png)

## Campo di Associazione

![20240324091529](https://static-docs.nocobase.com/20240324091529.png)

## Blocco di Associazione

![20240324091321](https://static-docs.nocobase.com/20240324091321.png)