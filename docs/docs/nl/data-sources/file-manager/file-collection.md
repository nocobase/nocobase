---
pkg: "@nocobase/plugin-file-manager"
---

# Bestands-collectie

## Introductie

Bestanden zijn records in een speciaal gestructureerde collectie, die we een bestands-collectie noemen. Deze wordt gebruikt om metadata van bestanden op te slaan en kan worden beheerd via de Bestandsbeheerder.

## Gebruikershandleiding

Een Bestands-collectie aanmaken

![20240324090414](https://static-docs.nocobase.com/20240324090414.png)

Vooraf ingestelde velden in een Bestands-collectie

![20240324090527](https://static-docs.nocobase.com/20240324090527.png)

Gebruik in blokken

![20240324090848](https://static-docs.nocobase.com/20240324090848.png)

Relatieveld

![20240324091529](https://static-docs.nocobase.com/20240324091529.png)

Relatieblokken

![20240324091321](https://static-docs.nocobase.com/20240324091321.png)