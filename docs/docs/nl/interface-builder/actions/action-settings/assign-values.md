:::tip AI-vertaalmelding
Deze documentatie is automatisch vertaald door AI.
:::

# Veldtoewijzing

## Introductie

Veldtoewijzing houdt in dat u de waarde van een veld vooraf instelt voordat u een actie uitvoert. Wanneer u een record bijwerkt of aanmaakt, worden deze gegevens automatisch ingevuld, zonder dat u ze handmatig hoeft in te voeren. Denk hierbij aan scenario's zoals het in bulk bijwerken van orderstatussen, of het automatisch instellen van de levertijd op de huidige tijd wanneer een bestelling is afgeleverd.

![20251029143712](https://static-docs.nocobase.com/20251029143712.png)

![20251029144011](https://static-docs.nocobase.com/20251029144011.png)

![20251029144142](https://static-docs.nocobase.com/20251029144142.png)

## Gebruik

### Veld configureren

U kunt alleen velden configureren van de collectie van het huidige blok.

![20251029144652](https://static-docs.nocobase.com/20251029144652.png)

### Constante

![20251029144243](https://static-docs.nocobase.com/20251029144243.png)

### Variabele

![20251029144547](https://static-docs.nocobase.com/20251029144547.png)

Meer informatie over variabelen vindt u in [Variabelen](/interface-builder/variables)