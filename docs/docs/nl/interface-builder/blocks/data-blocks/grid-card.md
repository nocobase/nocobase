pkg: "@nocobase/plugin-block-grid-card"
---
# Rasterkaart

## Introductie

Het Rasterkaart-blok toont samenvattende informatie van datarecords in een kaartformaat. U kunt het aantal kolommen configureren voor verschillende schermformaten, om zo een gebruiksvriendelijke weergave op diverse apparaten te garanderen.

## Blokinstellingen

![20251023205037](https://static-docs.nocobase.com/20251023205037.png)

### Gegevensbereik

![20251023205425](https://static-docs.nocobase.com/20251023205425.png)

Voor meer informatie, zie [Gegevensbereik instellen](/interface-builder/blocks/block-settings/data-scope)

### Het aantal kolommen per rij instellen

![20251023210137](https://static-docs.nocobase.com/20251023210137.gif)

U kunt het aantal kolommen configureren voor verschillende schermformaten.

![20251023210303](https://static-docs.nocobase.com/20251023210303.png)

## Velden configureren

### Velden uit deze collectie

![20251023210533](https://static-docs.nocobase.com/20251023210533.png)

### Velden uit gerelateerde collecties

![20251023210629](https://static-docs.nocobase.com/20251023210629.png)

Voor de veldinstellingen van het Rasterkaart-blok, zie [Detailveld](/interface-builder/fields/generic/detail-form-item)

## Acties configureren

### Globale acties

![20251023211330](https://static-docs.nocobase.com/20251023211330.png)

- [Filteren](/interface-builder/actions/types/filter)
- [Nieuw toevoegen](/interface-builder/actions/types/add-new)
- [Verwijderen](/interface-builder/actions/types/delete)
- [Vernieuwen](/interface-builder/actions/types/refresh)
- [Importeren](/interface-builder/actions/types/import)
- [Exporteren](/interface-builder/actions/types/export)
- [Sjabloon afdrukken](/template-print/index)
- [Bulk bijwerken](/interface-builder/actions/types/bulk-update)
- [Bijlagen exporteren](/interface-builder/actions/types/export-attachments)
- [Workflow activeren](/interface-builder/actions/types/trigger-workflow)
- [AI-medewerker](/interface-builder/actions/types/ai-employee)
- [JS Actie](/interface-builder/actions/types/js-action)

### Rij-acties

![20251023211054](https://static-docs.nocobase.com/20251023211054.png)

- [Bewerken](/interface-builder/actions/types/edit)
- [Verwijderen](/interface-builder/actions/types/delete)
- [Koppelen](/interface-builder/actions/types/link)
- [Pop-up](/interface-builder/actions/types/pop-up)
- [Record bijwerken](/interface-builder/actions/types/update-record)
- [Sjabloon afdrukken](/template-print/index)
- [Workflow activeren](/interface-builder/actions/types/trigger-workflow)
- [AI-medewerker](/interface-builder/actions/types/ai-employee)
- [JS Actie](/interface-builder/actions/types/js-action)