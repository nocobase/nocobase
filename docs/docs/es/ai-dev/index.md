...
- NocoBase está migrando de `client` (v1) a `client-v2`. Actualmente `client-v2` aún está en desarrollo. El código de cliente generado por el desarrollo con AI se basa en `client-v2` y solo puede usarse en la ruta `/studio/`. Está disponible para que lo pruebe, pero no se recomienda su uso directo en producción.
