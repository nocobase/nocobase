---
pkg: "@nocobase/plugin-file-manager"
---

pkg: "@nocobase/plugin-file-manager"
---

# Colección de Archivos

## Introducción

Los archivos son registros dentro de una colección con una estructura especial, conocida como colección de archivos. Se utiliza para almacenar los metadatos de los archivos y se puede gestionar a través del Gestor de Archivos.

## Manual de Usuario

Crear una Colección de Archivos

![20240324090414](https://static-docs.nocobase.com/20240324090414.png)

Campos Preestablecidos en una Colección de Archivos

![20240324090527](https://static-docs.nocobase.com/20240324090527.png)

Uso en Bloques

![20240324090848](https://static-docs.nocobase.com/20240324090848.png)

Campo de Asociación

![20240324091529](https://static-docs.nocobase.com/20240324091529.png)

Bloques de Asociación

![20240324091321](https://static-docs.nocobase.com/20240324091321.png)