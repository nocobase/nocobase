...
- `./storage` debe montarse tanto en `app` como en `caddy`, y el contenedor proxy debería usar un montaje de solo lectura
- El contenedor `caddy` debe esperar a que se genere `nocobase.caddy` y luego enlazarlo a `/etc/caddy/Caddyfile` con `ln -sf`
- Expón al host solo el puerto del contenedor de Caddy. El servicio `app` solo necesita `expose: "13000"` dentro de la red de Compose
