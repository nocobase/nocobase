...
- Antes de devolver un archivo heredado de `/storage/uploads/` mediante `alias`, NocoBase comprueba el inicio de sesión a través de `auth_request`; el acceso anónimo devuelve `401` de forma predeterminada
...
Si un programa existente necesita acceso anónimo a `/storage/uploads/`, añade `LEGACY_LOCAL_STORAGE_PUBLIC_ACCESS=true` a `app.environment` y reinicia el contenedor. No elimines `auth_request`; el endpoint de autenticación evalúa esta variable. Los permisos de `/files/` no cambian.
...
Si mantienes Nginx por tu cuenta en lugar de usar la configuración generada, asegúrate de que `/files/` y la ruta correspondiente bajo `APP_PUBLIC_PATH` se reenvíen a NocoBase antes de las reglas de fallback de la SPA. Al servir `/storage/uploads/` mediante `alias`, configura también `auth_request` para no omitir la comprobación de inicio de sesión. Consulta [Proxy inverso con Nginx](../../nocobase-cli/production/reverse-proxy/nginx.md) para ver un ejemplo completo.
